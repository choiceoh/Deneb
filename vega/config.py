...
def _find_path(env_var, local_candidates, fallback_globs=None, check_fn=os.path.isfile, use_which=None):
    """공통 경로 탐색: 환경변수 → 로컬 후보 → 폴백 글롭 → which → None"""
...
MD_DIR = _find_path(
    ...
    ['~/.deneb/agents/main/knowledge/projects',
     '~/.deneb/*/knowledge/projects'],
...
class VegaError(Exception):
    ...
    def __init__(self, message, usage=None, error_type='user_error', recovery=None):
        self.message = message
        self.usage = usage
        self.error_type = error_type
        self.recovery = recovery  # [{'action': 'search "키워드"', 'hint': '설명'}]

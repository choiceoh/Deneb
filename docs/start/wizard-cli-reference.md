...
read_when:
  - You need detailed behavior for deneb onboard
...
This page is the full reference for `deneb onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.deneb/deneb.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `deneb doctor` before continuing.
  ...
  <Step title="Health check">
    ...
    - `deneb status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.deneb/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.deneb/agents/<agentId>/agent/auth-profiles.json`
...
`~/.deneb/credentials/oauth.json` (or `$DENEB_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>

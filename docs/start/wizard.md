...
`deneb dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`deneb configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.deneb/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `deneb doctor` first.
</Note>
...
Use `deneb agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.deneb/workspace-<agentId>`.
...
- CLI command reference: [`deneb onboard`](/cli/onboard)

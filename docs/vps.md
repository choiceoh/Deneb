---
summary: "VPS hosting hub for Deneb (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/deneb-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `DENEB_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `DENEB_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/deneb-compile-cache`

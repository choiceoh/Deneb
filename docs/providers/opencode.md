---
summary: "Use OpenCode Zen and Go catalogs with Deneb"
read_when:
...
OpenCode exposes two hosted catalogs in Deneb:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Deneb keeps the runtime provider ids
...
deneb onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
deneb onboard --auth-choice opencode-go
deneb onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```

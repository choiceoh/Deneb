---
summary: "Use MiniMax M2.5 in Deneb"
read_when:
  - You want MiniMax models in Deneb
...
```bash
deneb plugins enable minimax  # skip if already loaded.
deneb gateway restart  # restart if gateway is already running
deneb onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/deneb/deneb/tree/main/extensions/minimax) for details.
...
Configure manually via `deneb.json`:
...
- Use `deneb models list` and `deneb models set minimax/MiniMax-M2.5` to switch.
...
- Running `deneb configure` and selecting **MiniMax M2.5**, or

---
summary: "Run Deneb with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Deneb against a local SGLang server
...
Deneb can connect to SGLang using the `openai-completions` API.

Deneb can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Deneb will query:

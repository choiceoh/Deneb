---
summary: "Use Venice AI privacy-focused models in Deneb"
read_when:
  - You want privacy-focused inference in Deneb
...
```bash
deneb onboard --auth-choice venice-api-key
```
...
```bash
deneb agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Deneb shows all available Venice models. Pick based on your needs:
...
```bash
deneb models set venice/kimi-k2-5
deneb models set venice/claude-opus-4-6
```

List all available models:

```bash
deneb models list | grep venice
```

## Configure via `deneb configure`

1. Run `deneb configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Deneb automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
deneb agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
deneb agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
deneb agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
deneb agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
deneb agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `deneb models list` to see currently available models. Some models may be temporarily offline.

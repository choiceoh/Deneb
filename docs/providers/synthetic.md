---
summary: "Use Synthetic's Anthropic-compatible API in Deneb"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Deneb registers it as the
...
```bash
deneb onboard --auth-choice synthetic-api-key
```
...
Note: Deneb's Anthropic client appends `/v1` to the base URL, so use

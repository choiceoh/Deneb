---
summary: "Use OpenAI via API keys or Codex subscription in Deneb"
read_when:
  - You want to use OpenAI models in Deneb
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Deneb.
...
```bash
deneb onboard --auth-choice openai-api-key
# or non-interactive
deneb onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Deneb forwards both through the `openai/*` Responses path.
Deneb intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Deneb does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Deneb.
...
deneb onboard --auth-choice openai-codex

# Or run OAuth directly
deneb models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Deneb
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Deneb also supports:

- `openai-codex/gpt-5.3-codex-spark`

Deneb treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Deneb also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Deneb uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Deneb also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Deneb enables it by default for
...
Deneb, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Deneb exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Deneb applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Deneb now auto-enables OpenAI server-side

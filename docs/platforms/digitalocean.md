---
summary: "Deneb on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Deneb on DigitalOcean
  - Looking for cheap VPS hosting for Deneb
title: "DigitalOcean"
---

# Deneb on DigitalOcean

## Goal

Run a persistent Deneb Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
- Channel setup (Telegram, WhatsApp, Telegram, etc.)
...
# View logs
journalctl --user -u deneb-gateway.service -f
```
...
```bash
deneb config set gateway.bind tailnet
deneb gateway restart
```
...
- `~/.deneb/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u deneb --no-pager -n 50
```

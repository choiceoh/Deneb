---
title: Fly.io
description: Deploy Deneb on Fly.io
summary: "Step-by-step Fly.io deployment for Deneb with persistent storage and HTTPS"
read_when:
  - Deploying Deneb on Fly.io
...
fly apps create my-deneb

# Create a persistent volume (1GB is usually enough)
fly volumes create deneb_data --size 1 --region iad
```
...
| `internal_port = 3000`      | Must match `--port 3000` (or `DENEB_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`         | 512MB is too small; 2GB recommended                                      |
| `DENEB_STATE_DIR = "/data"` | Persists state on the volume                                             |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set DENEB_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `DENEB_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `deneb.json` where they could be accidentally exposed or logged.
...
Or visit `https://my-deneb.fly.dev/`

Paste your gateway token (the one from `DENEB_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `DENEB_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/deneb.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/deneb.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/deneb.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/deneb.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/deneb.json"
```
...
**Fix:** Ensure `DENEB_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-deneb

# Release public IPs
fly ips release <public-ipv4> -a my-deneb
fly ips release <public-ipv6> -a my-deneb
...
fly ips allocate-v6 --private -a my-deneb
```
...
fly proxy 3000:3000 -a my-deneb

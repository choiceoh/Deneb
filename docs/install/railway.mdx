...
Deploy Deneb on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/deneb-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/deneb` — Control UI

## What you get

- Hosted Deneb Gateway + Control UI
...
- `DENEB_STATE_DIR=/data/.deneb` (recommended)
- `DENEB_WORKSPACE_DIR=/data/workspace` (recommended)
- `DENEB_GATEWAY_TOKEN` (recommended; treat as an admin secret)

## Setup flow

1. Visit `https://<your-railway-domain>/setup` and enter your `SETUP_PASSWORD`.
2. Choose a model/auth provider and paste your key.
3. (Optional) Add Telegram tokens.
4. Click **Run setup**.
...
1. Go to [https://telegram.com/developers/applications](https://telegram.com/developers/applications)
...
This exports your Deneb state + workspace so you can migrate to another host without losing config or memory.

---
summary: "Run Deneb Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Deneb running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Deneb on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Deneb Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Deneb 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Deneb Gateway in Docker
- Persist `~/.deneb` + `~/.deneb/workspace` on the host (survives restarts/rebuilds)
...
When binding to LAN (`DENEB_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

```bash
docker compose run --rm deneb-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
```

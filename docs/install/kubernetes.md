---
summary: "Deploy Deneb Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Deneb on a Kubernetes cluster
  - You want to test Deneb in a Kubernetes environment
title: "Kubernetes"
---

# Deneb on Kubernetes

A minimal starting point for running Deneb on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Deneb is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/deneb 18789:18789 -n deneb
...
```bash
kubectl get secret deneb-secrets -n deneb -o jsonpath='{.data.DENEB_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: deneb (configurable via DENEB_NAMESPACE)
├── Deployment/deneb        # Single pod, init container + gateway
├── Service/deneb           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/deneb-config  # deneb.json + AGENTS.md
└── Secret/deneb-secrets    # Gateway token + API keys
```
...
Edit `deneb.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret deneb-secrets -n deneb \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/deneb -n deneb
```

### Custom namespace

```bash
DENEB_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/deneb/deneb:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # deneb.json + AGENTS.md

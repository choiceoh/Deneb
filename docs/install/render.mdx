...
Deploy Deneb on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/deneb/deneb)
...
repository configures everything needed to run Deneb:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: DENEB_STATE_DIR
        value: /data/.deneb
      - key: DENEB_WORKSPACE_DIR
        value: /data/workspace
      - key: DENEB_GATEWAY_TOKEN
...
4. Optionally configure messaging channels (Telegram, Telegram)
5. Click **Run setup**

### Access the Control UI

The web dashboard is available at `https://<your-service>.onrender.com/deneb`.
...
If you use the original Deneb repository, Render will not auto-deploy your Deneb. To update it, run a manual Blueprint sync from the dashboard.
...
For Deneb, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Deneb host.

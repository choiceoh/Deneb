---
summary: "Optional Docker-based setup and onboarding for Deneb"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Deneb on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Deneb in Docker)
...
- builds the gateway image locally (or pulls a remote image if `DENEB_IMAGE` is set)
- runs onboarding
...
- `DENEB_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/deneb/deneb:latest`)
- `DENEB_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `DENEB_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `DENEB_EXTRA_MOUNTS` — add extra host bind mounts
- `DENEB_HOME_VOLUME` — persist `/home/node` in a named volume
- `DENEB_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `DENEB_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `DENEB_SANDBOX=1` for local builds.
- `DENEB_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `DENEB_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `DENEB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `DENEB_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `DENEB_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm deneb-cli dashboard --no-open`.
...
```bash
export DENEB_SANDBOX=1
export DENEB_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `deneb-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `DENEB_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm deneb-cli gateway probe
docker compose run -T --rm deneb-cli devices list --json
```
...
`deneb-cli` uses `network_mode: "service:deneb-gateway"` so CLI commands can
...
network path instead of the bundled `deneb-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `deneb-cli`.
...
- [GitHub Container Registry package](https://github.com/deneb/deneb/pkgs/container/deneb)

Use image name `ghcr.io/deneb/deneb` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/deneb/deneb`
- `org.opencontainers.image.url=https://deneb.ai`
- `org.opencontainers.image.documentation=https://docs.deneb.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=Deneb`
- `org.opencontainers.image.description=Deneb gateway and CLI runtime container image`
...
image instead, set `DENEB_IMAGE` before running the script:

```bash
export DENEB_IMAGE="ghcr.io/deneb/deneb:latest"
./docker-setup.sh
```

The script detects that `DENEB_IMAGE` is not the default `deneb:local` and
...
`docker-compose.yml` and helper files. `DENEB_IMAGE` skips local image build
...
```bash
docker build -t deneb:local -f Dockerfile .
docker compose run --rm deneb-cli onboard
docker compose up -d deneb-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`DENEB_EXTRA_MOUNTS` or `DENEB_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm deneb-cli dashboard --no-open
docker compose run --rm deneb-cli devices list
...
`DENEB_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`deneb-gateway` and `deneb-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export DENEB_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `DENEB_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `DENEB_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export DENEB_HOME_VOLUME="deneb_home"
./docker-setup.sh
```
...
- If you change `DENEB_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `DENEB_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export DENEB_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `DENEB_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export DENEB_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg DENEB_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `DENEB_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export DENEB_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
```bash
docker compose run --rm deneb-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

If you need Playwright to install system deps, rebuild the image with
`DENEB_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `DENEB_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `DENEB_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.deneb`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/deneb-config /path/to/deneb-workspace
```
...
```bash
docker compose run --rm deneb-cli channels login
```
...
background. In plain terms: Docker keeps checking if Deneb is still
...
```bash
docker compose exec deneb-gateway node dist/index.js health --token "$DENEB_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `DENEB_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm deneb-cli config set gateway.mode local
docker compose run --rm deneb-cli config set gateway.bind lan
docker compose run --rm deneb-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`DENEB_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.deneb/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `DENEB_CONFIG_DIR` to `/home/node/.deneb` and `DENEB_WORKSPACE_DIR` to `/home/node/.deneb/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/deneb/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.deneb/logs/gateway.log`, `~/.deneb/logs/gateway.err.log`, and `/tmp/deneb/deneb-gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.deneb/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Deneb auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `deneb sandbox recreate ...` command.
...
This builds `deneb-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `deneb-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "deneb-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `deneb-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`deneb-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Deneb provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `DENEB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `DENEB_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `DENEB_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-deneb-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-deneb-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-deneb-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/deneb/deneb/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Deneb runs commands with `sh -lc` (login shell), which

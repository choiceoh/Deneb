...
Deneb supports **camera capture** for agent workflows:
...
```bash
deneb nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
deneb nodes camera snap --node <id> --facing front
deneb nodes camera clip --node <id> --duration 3000
deneb nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`deneb.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `deneb` CLI to invoke camera commands on the macOS node.

Examples:

```bash
deneb nodes camera list --node <id>            # list camera ids
deneb nodes camera snap --node <id>            # prints MEDIA:<path>
deneb nodes camera snap --node <id> --max-width 1280
deneb nodes camera snap --node <id> --delay-ms 2000
deneb nodes camera snap --node <id> --device-id <id>
deneb nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
deneb nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
deneb nodes camera clip --node <id> --device-id <id>
deneb nodes camera clip --node <id> --no-audio
```

Notes:

- `deneb nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
deneb nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```

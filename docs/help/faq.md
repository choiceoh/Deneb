---
summary: "Frequently asked questions about Deneb setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [Can I ask Deneb to update itself?](#can-i-ask-deneb-to-update-itself)
  ...
  - [How important is it to run Deneb on a dedicated machine?](#how-important-is-it-to-run-deneb-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run Deneb in a VM and what are the requirements](#can-i-run-deneb-in-a-vm-and-what-are-the-requirements)
- [What is Deneb?](#what-is-deneb)
  - [What is Deneb, in one paragraph?](#what-is-deneb-in-one-paragraph)
  - [Value proposition](#value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for Deneb](#what-are-the-top-five-everyday-use-cases-for-deneb)
  - [Can Deneb help with lead gen outreach ads and blogs for a SaaS](#can-deneb-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Deneb saved locally?](#is-all-data-used-with-deneb-saved-locally)
  - [Where does Deneb store its data?](#where-does-deneb-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [Recommended backup strategy](#recommended-backup-strategy)
  - [How do I completely uninstall Deneb?](#how-do-i-completely-uninstall-deneb)
...
- [Config basics](#config-basics)
  ...
  - [Can the Deneb browser run headless?](#can-the-deneb-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Deneb instances talk to each other (local + VPS)?](#can-two-deneb-instances-talk-to-each-other-local-vps)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Deneb instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-deneb-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Deneb but keep it installed?](#how-do-i-completely-reset-deneb-but-keep-it-installed)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `deneb gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-deneb-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `deneb gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-deneb-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run Deneb in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-deneb-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [ELI5: `deneb gateway restart` vs `deneb gateway`](#eli5-deneb-gateway-restart-vs-deneb-gateway)
...
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose Deneb to inbound DMs?](#is-it-safe-to-expose-deneb-to-inbound-dms)
...
7. **Gateway snapshot**
   ...
   ```bash
   deneb health --json
   deneb health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://deneb.ai/install.sh | bash -s -- --install-method git
```

This installs Deneb **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/deneb/deneb/issues](https://github.com/deneb/deneb/issues)
[https://github.com/deneb/deneb/pulls](https://github.com/deneb/deneb/pulls)
...
- `deneb status`: quick snapshot of gateway/agent health + basic config.
- `deneb models status`: checks provider auth + model availability.
- `deneb doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `deneb status --all`, `deneb logs --follow`,
`deneb gateway status`, `deneb health --verbose`.

Quick debug loop: [First 60 seconds if something is broken](#first-60-seconds-if-something-is-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### Recommended way to install and set up Deneb
...
If you don't have a global install yet, run it via `pnpm deneb onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `DENEB_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `deneb config get gateway.auth.token` (or generate one: `deneb doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `deneb gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `deneb gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install Deneb on the new machine.
2. Copy `$DENEB_STATE_DIR` (default: `~/.deneb`) from the old machine.
...
under `~/.deneb/` (for example `~/.deneb/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-deneb-store-its-data),
...
[https://github.com/deneb/deneb/blob/main/CHANGELOG.md](https://github.com/deneb/deneb/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### Cannot access docs.deneb.ai (SSL error)

Some Comcast/Xfinity connections incorrectly block `docs.deneb.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.deneb.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsdenebai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/deneb/deneb/tree/main/docs](https://github.com/deneb/deneb/tree/main/docs)
...
```bash
curl -fsSL https://deneb.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://deneb.ai/install.sh | bash -s -- --beta --verbose
```
...
deneb update
deneb update status
deneb update --channel stable|beta|dev
deneb update --tag <dist-tag|version>
deneb update --no-restart
```

If you must automate from an agent:

```bash
deneb update --yes --no-restart
deneb gateway restart
```
...
`deneb onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run Deneb with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like Deneb.
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `deneb models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `deneb models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `deneb models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. Deneb no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Deneb supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Deneb fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like Deneb. Onboarding can run the OAuth flow for you.

See [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers), and [Onboarding (CLI)](/start/wizard).

### How do I set up Gemini CLI OAuth

Gemini CLI uses a **plugin auth flow**, not a client id or secret in `deneb.json`.

Steps:

1. Enable the plugin: `deneb plugins enable google`
2. Login: `deneb models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. Deneb needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
Onboarding accepts `@username` input and resolves it to a numeric ID, but Deneb authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `deneb logs --follow` and read `from.id`.
...
If you run Deneb via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the Deneb code install. Your state
(`~/.deneb`) and workspace (`~/.deneb/workspace`) stay untouched.
...
### How important is it to run Deneb on a dedicated machine
...
Deneb is lightweight. For a basic Gateway + one chat channel:
...
Deneb is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Deneb
...
### Can Deneb help with lead gen outreach ads and blogs for a SaaS
...
Deneb draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Deneb is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Deneb when you
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.deneb/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.deneb/deneb.json`). Precedence is `<workspace>/skills` > `~/.deneb/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.deneb/deneb.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.deneb/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Deneb treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `DENEB_SKIP_CRON` is not set.
...
```bash
deneb cron run <jobId> --force
deneb cron runs --id <jobId> --limit 50
```
...
### Can Deneb run tasks on a schedule or continuously in the background
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Deneb workspace); Deneb treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.deneb/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

### How do I use my existing signed-in Chrome with Deneb

Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

```bash
deneb browser --browser-profile user tabs
deneb browser --browser-profile user snapshot
```
...
Deneb memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Deneb also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Deneb auto-selects a provider when it
...
### Is all data used with Deneb saved locally

No - **Deneb's state is local**, but **external services still see what you send them**.
...
### Where does Deneb store its data

Everything lives under `$DENEB_STATE_DIR` (default: `~/.deneb`):

| Path                                                         | Purpose                                                            |
| ------------------------------------------------------------ | ------------------------------------------------------------------ |
| `$DENEB_STATE_DIR/deneb.json`                                | Main config (JSON5)                                                |
| `$DENEB_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
...
Legacy single-agent path: `~/.deneb/agent/*` (migrated by `deneb doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.deneb/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.deneb`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
  `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.deneb`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.deneb/skills`).

Default workspace is `~/.deneb/workspace`, configurable via:
...
Do **not** commit anything under `~/.deneb` (credentials, sessions, tokens, or encrypted secrets payloads).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall Deneb
...
`workspace` to the repo root. The Deneb repo is just source code; keep the
...
Deneb reads an optional **JSON5** config from `$DENEB_CONFIG_PATH` (default: `~/.deneb/deneb.json`):

```
$DENEB_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.deneb/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `DENEB_GATEWAY_TOKEN`).
...
Deneb enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `deneb doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one Deneb.` every time.
- `random`: rotating funny/seasonal taglines (default behavior).
- If you want no banner at all, set env `DENEB_HIDE_BANNER=1`.
...
**Recommended:** run `deneb configure --section web` and choose a provider.
...
- Daemons read env vars from `~/.deneb/.env` (or the service environment).
...
### Can the Deneb browser run headless
...
- Gateway is running: `deneb gateway status`
- Gateway health: `deneb status`
- Channel health: `deneb channels status`
...
### Can two Deneb instances talk to each other local VPS
...
`deneb agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
deneb agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.deneb/deneb.json`).
- If you have no backup, re-run `deneb doctor` and reconfigure channels/models.
...
### How does Deneb load environment variables

Deneb reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.deneb/.env` (aka `$DENEB_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.deneb/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`DENEB_LOAD_SHELL_ENV=1`, `DENEB_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`deneb models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means Deneb won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.deneb/.env`:
...
### Is there a way to make a team of Deneb instances one CEO and many agents
...
### How do I completely reset Deneb but keep it installed
...
```bash
deneb reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `DENEB_PROFILE`), reset each state dir (defaults are `~/.deneb-<profile>`).
- Dev reset: `deneb gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), Deneb skips the heartbeat run to save API calls.
...
- **Disk growth:** sessions + transcripts live under `~/.deneb/agents/<agentId>/sessions/`.
...
Deneb's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Deneb currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
4. Run `deneb onboard` and choose `Ollama`
...
- for manual switching, use `deneb models list` and `deneb models set ollama/<model>`
...
- `deneb models set ...` (updates just model config)
- `deneb configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.deneb/deneb.json`
...
Yes. Deneb ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.deneb/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `deneb agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so Deneb can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.deneb/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.deneb/agent/*` (migrated by `deneb doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.deneb/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `deneb models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `deneb models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `deneb models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.deneb/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    deneb models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Deneb will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Deneb now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Deneb uses provider-prefixed IDs like:
...
Deneb may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `deneb models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
deneb models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
deneb models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
deneb models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
deneb models auth order set --provider anthropic --agent main anthropic:default
```
...
```
--port > DENEB_GATEWAY_PORT > gateway.port > default 18789
```

### Why does deneb gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `deneb gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does deneb gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `DENEB_STATE_DIR` mismatch).

Fix:

```bash
deneb gateway install --force
```
...
Deneb enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `deneb gateway --port <port>`.

### How do I run Deneb in remote mode client connects to a Gateway elsewhere
...
- Fastest: `deneb dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `deneb doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `DENEB_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- If mismatch persists after the one retry, rotate/re-approve the paired device token:
  - `deneb devices list`
  - `deneb devices rotate --device <id> --role operator`
- Still stuck? Run `deneb status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `DENEB_CONFIG_PATH` (per-instance config)
- `DENEB_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `deneb --profile <name> …` per instance (auto-creates `~/.deneb-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `deneb --profile <name> gateway install`.

Profiles also suffix service names (`ai.deneb.<profile>`; legacy `com.deneb.*`, `deneb-gateway-<profile>.service`, `Deneb Gateway (<profile>)`).
...
```
deneb tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/deneb/deneb-YYYY-MM-DD.log
```
...
1. Is the Gateway running? `deneb gateway status`
2. Is the Gateway healthy? `deneb status`
3. Does the UI have the right token? `deneb dashboard`
...
```bash
deneb channels status
deneb channels logs --channel telegram
```

Then match the error:

- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Deneb already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
### ELI5 deneb gateway restart vs deneb gateway

- `deneb gateway restart`: restarts the **background service** (launchd/systemd).
- `deneb gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `deneb gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Deneb assistant setup](/start/deneb) and [Agent send](/tools/agent-send).

CLI sending:

```bash
deneb message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose Deneb to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `deneb pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `deneb pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `deneb doctor` to surface risky DM policies.

...
| `pilot.CallTinyYesNo` / `CallRoleYesNo` | tiny (역할 지정) — **한 단어 YES/NO 판정 전용**: 1토큰·temperature 0·비스트리밍 호출로 첫 토큰의 logprobs 에서 P(YES) 를 읽는다. 백엔드가 logprobs 를 안 주면(오픈라우터 폴백 칸 전부) 토큰 텍스트로 강등. 후보·요청 셰이핑은 `CallRoleLLM` 과 공유(`roleCandidates`·`shapeRoleExtra`) |
...
| 프로젝트별 최신 근황 digest | `domain/wiki/project_digest.go` (드림 사이클 Phase 3d) → 프로젝트 대표페이지 `## 현재 상태` 섹션 (`project_status.go`) → `miniapp.project.digests` (안드로메다 프로젝트 홈·오늘 마감) | **lightweight** | 드림 사이클이 소비하는 그 일지/메모 입력을 프로젝트별로 롤업(헤드라인+불릿 2~3)하는 내부 배경 요약. open_loops와 동형의 격리된 1콜, fail-open, 실제 `프로젝트/*` 페이지에 앵커. **왜 lightweight인가**: 컴팩션·youtube와 같은 내부 배경 요약 도그마(로컬·바운드) — analysis(클라우드)로 둘 이유 없음. 화면은 대표페이지 섹션만 읽어 LLM 핫패스 아님. 메일분석 이벤트 갱신(`wiki_mail_analysis.go` → `AppendProjectStatusLine`)은 **LLM 없는 결정적 날짜 불릿**이라 아래 표 참조 |
...
| 폰 알림 tiny 게이트 (판단턴 앞 1차 트리아지) | `runtime/phoneevents/tiny_gate.go` (`worthFullJudgment`) | **tiny** (`CallTinyYesNo`) | 알림마다 호출되는 푸시 가치 1차 판정. **확률 게이트**: P(YES) ≥ 문턱(`DENEB_PHONE_GATE_THRESHOLD`, 기본값은 코드 상수)이면 판단턴으로 — 예전 텍스트 게이트는 탐욕 디코딩이라 사실상 0.5 고정 문턱이었다. logprobs 없는 폴백(오픈라우터 nemotron)은 토큰 판정(명시적 NO 만 드롭), 호출 실패는 fail-open. 판정은 전부 Info 로그(`phone-event tiny-gate`, 드롭 포함)로 남는다 — 예전엔 드롭이 Debug 뿐이라 재현율을 원장 재생으로만 잴 수 있었다(2026-09-19 실측: 537건 재생). |

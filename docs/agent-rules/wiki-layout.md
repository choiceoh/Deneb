...
- **대표페이지는 회상 큐(cues:)를 채운다**: 실사용 어휘(메일 제목·회의가 이 프로젝트를 부르는 말) 중 페이지에 없는 토큰. 결정적 백필 도구 `scripts/audit/wiki_cue_backfill.py`(dry-run 기본, 타 프로젝트 정체성 토큰 자동 기각) — 2026-07-21 보급률 8/66에서 백필.

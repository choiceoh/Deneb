...
- **어휘 고정**: `제안 → 견적 → 입찰 → 계약협의 → 시공 → 운영` (진행 순) + `종결`/`유실` (말단). 어휘 밖 값은 파스/렌더에서 버려짐(`page.go:normalizeStage`).
- 현장 페이지의 `status:`(후보/계약/개설/준공 — 현장 지도용 per-site 생애)와 **별개 축**: stage는 딜의 단계, 프로젝트당 하나.
- **현장 문서 게이트**: 현장 상세 섹션·write-site 현장 페이지는 `계약협의` 이상부터 (기존 O&M 자산 예외). 그 전 단계는 `sites:` 메타데이터 한 줄까지만 — 운영자 확정 "최소 계약서 협의 수준은 되어야지".
...
- **소비처**: ① 진행상황 모아보기(`miniapp.project.digests` → 네이티브·안드로메다)가 이 값으로
  섹션 그룹핑 (미기입은 "거래처 미지정" 그룹, 전부 미기입이면 flat), ② 회상 프로젝트 앵커
...
- 하베스트는 "○○ 어떻게 됐어요?" 질문만 던졌다 — 답을 안 하면 회의 사실이 증발. 이제 매칭된 회의가
  ...
  조용히 남긴다(`meeting_attendance.go`의 `RecordMeetingAttendance`, 이벤트 키로 dedup·영속).

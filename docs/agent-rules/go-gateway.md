...
- `internal/runtime/rpc/` — Registry-based RPC method dispatcher (thread-safe). 200+ methods.
...
- `internal/pipeline/chat/toolwire/` — Tool registration hub: wires tool implementations (from tools/) with JSON schemas (from schema/tool_schemas_gen.go) into a ToolRegistrar. Contains schema/tool_schemas.json (codegen source) and schema/tool_schemas_gen.go (generated). Never imports chat/.
- `internal/pipeline/chat/toolreg/` — Thin leaf: session tool-preset constants/helpers only. Registration wiring lives in `toolwire/`.
- `internal/pipeline/chat/tools/` — Pure tool implementations (read/write/edit/grep, exec, web, wiki, knowledge, polaris, mail_archive, code_action, fetch_tools, calendar, contacts, …; subpackages under `runtimeops/`, `mailarchive/`, `codeaction/`). Depends on toolport/ for execution types and tooldeps/ for wiring bags.
- `internal/pipeline/chat/toolreg_core.go` — Thin wrapper: calls toolwire.RegisterCoreTools() + registers chat-internal-state tools (skills, wiki, contacts, calendar, persona, notebook) and the registry bridge (fetch_tools, code_action).
...
> 위 목록은 chat 파이프라인 중심의 상세 노트다. `internal/` 전체(runtime/pipeline/ai/domain/platform/infra/hanja/…)의 한 줄 오리엔테이션 맵은 `docs/agent-rules/architecture.md` "Gateway Module Map" 참조.

...
워크플로가 `runs-on: [self-hosted, srv4]` 이라 srv4 에 러너가 등록돼야 동작한다. 러너가 게이트웨이와 같은 호스트라 `~/.cache/deneb-apk` 가 **곧 serve dir** — 별도 동기화 불필요. 현역 러너: `srv4-apk` (2026-07-06 등록, srv1 에 있던 구 APK 러너는 해제).
...
- 커스텀 라벨 `srv4` 은 `.github/actionlint.yaml` 에 등록돼 있어 워크플로 린트(`workflow-sanity.yml`)를 통과한다.

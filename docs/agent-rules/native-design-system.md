...
| 화면 프레임 → `DenebScreenScaffold(title, onBack, tabBar?)` (flat AMOLED, `←`, 제목) | 폼 **기반**: `Switch`·`Checkbox`·`Slider` 직접 / 단일 선택→`DenebSegmentedRow`+`DenebSegment`(Material `SegmentedButton`은 2026-09-20 은퇴 — 행 크기 활자, 선택=ink+2dp 밑줄, 박스·fill 없음) / `OutlinedTextField`→`DenebTextField` (박스 없이 라벨·값·hairline 밑줄; 검색·필터 입력은 `DenebUnderlineSearchField`) / 칩→`DenebChip` |
...
| **뷰 전환(피벗)** → `DenebPivotRow(labels, selectedIndex, onSelect)` — Light 활자, **밝기만으로 상태**(활성=ink·나머지=흐림), 밑줄·fill·액센트·하airline 없음. 제목 자리면 `viewTitle`(피드·결재·로그), 제목 아래 페이지 안이면 `style = DenebType.subject`(스킬 목록·Propus 로그 / 1일·7일 / 노드·모델·작업). 넘치면 줄바꿈·축소 대신 가장자리로 흘린다(Zune bleed) | `DenebSegmentedRow`는 **폼 선택**(검색 모드·반복 종류·기간 집계·카드 안 탭)에만 — 같은 ink/흐림 목소리에 선택 표시로 2dp 밑줄 하나(엔진 모델 탭 문법). 페이지가 *무엇을 보여줄지* 바꾸는 데 쓰면 위반 |
| **태그(배지)** → `DenebBadge(text, color, containerColor)` — `meta` 크기·4dp 모서리·6/1dp 패딩. 색은 호출부의 일(범주·시맨틱, ADR 0008), 모양은 아님 — 999dp/50% 스타디움 배지 금지 | — |
...
>
> ★**북극성 정정(2026-08-30, ADR 0007)**: 원문은 제품 북극성을 "토스식 슈퍼앱"으로 적고 `[[project_superapp_vision]]`을 인용했으나 **그 위키 페이지는 890페이지 중에 없다** — 존재하지 않는 근거였다. 방향도 제품과 반대다(슈퍼앱=다서비스·대중 온보딩·서비스 메뉴 vs Deneb=단일 사용자 비서실장 단일 페르소나, ADR 0001). 북극성은 **"메뉴가 아니라 결과"**로 대체됐고, 토스는 **장인 참조**(밀도·한국어 타이포·절제·모션 품질)로만 남는다. 운영자가 건넨 외부 화면 참조는 [docs/research/design-references/](docs/research/design-references/README.md)에 "빌릴 것 / 빌리면 안 되는 것" 틀로 쌓는다(2026-09-06 Grok 비서실장 봇부터).
...
- **피벗·상태 ★활자화(2026-09-20, 운영자 "Typo 중심의 윈도폰·Zune HD UI 더 적용")**: 뷰 전환기 3곳(스킬 목록|Propus 로그 파란 텍스트 탭, 관찰 1일|7일, 플릿 알약 탭)이 전부 `DenebPivotRow`가 됐다 — 피드|결재|로그가 이미 쓰던 Zune 문법을 프리미티브로 올린 것. 빈·오류 상태는 아이콘+중앙정렬을 버리고 `subject` 문장 왼쪽 정렬로. **빌리지 않은 것**: 페이지 제목 확대(원리 1 — 비계가 콘텐츠보다 커진다), 타일 홈(북극성 "메뉴가 아니라 결과"), 액센트 타일 색, 턴스타일 모션(모션 ADR 미결). 참조 카드는 [docs/research/design-references/README.md](../research/design-references/README.md). 자가개선 코딩 탭의 **카운트 달린 상태 필터**(전체 12·대기 3…)는 목록을 좁히는 필터라 손대지 않았다 — 피벗으로 바꿀지는 별도 판단.
- **알약 은퇴 ★(2026-09-20, 운영자 "이상하게 못생긴 알약모양 그런 것들도 좀 바꿔주지")**: 스타디움 모양 크롬을 전부 걷어냈다 — Material `SegmentedButton`(→`DenebSegment` 활자+밑줄), `SuggestionChip` 2곳(첨부 칩 → `DenebChip`), 하단 탭바의 M3 인디케이터 필(→없음, 채운 글리프+ink 라벨이 곧 상태), deneb-ui 카드 `<tabs>`의 필 컨테이너(→`DenebSegment`), 999dp/50% 배지 5곳(플릿 파라미터·잡 상태·결재 양식·RSI 상태·카드 stat 추이 → `DenebBadge` 4dp). 남은 원형은 **기능 원**뿐이다: 상태 점·아바타·아이콘 버튼 리플·카운트 배지·슬라이더 썸. 채팅 컴포저의 둥근 입력창(28dp)은 운영자가 Grok 참조(2026-09-06)에서 본 형태라 손대지 않았다. 정보 태그(파라미터 수·양식)는 액센트가 아니라 중립색으로 — 만질 수 없는 것에 `primary`를 쓰지 않는다(ADR 0008 job 1).
...
1. **모션** — 단일 소스 `ui/DenebMotion.kt`(토큰·프리셋·`denebPressable`·`denebBreathing`) + `ui/DenebNavMotion.kt`(화면 전환). 인라인 `tween(매직넘버)` 금지 — 새 의도면 토큰을 추가하고 이름을 붙인다. **화면 전환 문법: 바텀바 유지 = 측면 이동(빠른 페이드) / 바 숨김 = 드릴인(끝에서 슬라이드+부모 1/4 패럴랙스, pop은 역방향)** — 분류식은 `denebBottomBarRoutes` 하나라 크롬과 모션이 항상 일치한다. 리스트→상세 연속성은 `denebSharedBounds("surface-field-id")`(예: `"mail-subject-42"`) — 스코프 부재 시 no-op이므로 프리뷰·데스크톱 분할뷰에 안전, previewable body에 스코프 배선 불요. 새 destination은 `composable<T>` 대신 `denebComposable<T>`. **햅틱은 시맨틱 어휘로만**(`ui/components/Haptics.kt`) — ★**탭은 `denebPressable`이 이미 발사한다**(2026-09-01): 그 모디파이어를 쓰는 행·카드는 수동 `tap()` 금지(중복), 클릭이 탭보다 풍부하면 `haptic = false`로 끄고 그쪽 어휘를 부른다. 네 규칙(`design-lint`)이 누락·중복·결정 다이얼로그 무음·프리미티브 소실을 각각 잡는다: 탭=`tap`(VirtualKey 강도), 커밋=`confirm`/파괴=`reject`/토글=`toggle(on)`(멀티선택 `FilterChip` 포함)/PTR 트리거=`refresh`/수제 드래그 문턱=`arm`; `combinedClickable`(=`denebPressable` onLongClick)은 롱프레스 햅틱을 **자동 발사**하므로 수동 `longPress()` 중복 금지; 뒤로/취소/수동 이벤트(스트림·도착)는 무음. ★**결정 다이얼로그**(2026-09-01): 취소 버튼이 있는 `AlertDialog`/`DatePickerDialog`의 확인 버튼이 결정이 나는 곳 — 커밋(저장·보내기·다운로드·선택기 확인)=`confirm()`, 파괴·폐기(삭제·나가기·작업 취소)=`reject()`; 파괴 다이얼로그를 **여는** 버튼도 `reject()`(크론·스킬·모델 삭제 관례). 한 버튼짜리 알림(닫기/확인)은 무음. ★**형제 스와이프**(피드|결재|로그): `DenebSiblingSwipeHost`가 문턱을 넘는 MOVE에서 `arm()`을 발사하고 릴리즈는 무음(PTR과 같은 계약, 결정은 `SiblingSwipeTracker`) — 피벗 라벨의 탭은 `DenebTitlePivot`이 소유하므로 화면은 두 곳에 **맨 목적지 람다**를 넘긴다(람다 안의 `tap()` 금지).

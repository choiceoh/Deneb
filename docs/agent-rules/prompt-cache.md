...
2. **per-turn 주입은 마지막 user 메시지의 wire-only suffix 로.** `chat/run_tail_inject.go` (`buildTailAdditions` + `injectTailAdditions`) 가 단일 진입점이다: recall 증거, auto-delivery 지시문, 그리고 **응답 언어·모드 앵커**(`responseLanguageAnchor` — 항상 마지막 추가분; 시스템 머리의 "Always respond in Korean"이 5만 토큰 뒤에 있어도 모델이 마지막으로 읽는 바이트가 "기록을 이어쓰지 말고 한국어로 답하라"가 되게 한다. 2026-09-17 텔레마코스 응답 오염 사건, [docs/research/telemachus-response-contamination-2026-09-18.md](../research/telemachus-response-contamination-2026-09-18.md))가 여기로 간다. transcript 에는 깨끗한 원문이 저장돼 **같은 런 안의** 다음 턴 히스토리는 이번 턴이 캐시한 prefix 와 byte-동일하다. ★도구 단계(마지막 메시지 = tool-results user 메시지)에는 같은 앵커가 `BeforeAPICall` per-request 사본의 **꼬리 text 블록**으로 붙는다(`chat/run_midrun_anchor.go`, trailing-cache 훅 뒤): 히스토리 바이트는 불변이고 요청 N 의 캐시 prefix 는 깨끗한 tool results 끝까지라 다음 단계가 그대로 재사용한다 — 비용은 앵커 자신의 토큰뿐. content-prefix 프로바이더(§1.6, kimi)에도 적용한다 — §1.6이 금하는 것은 *히스토리 변이*이고 이 블록은 히스토리를 건드리지 않아 손실이 직전 단계 앵커가 있던 **256토큰 청크 하나**로 바운드된다(Anthropic의 이동하는 trailing 마커와 같은 부류). 2026-09-18 main 이 kimi 로 옮겨간 뒤라 제외했으면 주 경로에 앵커가 없었을 것. 특정 모델에서 꼬리 노트가 툴 루프를 흔드는 것이 실측되면 **모델별로** 끈다 — builtin `modelcaps.MidRunAnchorByDefault` 또는 deneb.json `midRunAnchor: false`(`NoMidRunAnchor`), 전역 스위치가 아니다. 비용 = 추가분 자신의 토큰뿐. ★단 **런 경계**에서는 히스토리 리로드가 클린 원문으로 되돌아와 그 메시지 지점부터 prefix 가 갈라진다 — content-prefix 캐시(§1.6)에서 대화 전체 재과금의 주범이었고, `chat/tail_register.go` 의 재부착이 이를 봉합한다.
...
두 cheap 패스 공통 **보호 목록** (`compaction/protected.go:protectedToolResultIDs`): ① `fetch_tools` 결과 — 그 결과가 곧 도구 스키마라서 지우면 모델이 동일 재fetch를 반복한다 (실측 2026-07-05: fetch_tools 호출의 20%가 런 내 동일입력·동일결과 반복, 한 런에서 7회). ② `skills` action=read/list 결과 — 스킬 본문/매니페스트는 결정적이고 JIT 로딩 규약상 세션 내 반복 참조된다 (실측 2026-07-20 7d: skills 호출 73건 중 38건(52%)이 4턴 초과 간격 동일 재실행 = Tier 2b 가 이미 스텁한 창이며, 결과가 남아있는 창에서의 재실행은 0건 — 스텁이 재호출의 원인. 반복 전건이 summary-tier 미발화 세션에서 발생해 cheap pass 보호로 해결되는 부류임을 교차 확인). 변이 액션(create/patch/…)의 ack 는 256 rune 미만이라 어차피 스텁 대상이 아니다. LLM 요약 tier 는 보호 대상이 아니므로 초장기 런에서는 여전히 압축될 수 있다 (상주 비용 바운드). **보호 중복 제거 카브아웃** (`supersededProtectedResultIDs`): 모델이 동일 (tool name + input JSON) 보호 호출을 습관적으로 반복해 결과가 byte-동일하면 (실측 2026-07-20 7d cache-cost-audit: fetch_tools 반복 69건 중 36건이 결과가 상주 중인 비압축 세션에서 발생, 중위 사본 12.6KB) 최신 비에러 사본만 보호를 유지하고 커토프 이전의 오래된 중복은 Tier 2b 가 전용 placeholder 로 스텁한다 — 내용은 최신 사본에 그대로 상주하므로 재fetch 유발 없음. 스텁의 prefix 1회 파손 비용은 기존 256 rune 임계(`DefaultStubMinChars`)가 동일하게 게이트하고, 출력 드리프트·에러 결과·스필오버 결과(sp_ id 유일)는 동일성 검사에서 자연 배제된다.

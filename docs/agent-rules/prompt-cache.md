...
두 cheap 패스 공통 **보호 목록** (`compaction/protected.go:protectedToolResultIDs`): ① `fetch_tools` 결과 — 그 결과가 곧 도구 스키마라서 지우면 모델이 동일 재fetch를 반복한다 (실측 2026-07-05: fetch_tools 호출의 20%가 런 내 동일입력·동일결과 반복, 한 런에서 7회). ② `skills` action=read/list 결과 — 스킬 본문/매니페스트는 결정적이고 JIT 로딩 규약상 세션 내 반복 참조된다 (실측 2026-07-20 7d: skills 호출 73건 중 38건(52%)이 4턴 초과 간격 동일 재실행 = Tier 2b 가 이미 스텁한 창이며, 결과가 남아있는 창에서의 재실행은 0건 — 스텁이 재호출의 원인. 반복 전건이 summary-tier 미발화 세션에서 발생해 cheap pass 보호로 해결되는 부류임을 교차 확인). 변이 액션(create/patch/…)의 ack 는 256 rune 미만이라 어차피 스텁 대상이 아니다. LLM 요약 tier 는 보호 대상이 아니므로 초장기 런에서는 여전히 압축될 수 있다 (상주 비용 바운드).

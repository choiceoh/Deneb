...
> Deneb 는 메인 챗 LLM 외에도 **로컬 GPU 플릿에서 상주 서빙되는 전용 모델들**을 호출한다. 호스트 배치(2026-07-17 갱신): **srv4** = 게이트웨이·wormhole·diffusiongemma 엔진(:8100)·메일서버, **srv1** = **2026-09-06 운영자가 사이드카를 전부 내렸다 — 지금 남은 건 sparkfleet(:18900) 뿐이다.** 내려간 것: PaddleOCR-VL(:18011)·MOSS-Transcribe-Diarize(:18014 — 2026-07-18 VibeVoice-ASR :18013 대체)·hy-mt2(:8102)·lfm2.5(:8101) (qwen3.6(:8000)은 그보다 앞선 2026-08-06부터 죽어 있었다 — 워커와 RAM 경합), **srv2** = 로컬 엔진(:8000) — **2026-09-06 현재 `glm-5.3-flash`(NVFP4)를 서빙한다**(`/v1/models` 실측). ⚠️웜홀의 `deepseek-v4-flash`·`dsv4-nothink` 엔트리는 여전히 이 주소를 가리키지만 `upstreamModel`이 `deepseek-v4-flash`라 **서버에 없는 모델을 요청한다**. 2026-09-12 갱신: srv2 가 vLLM 에서 **ST 엔진**으로 바뀌면서 이 증상이 한 번 바뀌었다 — vLLM 은 모르는 모델에 404 를 내서 fallback(`glm-5.3`, z.ai)으로 샜지만, **ST 는 이름을 검사하지 않고 요청한 이름을 응답에 그대로 되돌려 주면서 GLM-5.3 으로 답했다**(답은 맞고 이름표만 틀린, 청구서에도 안 보이는 쪽). stkernel 이 이를 고쳐 이제 ST 도 vLLM 과 같은 문장으로 404 를 내므로 **다시 유료 fallback 으로 샌다**. 어느 쪽이든 조치는 하나다 — **이 두 엔트리를 지우거나 `upstreamModel: glm-5.3-flash` 로 고칠 것.** 실제로 붙는 엔트리는 `glm-5.3-flash-local`/`-local-low`(`upstreamModel: glm-5.3-flash`)뿐이다, **srv3** = **PaddleOCR-VL(:18011, 2026-09-06 srv1에서 이사)** + 대형 모델 4노드 TP 서빙의 워커(srv4 도 이 서빙의 워커를 겸한다). wormhole 의 `qwen3.6-35b-a3b` 엔트리는 srv1 tailnet(`http://100.105.145.6:8000/v1`) + `fleet: true`(SparkFleet 등록 시 노드 이동 자동 추적)로 남아 있으나 **백엔드는 죽어 있고, 이 엔트리를 이름으로 지정한 호출은 전부 fallback 으로 샌다** — 2026-08 에 그 fallback 이 유료 API 여서 12일간 1,346 회 과금됐다(수리: 사슬 재배선 + `metered` 게이트, model-roles.md 도그마 #7). 게이트웨이는 크로스호스트 사이드카를 env 오버라이드(`DENEB_OCR_VL_URL`/`DENEB_ASR_URL` = srv1 tailnet)로 소비한다. 대부분 vLLM 의 OpenAI 호환 `/v1` 엔드포인트지만, 일부(VibeVoice-ASR)는 전용 서비스로 상주한다. 로컬 추론은 **원칙이 아니라 선호**다(운영자 명시, 2026-08-01): 비용·주권·레이턴시가 비슷하면 플릿 자급이 기본값이지만, 클라우드가 실측으로 유의미하게 이기면 채택한다 — main(glm/kimi)·vision(gemini)이 그 선례. 클라우드 옵션을 "로컬 원칙 위반"으로 기각하지 말 것; 기각 논거는 아키텍처(파이프라인 우회 여부)·비용·품질 실측으로. 이 파일은 "어떤 모델이, 어디서, 어떻게" 돌아가는지의 단일 진실원이다.

> **srv1 사이드카 전면 중단의 결과 (2026-09-06 실측)** — 이 문단은 위 호스트 배치보다 우선한다.
>
> - **OCR = srv3에서 복구됨(2026-09-06).** 한동안 tesseract 폴백으로 내려가 있었다 — 게이트웨이가 복구 시점에 `✅ 사이드카 복구: ocr (다운 7일 17시간)`을 찍었으므로 **실제 공백은 2026-08-29부터 7일 17시간**이었다(운영자가 오늘 내린 게 아니라 그 전부터 죽어 있었다). 이사 상세는 아래 PaddleOCR-VL 절.
> - **ASR = 영향 없음.** primary가 Gemini(`DENEB_ASR_PROVIDER=gemini`, systemd 드롭인)이고 MOSS 사이드카는 *reachable일 때만* 시도하는 폴백이다.
> - **wormhole의 srv1 엔트리(lfm2.5-8b-a1b :8101 · hy-mt2-30b-a3b :8102 · qwen3.6-35b-a3b :8000)는 전부 죽은 포인터**지만, 현행 역할 배치가 아무것도 이들을 가리키지 않아 실사용 경로는 없다(main=k3 · submain/vision/fallback=glm-5.3-flash · tiny=glm-5.3-flash-nothink · lightweight=deepseek-v4-flash-api · coding=glm-5.3). **이름으로 지정해 부르면 2026-08의 유료 폴백 사고(12일 1,346회) 경로가 그대로 다시 열린다.**
...
> ★**정착(settle) 규칙**(2026-09-03 리뷰 반영 최종): `wait_ms` 를 통째로 자던 것을 **"렌더 + 콘텐츠 정지 + 네트워크 정지 +
> (셀렉터가 있으면) 그 셀렉터가 매칭"이 모두 성립하면 조기 종료**로 바꿨다(`settlePage`). 조건 = 본문 400자 이상 ·
> 길이 350ms 무변화 · 인플라이트 0에 400ms 무활동 · 최소 대기 통과.
> ★**`wait_ms` 는 상한이자 바닥이다** — 값을 주면 그만큼은 반드시 기다리고(늦게 그려지는 페이지의 유일한 처방),
> 생략하면 사이드카가 알아서 끝낸다. 실측(골든셋 합계): 생략 3.8초 · 명시 10.0초 · 구 고정수면 10.0초.
> 그래서 도구 스키마는 **생략을 권하고** 필요할 때만 지정하도록 바꿨다.
> ⚠️ **가드는 전부 실측으로 벌었고, 하나라도 빼면 조용히 본문을 잃는다**(프로브가 변이시험으로 증명):
> ① 400자 바닥 없으면 스켈레톤(`...` 3자)이 즉시 안정으로 보여 늦은 본문 유실.
> ② 네트워크 추적 없으면 `networkidle` 이후 시작한 fetch 를 못 본다.
> ③ 최소 대기 없으면 내비만으로 400자를 넘긴 페이지가 기사 fetch 전에 종료된다.
> ④ **명시 wait_ms 바닥이 없으면** 네트워크 없는 늦은 하이드레이션(인라인 blob)을 wait_ms=4000 을 줘도 놓친다.
...
- 런처: **`~/start-paddleocr-vl.sh`** (★**srv3** 호스트 — 2026-09-06 srv1에서 이사, **레포 밖** 로컬 파일). 게이트웨이(srv4)는 `DENEB_OCR_VL_URL=http://10.10.10.3:18011` 로 크로스호스트 소비 — **tailnet이 아니라 200G 패브릭 주소**다(사이드카 경로를 하루에도 여러 번 끊기는 Wi-Fi 업링크에 얹을 이유가 없다). 컨테이너 `paddleocr-vl`, port **18011**, `--restart unless-stopped`.
- **왜 srv3으로 옮겼나 (2026-09-06)**: srv1은 디스크 93%(63GB)에 검증된 eugr 이미지가 이미 지워져 20GB 재다운로드가 필요했고, 사이드카도 전부 내려간 상태였다. srv3은 디스크 1.9TB 여유에 플릿에서 메모리 여유가 가장 컸다. 가중치 1.8GB는 패브릭으로 6초(274MB/s)에 복사된다.
- **srv3도 FP8로 돈다 — eugr 이미지 없이(2026-09-06 실측)**: 로컬 `vllm-node:latest`(vLLM 0.22.1rc1)가 `--quantization fp8`에서 **`CutlassFP8ScaledMMLinearKernel`을 그대로 선택**한다(로그로 확인). eugr 나이틀리 23GB를 옮길 필요가 없었다. 런처에 `PADDLEOCR_FP8`/`PADDLEOCR_SPEC` 토글을 넣어 BF16 폴백도 가능하게 해뒀다.
- **ngram speculative는 srv3에서 기본 OFF**: prompt-lookup 방식인데 OCR 프롬프트는 이미지+`"OCR:"`이라 드래프터가 베낄 토큰이 프롬프트에 없다 — 이득 없이 오버헤드만. 2026-07-07에 측정된 1.6x는 FP8 몫이었다.
- **FP8 vs BF16 실측(같은 이미지·같은 경로, 게이트웨이 Go 클라이언트)**: 워밍 후 **0.91s vs 1.08s**(FP8 6회 반복 전부 0.91s로 편차 없음) · 가중치 **1.45 vs 1.82 GiB** · KV **154,880 vs 113,728 토큰**(동시성 18.9x vs 13.9x) · 총 점유 **~7.3 vs ~7.8 GiB**. 출력은 4줄·129자로 동일(금액·용량 숫자 포함). ⚠️이 이미지는 출력이 짧아 프리필이 지배적이라 16%만 벌었다 — 문서의 1.6x는 디코드가 지배하는 조밀한 페이지 기준이다.
...
- **메모리 예산 — 2026-09-06 실측으로 0.06 하향**: `--gpu-memory-utilization 0.06` + `--max-num-seqs 8` + `--max-model-len 8192` → **총 ~7.3GiB 점유**(FP8 모델 1.45GiB + KV 154,880 토큰 = 8k 요청 기준 동시성 **18.9x**). GLM-5.3 4노드 TP 서빙이 플릿 메모리를 거의 다 쥐고 있어 노드당 여유가 13~21GiB뿐이라 기존 0.10(~12GiB)이 안 들어간다. ⚠️★**`free`의 available은 CUDA가 쓸 수 있는 양이 아니다** — srv4는 available 18GiB인데도 util 0.06(7GiB) 요청이 **CUDA 컨텍스트 생성 단계에서** `cudaErrorMemoryAllocation`으로 죽었다(`_accelerator_getMemoryInfo`). 서빙이 걷히면 아래 구 예산으로 되돌려도 된다.
- 구 예산(2026-07-07, 서빙이 메모리를 안 쓰던 시절): `--gpu-memory-utilization 0.10` + `--max-num-seqs 8` + `--max-model-len 8192`. FP8 로 가중치가 작아져 util 0.10(~12GB)에서 **동시성 58x**(KV ~8GB) — 다페이지 배칭의 근거(구 0.03/seqs4/BF16 은 동시성 3.4x 로 배칭 사실상 불가였음). srv1 여유 ~28GB 내라 안전. ⚠️ **병목은 디코드**(출력토큰 생성)지 프리필 아님 — NaViT 가 vision 토큰을 ~1.3k 로 정규화해 **입력 해상도와 지연 무관**(이미지 축소 무의미, 측정으로 기각). 디코드가 느린 건 GB10 메모리대역폭(~273GB/s)에 0.9B 가 묶여서(토큰당 read) → FP8 이 반감(160→249 tok/s). 런처가 `GPU_MEM_UTIL`·`PADDLEOCR_MAX_NUM_SEQS`·`PADDLEOCR_IMAGE` env override 지원. **롤백**: `start-paddleocr-vl.sh.bak-pre-fp8`.

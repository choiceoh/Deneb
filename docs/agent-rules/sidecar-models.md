...
- **srv3은 BF16으로 돈다**: eugr(FP8+ngram) 이미지가 srv3에 없어 로컬 `vllm-node:latest`(vLLM 0.22.1rc1)로 서빙한다. 그래서 런처에 `PADDLEOCR_FP8=0` / `PADDLEOCR_SPEC=0` 토글을 넣었다(기본값은 기존 FP8+ngram 그대로라 eugr 호스트의 동작은 무변경). 출력은 FP8과 byte-identical이 검증돼 있었고(2026-07-07), 대가는 페이지당 속도뿐이다 — **실측 워밍 후 1.08s/page**(콜드 첫 호출 38s).

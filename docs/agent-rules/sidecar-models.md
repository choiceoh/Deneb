...
| **xprovence-reranker-bgem3-v2** (568M) | 위키 검색 크로스인코더 리랭크 (top-10 재정렬) | `http://127.0.0.1:8004` (`scripts/deploy/rerank-server.py`, user 유닛 `deneb-rerank.service`) | `ai/rerank/client.go` → `domain/wiki/rerank.go` (drop-in `DENEB_RERANK_URL`/`_MODEL`/`_FORCE`) | 2026-07-19 실배선 (#3992): 병합골드 P@1 83.7→87.1(+3.4pp)·p95 272ms. **nemotron-eval venv** 재사용(torch cu130 + transformers **4.51** — xprovence 원격코드가 transformers<5 필수 — + spacy `xx_sent_ud_sm`). fp16 ~1.2GB. `FORCE=on`이 기본 모호성 게이트(6%만 발화)를 대체. 클라이언트 fail-open — 사이드카 다운 시 검색 무변경. Apache 폴백 `--model bge`(bge-reranker-v2-m3; xprovence는 CC BY-NC, 단일사용자 개인배치로 운영자 수용). 롤백 = 게이트웨이 drop-in `rerank.conf` 삭제 + 재기동 |

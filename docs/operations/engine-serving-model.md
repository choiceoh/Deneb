...
- `비전은 그대로 — …`: 새 모델이 이미지를 받지 않아 비전 역할은 옮기지 않았다. 이유는
  셋 중 하나다: 엔진이 이번 부팅에서 비전 타워를 물지 않았다(가장 흔함), 라우터 엔트리가
  `vision: false`로 막혀 있다, 엔진이 이미지 입력 여부를 알리지 않고 엔트리에도 표시가 없다.
...
- 비전 역할은 **엔진이 이번 부팅에서 이미지를 받는다고 보고할 때만** 옮긴다. 보고는 도어의
  `/v1/models` 모델 카드 `capabilities.vision`이다. ST는 이를 실제로 물은 비전 타워에서
  도출하므로 상수가 아니다. 라우터 엔트리의 `vision: false`는 보고보다 우선하는 금지다.
  보고가 없는 엔진(vLLM)이면 엔트리가 `vision: true`일 때만 옮긴다.
- 라우터(wormhole)도 같은 보고로 이미지를 거른다. 60초마다 로컬 백엔드의 `/v1/models`를 읽고,
  보고가 있으면 엔트리의 `vision: true`나 내장 표보다 그 보고를 먼저 따른다. 텍스트 전용
  부팅에 이미지가 가면 400이 나고, 그 이미지가 히스토리에 남아 이후 턴까지 같은 400을
  낸다. 그래서 거르는 쪽이 맞다. 엔진 엔트리에는 `vision` 키를 두지 않는 것이 기본이다.
- Qwen3.8은 stkernel #1274(2026-09-19)부터 그림을 받는다. 단 `--vision auto`라서 **모든
  노드의 랭크 디렉터리에 `vision.safetensors`가 있을 때만** 켜지고, 일부에만 있으면 부팅을
  거부한다. 파일은 `preshard --vision`으로 만들고 `fanout-st-ranks.sh`로 나눈다 <!-- docref:ignore -->
  (`start-st-qwen38.sh` 머리말). 없으면 텍스트 전용으로 뜨고, 도어가 `vision: false`를 보고한다. <!-- docref:ignore -->

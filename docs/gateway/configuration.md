...
read_when:
  - Setting up Deneb for the first time
...
Deneb reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.deneb/deneb.json`.

If the file is missing, Deneb uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `deneb onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    deneb onboard       # full onboarding flow
    deneb configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    deneb config get agents.defaults.workspace
    deneb config set agents.defaults.heartbeat.every "2h"
    deneb config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.deneb/deneb.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Deneb only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`deneb doctor`, `deneb logs`, `deneb health`, `deneb status`)
- Run `deneb doctor` to see exact issues
- Run `deneb doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `deneb.json`.
    ...
    ```bash
    deneb config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
  ...
  <Accordion title="Set up heartbeat (periodic check-ins)">
    ...
    - `target`: `last` | `telegram` | `telegram` | `none`
...
The Gateway watches `~/.deneb/deneb.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `deneb config set` for single keys.
    </Warning>
    ...
    ```bash
    deneb gateway call config.get --params '{}'  # capture payload.hash
    deneb gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.deneb/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    deneb gateway call config.patch --params '{
...
Deneb reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.deneb/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Deneb runs your login shell and imports only the missing keys:
...
Env var equivalent: `DENEB_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${DENEB_GATEWAY_TOKEN}" } },

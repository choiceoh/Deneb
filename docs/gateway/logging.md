...
- Default rolling log file is under `/tmp/deneb/` (one file per day): `deneb-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.deneb/deneb.json`:
  - `logging.file`
  - `logging.level`
...
`deneb gateway` supports a per-gateway style switch:
...
deneb gateway --verbose --ws-log compact

# show all WS traffic (full meta)
deneb gateway --verbose --ws-log full
```

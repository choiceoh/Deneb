...
`deneb doctor` is the repair + migration tool for Deneb. It fixes stale
...
- Extra workspace dir detection (`~/deneb`).
...
you to run `deneb doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.deneb/deneb.json` with the updated schema.
...
Doctor also checks the cron job store (`~/.deneb/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.deneb` folders exist across
  home directories or when `DENEB_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.deneb/deneb.json` is
...
offers to remove them and install the Deneb service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Deneb gateway services are considered first-class and are not
flagged as "extra."
...
- `deneb doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `deneb doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `deneb doctor` prompts before rewriting supervisor config.
- `deneb doctor --yes` accepts the default repair prompts.
- `deneb doctor --repair` applies recommended fixes without prompts.
- `deneb doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `deneb gateway install --force`.

...
read_when:
  - Running Deneb behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Deneb
...
- You run Deneb behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Deneb checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Deneb extracts the user identity from the configured header
...
- Deneb can stay on loopback HTTP behind the proxy.
...
If Deneb itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://deneb.example.com
    to: http://deneb-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`deneb security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Deneb config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `deneb security audit` and review findings

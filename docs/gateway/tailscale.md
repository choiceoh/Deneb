...
Deneb can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Deneb requires a shared password.
...
- `token` (default when `DENEB_GATEWAY_TOKEN` is set)
- `password` (shared secret via `DENEB_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Deneb verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Deneb only treats a request as Serve when it arrives from loopback with
...
Prefer `DENEB_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want Deneb to undo `tailscale serve`

...
description: "Complete field-by-field reference for ~/.deneb/deneb.json"
summary: "Complete reference for every Deneb config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.deneb/deneb.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Deneb uses safe defaults when omitted.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `deneb doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["deneb-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-deneb",
          requireMention: false,
...
- If you add a non-default account via `deneb channels add` (or channel onboarding) while still on a single-account top-level channel config, Deneb moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@deneb", "deneb"] } }],
  },
}
```
...
Optional repository root shown in the system prompt's Runtime line. If unset, Deneb auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/deneb" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Deneb assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Deneb aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/deneb-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "deneb-sandbox-browser:bookworm-slim",
          network: "deneb-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Deneb materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.deneb/sandboxes`
...
In `remote` mode, host-local edits made outside Deneb are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `deneb.json`.
noVNC observer access uses VNC auth by default and Deneb emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `deneb-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from DENEB_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `DENEB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `DENEB_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `DENEB_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.deneb/workspace",
        agentDir: "~/.deneb/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@deneb"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.deneb/workspace-home" },
      { id: "work", workspace: "~/.deneb/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Deneb resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.deneb/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.deneb/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.deneb/workspace-public",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Deneb starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:deneb`      | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.deneb/attachments/<uuid>/` with a `.manifest.json`.
...
Deneb uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.deneb/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `DENEB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Deneb forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `deneb onboard --auth-choice opencode-zen` or `deneb onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `deneb onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `deneb onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `deneb onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `deneb onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `deneb onboard --auth-choice minimax-api`.
...
- Loaded from `~/.deneb/extensions`, `<workspace>/.deneb/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Deneb plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Deneb plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Deneb applies those as sanitized agent settings, not as raw Deneb config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `deneb plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or DENEB_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `DENEB_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `gateway.auth.mode: "trusted-proxy"`: delegate auth to an identity-aware reverse proxy and trust identity headers from `gateway.trustedProxies`.
...
- Optional response hardening header:
  - `gateway.http.securityHeaders.strictTransportSecurity` (set only for HTTPS origins you control)
...
```bash
DENEB_CONFIG_PATH=~/.deneb/a.json \
DENEB_STATE_DIR=~/.deneb-a \
deneb gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.deneb-dev` + port `19001`), `--profile <name>` (uses `~/.deneb-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.deneb/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-deneb-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `DENEB_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.deneb/workspace/canvas",
    liveReload: true,
    // enabled: false, // or DENEB_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__deneb__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__deneb__/a2ui/`
...
- Also serves A2UI at `/__deneb__/a2ui/`.
...
- `.env` files: CWD `.env` + `~/.deneb/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${DENEB_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `deneb.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.deneb/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/deneb-vault-resolver",
...
- Legacy OAuth imports from `~/.deneb/credentials/oauth.json`.
...
- Default log file: `/tmp/deneb/deneb-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Deneb.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `DENEB_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `deneb doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `deneb.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.

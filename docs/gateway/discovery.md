...
- Service types:
  - `_deneb-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `deneb` entrypoint or binary)
...
- `DENEB_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.deneb/deneb.json` controls the Gateway bind mode.
- `DENEB_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `DENEB_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `DENEB_CLI_PATH` overrides the advertised CLI path.

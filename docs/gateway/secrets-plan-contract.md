...
read_when:
  - Generating or reviewing `deneb secrets apply` plans
...
This page defines the strict contract enforced by `deneb secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`deneb secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `deneb.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `deneb secrets configure` or fix the target path to a supported shape above.

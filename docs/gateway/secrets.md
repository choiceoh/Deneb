...
Deneb supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- Outbound delivery paths also read from that active snapshot (for example Telegram reply/thread delivery and Telegram action sends); they do not re-resolve SecretRefs on each send.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `DENEB_GATEWAY_TOKEN` (or the legacy `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Deneb runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Deneb validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Deneb QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/deneb"],
...
- Deneb resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `deneb.json` refs)
...
When reload-time activation fails after a healthy state, Deneb enters degraded secrets state.
...
- Strict command paths (for example `deneb memory` remote-memory paths and `deneb qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `deneb status`, `deneb status --all`, `deneb channels status`, `deneb channels resolve`, `deneb security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `deneb secrets reload`.
...
```bash
deneb secrets audit --check
deneb secrets configure
deneb secrets audit --check
```
...
- plaintext values at rest (`deneb.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `deneb.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `deneb secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `deneb.json` plus `auth-profiles.json` for one agent scope
...
- `deneb secrets configure --providers-only`
- `deneb secrets configure --skip-provider-setup`
- `deneb secrets configure --agent <id>`
...
```bash
deneb secrets apply --from /tmp/deneb-secrets-plan.json
deneb secrets apply --from /tmp/deneb-secrets-plan.json --allow-exec
deneb secrets apply --from /tmp/deneb-secrets-plan.json --dry-run
deneb secrets apply --from /tmp/deneb-secrets-plan.json --dry-run --allow-exec
```
...
Deneb intentionally does not write rollback backups containing historical plaintext secret values.

...
> Deneb is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Deneb security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
deneb security audit --fix
deneb security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Deneb is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Deneb assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.deneb`, including `deneb.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- Deneb can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Deneb, use one VPS/host per user.
...
Deneb is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/deneb/deneb/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Deneb also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Deneb state                                 | filesystem perms on `~/.deneb`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.deneb/deneb.json`                                                   | yes      |
...
`deneb security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`deneb security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Deneb config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${DENEB_GATEWAY_PASSWORD}
```
...
Deneb stores session transcripts on disk under `~/.deneb/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.deneb` (see the audit section below). If you need
...
- If you install plugins from npm (`deneb plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.deneb/extensions/<pluginId>/` (or `$DENEB_STATE_DIR/extensions/<pluginId>/`).
  - Deneb uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
deneb pairing list <channel>
deneb pairing approve <channel> <code>
```
...
By default, Deneb routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
- “Paste the full contents of ~/.deneb or your logs.”

## Unsafe external content bypass flags

Deneb includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.deneb/deneb.json`: `600` (user read/write only)
- `~/.deneb`: `700` (user only)

`deneb doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `DENEB_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__deneb__/canvas/` and `/__deneb__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Deneb with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_deneb-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `DENEB_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `deneb doctor --generate-gateway-token`.
...
paths, set `DENEB_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Deneb
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Deneb verifies the identity by resolving the
...
- Deneb will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- When sharing diagnostics, prefer `deneb status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@deneb", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.deneb`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.deneb/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `deneb` profile).
...
Deneb’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Deneb limits these tools
...
1. Check Gateway logs: `/tmp/deneb/deneb-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.deneb/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `deneb security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Deneb version
...
Found a vulnerability in Deneb? Please report responsibly:

1. Email: [security@deneb.ai](mailto:security@deneb.ai)

---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Deneb agents"
read_when:
...
OpenShell is a managed sandbox backend for Deneb. Instead of running Docker
containers locally, Deneb delegates sandbox lifecycle to the `openshell` CLI,
...
- Deneb Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Deneb creates an OpenShell
...
- You edit files locally outside Deneb and want those changes visible in the
...
- When the sandbox is first created, Deneb seeds the remote workspace from
...
- Deneb does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Deneb after the initial seed,
the remote sandbox does **not** see those changes. Use
`deneb sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"deneb"`  | Sandbox source for first-time create                  |
...
1. Deneb calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Deneb calls `openshell sandbox ssh-config <name>` to get SSH connection

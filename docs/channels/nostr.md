...
read_when:
  - You want Deneb to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Deneb to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`deneb onboard`) and `deneb channels add` list optional channel plugins.
...
```bash
deneb plugins install @deneb/nostr
```

Use a local checkout (dev workflows):

```bash
deneb plugins install --link <path-to-deneb>/extensions/nostr
```
...
deneb channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "deneb@example.com",
        "lud16": "deneb@example.com"
      }
    }
  }
}

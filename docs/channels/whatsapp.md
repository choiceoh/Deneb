...
- Onboarding (`deneb onboard`) and `deneb channels add --channel whatsapp`
  prompt to install the WhatsApp plugin the first time you select it.
- `deneb channels login --channel whatsapp` also offers the install flow when
  the plugin is not present yet.
- Dev channel + git checkout: defaults to the local plugin path.
- Stable/Beta: defaults to the npm package `@deneb/whatsapp`.

Manual install stays available:

```bash
deneb plugins install @deneb/whatsapp
```
...
<Note>
Deneb recommends running WhatsApp on a separate number when possible. (The channel metadata and setup flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Deneb
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Deneb channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[deneb]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.deneb/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.deneb/credentials/` is still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `deneb channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `deneb.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope

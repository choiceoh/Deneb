...
```bash
deneb plugins install @deneb/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
deneb plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Deneb" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `deneb pairing list nextcloud-talk`
  - `deneb pairing approve nextcloud-talk <CODE>`

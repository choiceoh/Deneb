...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Deneb HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `deneb pairing approve slack <code>`.
...
- The interactive callback values are Deneb-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Deneb falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Deneb is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Deneb",
    "description": "Slack connector for Deneb"
  },
  "features": {
    "bot_user": {
      "display_name": "Deneb",
    ...
    "slash_commands": [
      {
        "command": "/deneb",
        "description": "Send a message to Deneb",
        "should_escape": false
      }
    ]
  },
...
Deneb supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Deneb falls back to normal delivery for remaining payloads.

---
summary: "Synology Chat webhook setup and Deneb config"
read_when:
  - Setting up Synology Chat with Deneb
...
```bash
deneb plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `deneb onboard` now shows Synology Chat in the same channel setup list as `deneb channels add`.
   - Non-interactive setup: `deneb channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Deneb gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Deneb.
   - Guided: `deneb onboard`
   - Direct: `deneb channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `deneb pairing list synology-chat`
  - `deneb pairing approve synology-chat <CODE>`
...
```bash
deneb message send --channel synology-chat --target 123456 --text "Hello from Deneb"
deneb message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```

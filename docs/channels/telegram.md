...
```
    ...
    Telegram does **not** use `deneb channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `deneb doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `deneb doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `deneb logs --follow`
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Deneb can stream partial replies in real time:
    ...
    - DM: Deneb keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Deneb keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Deneb falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Deneb skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Deneb automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Deneb retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Deneb pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.deneb/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Deneb enqueues system events like:
...
```bash
deneb message send --channel telegram --target 123456789 --message "hi"
deneb message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `deneb message poll` and support forum topics:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Deneb preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `deneb channels status` warns when config expects unmentioned group messages.
    - `deneb channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `deneb logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Deneb now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `DENEB_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `DENEB_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `DENEB_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- Multi-account precedence:
  ...
  - If neither is set, Deneb falls back to the first normalized account ID and `deneb doctor` warns.

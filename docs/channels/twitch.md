...
read_when:
  - Setting up Twitch chat integration for Deneb
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Deneb connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
deneb plugins install @deneb/twitch
```

Local checkout (when running from a git repo):

```bash
deneb plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `DENEB_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "deneb", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use DENEB_TWITCH_ACCESS_TOKEN env var)
...
```bash
DENEB_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

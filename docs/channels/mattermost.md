---
summary: "Mattermost bot setup and Deneb config"
read_when:
...
```bash
deneb plugins install @deneb/mattermost
```

Local checkout (when running from a git repo):

```bash
deneb plugins install ./extensions/mattermost
```
...
4. Configure Deneb and start the gateway.
...
Native slash commands are opt-in. When enabled, Deneb registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Deneb derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Deneb.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Deneb.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Deneb, not `404`.
...
- Approve via:
  - `deneb pairing list mattermost`
  - `deneb pairing approve mattermost <CODE>`
...
Use these target formats with `deneb message send` or cron/webhooks:
...
Deneb resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Deneb sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Deneb sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Deneb derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Deneb run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="deneb-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"deneb-mattermost-interactions",
...
`deneb message send` and cron/webhook deliveries.

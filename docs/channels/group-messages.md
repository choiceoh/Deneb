...
Goal: let Deneb sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
Add a `groupChat` block to `~/.deneb/deneb.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?deneb", "\\+?15555550123"],
        },
      },
    ],
  },
...
1. Add your WhatsApp account (the one running Deneb) to the group.
2. Say `@deneb …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.deneb/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.

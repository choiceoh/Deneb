...
Tlon is a decentralized messenger built on Urbit. Deneb connects to your Urbit ship and can
...
```bash
deneb plugins install @deneb/tlon
```

Local checkout (when running from a git repo):

```bash
deneb plugins install ./extensions/tlon
```
...
By default, Deneb blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `deneb message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Deneb replies in-thread.

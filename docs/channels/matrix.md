...
Matrix is the Matrix channel plugin for Deneb.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core Deneb.
...
```bash
deneb plugins install ./extensions/matrix
```
...
```bash
deneb channels add
deneb configure --section channels
```
...
- To resolve room names before saving them, use `deneb channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "Deneb Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.deneb/credentials/matrix/`.
...
```bash
deneb matrix verify status --include-recovery-key --json
```
...
```bash
deneb matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
deneb matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
deneb matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
deneb matrix verify backup restore
```

Verbose restore diagnostics:

```bash
deneb matrix verify backup restore --verbose
```
...
```bash
deneb matrix verify status --account assistant
deneb matrix verify backup restore --account assistant
deneb matrix devices list --account assistant
```
...
Deneb treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `deneb matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for Deneb to treat the device as fully verified.

### What bootstrap does

`deneb matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, Deneb tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `deneb matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, Deneb can attempt a stricter repair path.
If the current device is already owner-signed, Deneb preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- Deneb automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, Deneb creates or reuses a recovery snapshot under `~/Backups/deneb-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so Deneb knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `deneb doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, Deneb now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, Deneb will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.deneb/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, Deneb reuses the best existing
...
Deneb currently provides that in Node by:
...
Under Deneb's security model, the gateway host and local Deneb state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from Deneb secrets providers instead of only local files
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by Deneb.
When SAS emoji verification becomes available, Deneb starts that SAS flow automatically for inbound requests and confirms its own side.
...
Deneb does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old Deneb-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
deneb matrix devices list
```

Remove stale Deneb-managed devices with:

```bash
deneb matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, Deneb can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
deneb matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
deneb matrix direct repair --user-id @alice:example.org
```
...
Ack reactions use the standard Deneb resolution order:
...
```bash
deneb pairing list matrix
deneb pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Deneb reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Set `defaultAccount` when you want Deneb to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `deneb matrix verify ...` and `deneb matrix devices ...` when you want to override that implicit selection for one command.

## Target resolution

Matrix accepts these target forms anywhere Deneb asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; Deneb does not trust alias state claimed by the invited room.

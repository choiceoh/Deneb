...
read_when:
  - Setting up Zalo Personal for Deneb
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Deneb.
...
- Install via CLI: `deneb plugins install @deneb/zalouser`
- Or from a source checkout: `deneb plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `deneb channels login --channel zalouser`
...
- `deneb pairing list zalouser`
- `deneb pairing approve zalouser <code>`
...
- On startup, Deneb resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Deneb stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Deneb state. Example:
...
- Deneb sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Deneb sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `deneb channels logout --channel zalouser && deneb channels login --channel zalouser`
...
- The channel now runs fully in Deneb without external CLI binaries.

...
```bash
deneb plugins install @deneb/msteams
```

Local checkout (when running from a git repo):

```bash
deneb plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Deneb will offer the local install path automatically.
...
3. Configure Deneb with those credentials.
...
- Talk to Deneb via Teams DMs, group chats, or channels.
...
- On startup, Deneb resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.deneb/deneb.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Deneb, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                 |
   | ------------------ | ----------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `deneb-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `deneb plugins install @deneb/msteams`
   - From a local checkout: `deneb plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Deneb in Teams", "full": "Deneb in Teams" },
...
Deneb handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Deneb only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/DenebShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Deneb sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `deneb message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.deneb/msteams-polls.json`.
...
```bash
deneb message send --channel msteams \
...
deneb message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
deneb message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
deneb message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
deneb message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```

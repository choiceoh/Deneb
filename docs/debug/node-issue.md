...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Deneb
...
Running Deneb via Node with `tsx` fails at startup with:

```
[deneb] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `deneb status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch deneb.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node deneb.mjs status` works on Node 25.

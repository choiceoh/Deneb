...
Deneb supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Deneb also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `deneb configure --section web`. It stores the key in
`~/.deneb/deneb.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.deneb/.env` (or your service environment). See [Env vars](/help/faq#how-does-deneb-load-environment-variables).

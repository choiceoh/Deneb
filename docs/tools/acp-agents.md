...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Deneb run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Deneb in plain language to "run this in Codex" or "start Claude Code in a thread", Deneb should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Deneb-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Deneb native sub-agent runtime  |
...
- Deneb binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Deneb returns a clear unsupported/unavailable message.
...
- Deneb ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Deneb uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Deneb may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Deneb session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Deneb sandbox.
...
If no target resolves, Deneb returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Deneb returns a clear unsupported-control error.
...
When Deneb uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Deneb `agentId` path).
...
- Relative paths resolve from Deneb workspace directory.
...
- Deneb startup remains non-blocking while the backend health check runs.
...
```bash
deneb config set plugins.entries.acpx.config.permissionMode approve-all
deneb config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Deneb currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>

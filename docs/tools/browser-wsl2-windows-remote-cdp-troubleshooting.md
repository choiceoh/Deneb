...
read_when:
  - Running Deneb Gateway in WSL2 while Chrome lives on Windows
...
- Deneb Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/deneb/deneb/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Deneb and Chrome are on the same machine
...
- Deneb points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Deneb is not the problem yet.
...
Fix that before touching Deneb config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Deneb at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Deneb to succeed
...
```bash
deneb browser open https://example.com --browser-profile remote
deneb browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `deneb browser tabs` returns the target
...
3. Deneb config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Deneb config or Control UI auth

---
summary: "Agent tool surface for Deneb (browser, canvas, nodes, message, cron) replacing legacy `deneb-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `deneb-*` skills
title: "Tools"
---
...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `deneb.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Deneb logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:deneb`: all built-in Deneb tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`deneb node run`).
...
Deneb tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
Control the dedicated Deneb-managed browser.
...
- Omit `profile` for the safe default: isolated Deneb-managed browser (`deneb`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `deneb`).
...
- Quick smoke: `deneb nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when Deneb can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, Deneb infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `deneb gateway` restart in-place)
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.deneb/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Deneb clamps `tools.sessions.visibility` to `tree`.

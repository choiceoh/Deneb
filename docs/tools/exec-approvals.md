...
- **gateway host** → `deneb` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Deneb also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.deneb/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.deneb/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `deneb approvals allowlist ...`).
- `deneb security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `deneb doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
topics, Deneb preserves the topic for the approval prompt and the post-approval follow-up.

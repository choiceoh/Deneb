---
title: Lobster
summary: "Typed workflow runtime for Deneb with resumable approval gates."
description: Typed workflow runtime for Deneb — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Deneb run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Deneb runs one Lobster tool call and gets a structured result.
...
Deneb launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Deneb tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Deneb Gateway (see the [Lobster repo](https://github.com/deneb/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Deneb keeps core tools enabled. To restrict core
...
→ deneb calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ deneb calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Deneb tools that do.

...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `deneb sandbox explain`.
...
Legacy `agent.*` configs are migrated by `deneb doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=deneb-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${DENEB_STATE_DIR:-$HOME/.deneb}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

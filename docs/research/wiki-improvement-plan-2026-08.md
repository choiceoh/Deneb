...
| nightly recall-health | **도입(07-19) 이후 0회 실행** | srv4 러너 PATH에 go 없음 + `| tee`가 exit 마스킹 → 45회 전부 12초 만에 녹색 |
...
| dream 커밋의 `-id:` 헝크 | 21/30d | 0 | `git -C ~/.deneb/wiki log -p --since … | grep '^-id:'` |

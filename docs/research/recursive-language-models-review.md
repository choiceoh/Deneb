...
- **바운드는 셋 다 필요했다**: 청크 수(4)만으로는 부족해서, 위임 단계 **전체**에 phase 예산(150초)을 걸고, 위임 읽기를 한 번에 하나로 **직렬화**하고(`read_spillover` 가 parallel-safe 라 한 턴에 여러 개가 동시에 뜬다), **질문 길이**도 상한(1500B)을 뒀다 — 질문은 4개 프롬프트 전부와 응답 헤더에 그대로 실려서, 상한이 없으면 프롬프트 172KB·응답 160KB 가 나온다(실측). 마지막 것은 24K 도구 출력 상한을 넘겨 답변 대신 **새 스필 핸들**을 돌려주는 결과가 된다.
- **인용 검증은 두 단계 모두 전수 검사**: 맵·리듀스 답변 모두 인용이 최소 하나 있고 **모든 인용이 실제로 읽은 범위 안**이어야 채택한다. "범위 안 인용이 하나라도 있으면 통과"로 두면 `[L2] … [L999999]` 처럼 진짜와 가짜를 섞어 통과시킬 수 있고, 루트가 위임 답변에 대해 가진 유일한 검증 수단이 "인용된 줄을 직접 열어보기"라 **범위 밖 인용은 인용이 없는 것보다 나쁘다**.
...
- **착지** (3곳):
  ...
  디스크는 여전히 바운드된다: 세션이 끝나면 훅이 걷고, 이벤트 없이 사라진 세션은 TTL sweep 이 걷는다. 수명을 늘린 대가로 **세션별 상한**(48개/512MiB, 오래된 것부터 축출)을 함께 넣었다 — 대화 세션 행은 축출되지 않으므로 "살아있음"이 무제한 리스가 되기 때문이다. 단일 결과가 상한보다 크면 파일 자체를 클램프한다(거부하면 마커에 넣을 핸들이 없어져 잘린 가운데가 복구 불가가 된다).
- **미해소 (소유자 결정 대기)**: 게이트웨이 재시작은 인덱스가 인메모리라 핸들을 무효화한다 — 스필 파일은 남지만 `NewSpilloverStore` 가 빈 인덱스로 시작한다. 인덱스 영속화는 "스필 파일 수명을 프로세스 수명에서 떼어낸다"는 결정이라 ADR 감이고, 별도 PR 로 남겼다. 그때까지의 정확한 보장: **세션 수명 동안 유지, 단 재시작은 핸들 무효화**(not-found 는 도구 재실행을 안내하므로 조용한 오답은 아니다).
...
- **착지**: `spilledOutline` 이 **저장본 전체**(`persistedForm` = 리댁션 → 세션 상한 클램프)를 훑어 섹션 마커(마크다운 헤딩·`=== x ===` 배너)를 **저장본 기준 절대 줄번호**와 함께 최대 10개 나열한다(`outlineOf`). 그 번호가 곧 `read_spillover(offset=N)` 가 여는 위치다. spill 이 없으면(offset 대상이 없음) 개요를 내지 않고, 마커가 2개 미만이면 지도가 아니라 잡음이라 생략한다. 포인터 문자열 형태 불변은 테스트로 고정했다.
- **좌표계가 핵심**: 번호는 **원문이 아니라 저장본** 기준이다. 리댁션은 줄 수를 보존하지 않고(PEM 블록 30줄 → 토큰 1줄) 클램프는 꼬리를 잘라내므로, 원문 좌표로 센 번호는 헤딩이 없는 줄을 가리킨다(실측 20줄 어긋남). 같은 이유로 "프리뷰가 감추는 구간만" 훑지 않는다 — 프리뷰 head/tail 은 원문 좌표라 교집합을 내려는 순간 리댁션이 경계 너머로 옮긴 헤딩이 개요에도 프리뷰에도 없게 된다. 대신 전체를 훑고 **숨겨진 구간의 헤딩을 슬롯 우선 배정**한다.
- **리댁션이 먼저, 자르기는 나중**: 벤더 패턴이 길이에 앵커돼 있어(AKIA + 정확히 16자) 70바이트 캡으로 먼저 자르면 마스커가 매치하지 못하고 접두사가 그대로 프로바이더에 실린다.
- **초판 오류**: 이 항목은 처음에 `FormatPreview` 를 대상으로 적었는데, 그 함수는 **런타임 호출자가 없다**. 거기 손대면 제품 효과가 0이다. 착지 후에도 초판 구현(`middleOutline`, 생략 구간만·원문 좌표)을 그대로 서술해 두었다가 리뷰에서 지적받아 위와 같이 교정했다 — 옛 계약을 남겨두면 나중 변경이나 테스트가 오프셋·시크릿 결함을 되살릴 수 있다.
...
- **라이브 검증 부재**: 게이트웨이 동작 변경인데 `scripts/dev/live-test.sh restart && smoke` 를 못 돌렸다(이 작업 환경에 LLM 백엔드가 없다). DGX 호스트에서 별도로 확인해야 한다.

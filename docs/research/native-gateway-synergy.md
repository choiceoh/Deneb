...
> **후일담 (obsolete):** 두 표면 모두 이후 제거되었다 — `miniapp.topics.*`는 토픽-세션 폐지(PR 1963)로, `miniapp.topicdocs.*`와 설정의 토픽문서 탭은 단일 토픽 현실에서 과잉 UI라 제거. 지식 파일(`workspace/topics/*.md`) 주입 자체는 유지되며, 편집은 채팅으로 에이전트에게 요청한다(시스템 프롬프트가 원본 파일 경로를 자기서술).

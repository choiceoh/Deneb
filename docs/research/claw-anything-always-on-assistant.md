...
**A1. Post-action verification 루프 — P1 / M. 🚧 1차 착수됨 (이 PR).**
> **구현 현황:** buried-failure 탐지가 1차 착수됨. 여러 mutation 도구가 실패를 **인밴드**로
> 반환한다(예: gmail send 가 `"발송 실패: …", nil` — error slot 이 nil 이라 agent loop 가
> isError=false 로 처리 → 성공으로 오인 위험). `gateway-go/internal/pipeline/chat/tool_mutation_verify.go`
> 의 순수 테이블 기반 탐지기(`mutationOutcomeIsFailure`) + `MutationFailureAnnotator`
> PostProcessor 가 gmail·wiki·cron·gateway 의 인밴드 실패에 표준 배너(⚠️ 실행 실패 — 성공처럼
> 보고 금지, 재시도/사용자 통지)를 앞에 붙여 LLM 이 놓치지 못하게 한다(멱등, 성공엔 무동작).
> exhaustive 단위 테스트 + 레지스트리 e2e. **broadcast 에스컬레이션 추가됨:** `run_hooks.go`
> 의 `OnToolResult` 훅이 배너 달린 결과(`isMutationFailureResult`)를 잡아 Error 로그 +
> `chat.tool_failed` broadcast 로 운영자/UI 에 surface(logging.md 규칙 3). **남은 것:** 외부
...
**A2. Turn-level "execution budget" 가드 — P2 / S. ✅ 이미 구현됨.**
> 조사 결과 이 능력은 이미 존재한다: `gateway-go/internal/agentsys/agent/tool_loop.go`
> (`ToolLoopDetector.RecordAndCheck`) 가 generic_repeat·poll_no_progress·ping_pong·
> circuit_breaker 4종 탐지기 + Warning/Critical 임계값으로 동일 인자 반복·무진전 폴링·
> 핑퐁 패턴을 차단한다(`executor.go` 에서 매 도구 호출 전 검사, critical 시 block). 논문이
> 지목한 Opus "루프 갇힘"은 구조적으로 이미 막혀 있다. 추가 작업 불필요.
...
**C1. Pinned / anchor facts — P2 / M. 🚧 1차 착수됨 (이 PR).**
> **구현 현황:** 사용자 `/pin` 채널 착수. `gateway-go/internal/pipeline/chat/pinned_facts.go`
> (per-session 인메모리 스토어 + 순수 포맷터, recall_cache.go 패턴) + 슬래시 `/pin`·`/unpin`·
> `/pins`(한글 별칭 `/고정`·`/고정해제`·`/고정목록`, `slash_commands.go`+`slash_dispatch.go`,
> `/reset` 시 clear) + system prompt **Dynamic 블록**(캐시 마커 없음)에 "## 고정 사실" 주입
> (`system_prompt.go`, `run_exec.go` 가 매 턴 주입). 압축에 안 밀리고 매 턴 재assert → Fig 5a
> "길수록 하락" 완화. exhaustive 단위 테스트. **남은 것:** dreamer 자동 앵커 추출→pin 통합
> (2.3 semantic-anchor), pin 영속화(현재 세션 메모리, 재시작 시 소멸).
...
| 2026-06-07 | Claude | A1 1차(mutation 인밴드 실패 탐지 + 배너 PostProcessor: gmail·wiki·cron·gateway) 착수 |
| 2026-06-07 | Claude | C1 1차(/pin·/unpin·/pins → Dynamic 블록 고정 사실) 착수; A2(loop 가드)는 tool_loop.go 에 이미 구현됨 확인 |
| 2026-06-07 | Claude | B1 signals-only 트리거 + A1 broadcast 에스컬레이션(chat.tool_failed) 추가 |
| 2026-06-07 | Claude | 코드리뷰 1건 수정(heartbeat 활성-사용자 시 캘린더 fetch 회피); `/help`+슬래시 메타데이터 추가(/pin 발견성 — C1 사용성 보강) |

...
> exhaustive 단위 테스트 + 레지스트리 e2e. **notify 에스컬레이션 추가됨:** `run_hooks.go`
> 의 `OnToolResult` 훅이 배너 달린 결과(`isMutationFailureResult`)를 잡아 Error 로그 +
> `chat.tool_failed` broadcast + native notify relay 로 운영자/UI 에 surface(logging.md 규칙 3). **남은 것:** 외부
...
| 2026-06-07 | Claude | B1 signals-only 트리거 + A1 notify 에스컬레이션(chat.tool_failed + native relay) 추가 |

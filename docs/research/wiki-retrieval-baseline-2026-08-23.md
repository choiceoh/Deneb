...
## 후속 실행 (2026-08-23, 같은 날 오후)

위 권고 4항을 같은 날 실행했다. 요약: **①②④ 착지, ③은 구현→측정→기각.**

### ① emit-gold 후보 36건 반영 — 착지

37건 중 `프로젝트/프로젝트-템플릿`(템플릿 페이지, 실사업 아님) 1건을 제외한
36건을 검토 후 srv4 main 골드에 추가(207행; 원본 백업
`.bak-20260823-candidates`). 재측정 main(197케이스): P@1 91.9% · r@8 97.5% ·
MRR 0.942 — 신규 후보 포함에도 유지(커버리지 56→72 프로젝트, 회상은 신규
프로젝트도 찾는다).

### ② 멀티턴 컨텍스트 재작성 — 착지 (PR #4604)

`recall_context_rewrite.go`: 지시사 게이트(그거/거기/그 현장/아까… + 신호어
≤4개)가 켜지면 직전 사용자 턴(트랜스크립트 테일, 타임스탬프 프리픽스 제거·
현재 메시지 스킵)을 앞에 결합해 기존 질의 조립·앵커·랭킹이 자기완결 텍스트를
보게 한다. LLM 없음(1.5s 결정론 예산 유지). "직전 턴 있으면 곡성 페이지 회수,
없으면 회수 불가" 대응 단위테스트로 메커니즘 고정. 발견 3의 a1sim 상한선
(100%)이 이 경로의 도달 가능 목표다.

### ③ weak-primary RRF 확장 — 구현·측정·기각 (재추적 금지 목록에 추가)

앵커 게이트(상위 3결과의 경로·스니펫에 질의 토큰 없으면 발동, 확장 질의를
RRF 병합, 프라이머리 가중치 0.7)를 구현해 6개 골드에서 A/B(glm-5.2):

| 골드 | control P@1/r@8 | weak-primary P@1/r@8 |
|---|---|---|
| main (197) | 91.9 / 97.5 | 91.9 / 97.5 (동일) |
| analysis-xl (450) | 69.3 / 89.3 | 69.3 / 89.1 |
| facet (91) | 90.1 / 96.7 | 90.1 / 96.7 (동일) |
| typed (78) | 92.3 / 97.4 | 92.3 / 97.4 (동일) |
| hard-repointed (27) | 74.1 / 92.6 | 74.1 / 92.6 (동일) |
| **bm25 (24)** | 79.2 / 91.7 | **75.0 / 87.5 (−4.2pp)** |

판정: **기각, 코드 회수.** ① 5개 세트에서 자릿수까지 동일 — 발동이 거의
없다(실트래픽 질의는 어휘가 풍부해 상위가 앵커됨). 순수 어휘 간극(BM25 공백)은
현재 트래픽에서 드물다. ② bm25 셋 회귀의 원인: 게이트가 상위 3위만 검사해
**랭크 4+에 박힌 정확매치를 보호하지 못함** — 프라이머리 0.7 가중치 때문에
정확매치가 확장 노이즈 아래로 강등(7월 21일 실패 모드가 게이트를 새어 나온
것). 결론: 7월의 "+3–5pp" 레버는 비게이트 RRF 형태의 것이고, 배포 가능한
게이트 형태는 레버가 되지 못한다. 남은 회상 레버는 컨텍스트(②)와 골드
위생뿐이다.

### ④ pl2-kia-epc-002 대표 페이지 — 복구 완료

프로덕션 위키에 `프로젝트/pl2-kia-epc-002/대표.md` 생성(SSH 파일 작성 후
`miniapp.memory.write_page` 왕복으로 라이브 스토어 인덱스에 정규화 등록).
프론트매터: client 기아 · sites 광주 광산구 · kinds 태양광/루프탑 · stage 견적
(전부 형제 페이지·상세 입찰 페이지에서 출처 확인). 프로덕션에서
"기아 광주 캐노피" 검색 1위 히트 확인. 레이아웃 불변식(프로젝트=대표
필수) 복구.

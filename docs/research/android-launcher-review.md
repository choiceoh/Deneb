...
일반 런처는 온디바이스 dumb 클라이언트(아이콘 격자 + 멍청한 위젯). Deneb 런처는 **강한 게이트웨이(로컬 LLM·~50 내장 도구·OCR/ASR/임베딩·위키 지식·자율 에이전트·DGX GPU)의 thin 클라이언트** → 홈이 "아이콘+위젯"에서 **비서실장 라이브 콕핏**으로.
...
1. **홈 검색창 = 풀 에이전트** (앱검색 아님 — 위키·메일·캘린더·내장 도구로 답/실행)

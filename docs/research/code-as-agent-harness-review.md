...
1. **exec의 in-place 파일 편집 체크포인트** — `sed -i`·`>` 리다이렉트로 파일을 고치는 exec 명령이 fs Write/Edit 도구의 롤백 안전망을 우회하던 것을, 실행 전 대상 파일(존재하는 정규파일)을 기존 `pkg/checkpoint`로 스냅샷해 `/rollback` 복구 가능하게. 추출은 과포함+존재성 필터라 안전. 논문의 *복구가능성*만 도입, 임의 bash에 fragile한 *자동 롤백-on-failure*는 의도적 제외. → `tools/runtimeops/exec_safety.go:InPlaceFileTargets`, `tools/runtimeops/exec.go`.
...
| 아이디어 | 논문 | 스킵 이유 |
|---|---|---|
| **CodeAct** (코드=액션) | [2402.01030](https://arxiv.org/abs/2402.01030) | +20pt는 *어려운 멀티툴 조합*에서만; **원자 단일툴은 무승부**(API-Bank). Deneb 비서실장 워크로드는 대부분 원자적(메일·캘린더). 비용 큼: 임의 코드 실행(cmdsafety와 역행), 프롬프트 캐시 파괴(~45 스키마 카탈로그), 강한 모델 의존성(오픈 13% vs GPT-4 74% — 로컬 dsv4 위험). 조합 필요 시 이미 exec/code 도구 있음. 필드 합의도 *하이브리드*(구조화 JSON 래퍼) |
...
- **MemGPT 능동 회상** — 에이전트가 턴 중 `wiki`/`polaris(recall)` 도구를 호출(MemGPT의 active retrieval; diary는 wiki `log`/`daily` 액션). 유일 미보유 = *사전 압박 신호*(현재는 사후 `CompactionFired`만)인데, 압박 받는 긴 턴(크론 합성·딥리서치)은 **이미 설계상 결론을 wiki/diary에 영구 기록**하므로 실효 얇음.

...
| 아이디어 | 논문 | 스킵 이유 |
|---|---|---|
| **CodeAct** (코드=액션) | [2402.01030](https://arxiv.org/abs/2402.01030) | +20pt는 *어려운 멀티툴 조합*에서만; **원자 단일툴은 무승부**(API-Bank). Deneb 비서실장 워크로드는 대부분 원자적(메일·캘린더). 비용 큼: 임의 코드 실행(cmdsafety와 역행), 프롬프트 캐시 파괴(~50 스키마 카탈로그), 강한 모델 의존성(오픈 13% vs GPT-4 74% — 로컬 dsv4 위험). 조합 필요 시 이미 exec/code 도구 있음. 필드 합의도 *하이브리드*(구조화 JSON 래퍼) |

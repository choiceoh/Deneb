...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/deneb`)
...
If the page fails to load, start the Gateway first: `deneb gateway`.
...
- Channels: Telegram/Discord status + per-channel config (`channels.status`, `config.patch`)
...
- Config: view/edit `~/.deneb/deneb.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop deneb`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Deneb
...
Deneb **blocks** Control UI connections without device identity.

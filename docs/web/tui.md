...
```bash
deneb tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `deneb tui --deliver`
...
- Local shell commands receive `DENEB_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `DENEB_THEME=light` before launching `deneb tui`.
- To force the original dark palette instead, set `DENEB_THEME=dark`.
...
- Check the Gateway logs: `deneb logs --follow`.
- Confirm the agent can run: `deneb status` and `deneb models status`.
...
- No agents in picker: check `deneb agents list` and your routing config.

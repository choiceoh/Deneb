...
- Jobs persist under `~/.deneb/cron/` so restarts don’t lose schedules.
...
- For upgrades, `deneb doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
deneb cron list
deneb cron run <job-id>
deneb cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.deneb/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `deneb cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Deneb defaults to `announce`.
...
- CLI equivalents: `deneb cron add --light-context ...` and `deneb cron edit --light-context`.
...
- `delivery.channel`: `telegram` / `telegram` / `last`.
...
- Telegram targets should use explicit prefixes (e.g. `channel:<id>`, `user:<id>`) to avoid ambiguity.
...
- Job store: `~/.deneb/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.deneb/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Deneb classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Deneb archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `deneb cron runs` and adjust retention before logs become large
...
deneb cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
deneb cron edit <jobId> --agent ops
deneb cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
deneb cron run <jobId>
deneb cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `deneb cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
deneb cron edit <jobId> \
...
```bash
deneb system event --mode now --text "Next heartbeat: check battery."
```
...
- Check cron is enabled: `cron.enabled` and `DENEB_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Deneb applies exponential retry backoff for recurring jobs after consecutive errors:

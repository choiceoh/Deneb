...
```bash
deneb hooks enable session-memory
```
...
```bash
deneb hooks info session-memory
```

### Onboarding

During onboarding (`deneb onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.deneb/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<deneb>/dist/hooks/bundled/` (shipped with Deneb)
...
Hook packs are standard npm packages that export one or more hooks via `deneb.hooks` in
`package.json`. Install them with:

```bash
deneb hooks install <path-or-spec>
```
...
those to a prerelease, Deneb stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.deneb/hooks/<id>`.
Each `deneb.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `deneb hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.deneb.ai/automation/hooks#my-hook
metadata:
  { "deneb": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.deneb` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Deneb persists them.
...
- **Managed hooks** (`~/.deneb/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.deneb/hooks/my-hook
cd ~/.deneb/hooks/my-hook
```
...
metadata: { "deneb": { "emoji": "🎯", "events": ["command:new"] } }
---
...
deneb hooks list --eligible

# Verbose output (show missing requirements)
deneb hooks list --verbose

# JSON output
deneb hooks list --json
```
...
# JSON output
deneb hooks info session-memory --json
```
...
# Disable a hook
deneb hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.deneb/workspace`)
...
```bash
deneb hooks enable bootstrap-extra-files
```
...
**Output**: `~/.deneb/logs/commands.log`
...
tail -n 20 ~/.deneb/logs/commands.log

# Pretty-print with jq
cat ~/.deneb/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.deneb/logs/commands.log | jq .
```

**Enable**:

```bash
deneb hooks enable command-logger
```
...
```yaml
metadata: { "deneb": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "deneb": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.deneb/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.deneb/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.deneb/hooks/my-hook/handler.ts
   ```

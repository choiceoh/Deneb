...
read_when:
  ...
  - Wiring external systems into Deneb
title: "Webhooks"
---
...
- `channel` optional (string): The messaging channel for delivery. One of: `last`, `telegram`, `telegram`. Defaults to `last`.
- `to` optional (string): The recipient identifier for the channel (e.g., chat ID for Telegram, channel ID for Telegram). Defaults to the last recipient in the main session.
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Deneb config directory (typically `~/.deneb/hooks/transforms`).
...
- `deneb webhooks gmail setup` writes `hooks.gmail` config for `deneb webhooks gmail run`.

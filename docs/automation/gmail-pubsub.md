---
summary: "Gmail Pub/Sub push wired into Deneb webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to Deneb
...
Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> Deneb webhook.
...
uses the last delivery route (falls back to Telegram).
...
under `~/.deneb/hooks/transforms` (see [Webhooks](/automation/webhook)).

## Wizard (recommended)

Use the Deneb helper to wire everything together (installs deps on macOS via brew):

```bash
deneb webhooks gmail setup \
...
- Writes `hooks.gmail` config for `deneb webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, Deneb automatically sets
...
- Set `DENEB_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
- `--hook-url` points to Deneb `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to Deneb.

Recommended: `deneb webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account deneb@gmail.com
gog gmail history --account deneb@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account deneb@gmail.com

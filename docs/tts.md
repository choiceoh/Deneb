...
Deneb can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Deneb can send audio; Telegram gets a round voice-note bubble.
...
Deneb defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `deneb.json`.
...
- If `provider` is **unset**, Deneb prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.deneb/settings/tts.json`, override with `DENEB_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Deneb retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Deneb registers

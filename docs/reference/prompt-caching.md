...
- With Anthropic API-key auth profiles, Deneb seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Deneb injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Deneb exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.deneb/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$DENEB_STATE_DIR/logs/cache-trace.jsonl`
...
- `DENEB_CACHE_TRACE=1` enables cache tracing.
- `DENEB_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `DENEB_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `DENEB_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `DENEB_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

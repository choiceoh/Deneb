---
summary: "How Deneb builds prompt context and reports token usage + costs"
read_when:
...
Deneb tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Deneb assembles its own system prompt on every run. It includes:
...
For images, Deneb downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Deneb session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `deneb status --usage` and `deneb channels list` show
...
`cacheWrite`. If pricing is missing, Deneb shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Deneb can
...
Anthropic's 1M context window is currently beta-gated. Deneb can inject the
...
Deneb skips the `context-1m-*` beta header because Anthropic currently

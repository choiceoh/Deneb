...
`deneb acp` exposes an ACP agent over stdio and forwards prompts to a running
Deneb Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Deneb Gateway
...
- ACP client spawns `deneb acp` and speaks ACP messages over stdio.
...
`deneb acp` resolves the Gateway URL and auth from CLI flags or config:

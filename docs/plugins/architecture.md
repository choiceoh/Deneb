...
read_when:
  - Building or debugging native Deneb plugins
...
This page covers the internal architecture of the Deneb plugin system. For
...
Deneb classifies every loaded plugin into a shape based on its actual
...
When you run `deneb doctor` or `deneb plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `deneb status --all` and `deneb plugins doctor`.

## Architecture overview

Deneb's plugin system has four layers:

1. **Manifest + discovery**
   Deneb finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `deneb.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Deneb plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Deneb reads the registry to expose tools, channels, provider
...
That split lets Deneb validate config, explain missing/disabled plugins, and
...
normal chat actions. Deneb keeps one shared `message` tool in core, and
...
Deneb treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Deneb-facing
  surfaces
...
So if Deneb adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Deneb has shared
...
} from "deneb/plugin-sdk";

const plugin: DenebPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Deneb already treats image/audio/video understanding as one shared
...
If Deneb adds a new domain later, such as video generation, use the same
...
`DenebPluginApi`. That contract defines the supported registration points and
...
- ad hoc runtime objects that are not part of `DenebPluginApi` or
  `api.runtime`
...
Native Deneb plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Deneb currently treats them
...
Deneb exports capabilities, not implementation convenience.
...
At startup, Deneb does roughly this:
...
The manifest is the control-plane source of truth. Deneb uses it to:
...
Deneb keeps short in-process caches for:
...
- Set `DENEB_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `DENEB_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `DENEB_PLUGIN_DISCOVERY_CACHE_MS` and
  `DENEB_PLUGIN_MANIFEST_CACHE_MS`.
...
Deneb still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Deneb calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Deneb tries the normal registry/catalog path first                                       | _(not a plugin hook)_                                                                |
...
that still runs on Deneb's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Deneb's static catalog updates; it also uses
...
- If Deneb adds a new capability such as video generation later, define the
...
- Deneb only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Deneb plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `deneb/plugin-sdk` import when
authoring plugins:

- `deneb/plugin-sdk/plugin-entry` for plugin registration primitives.
- `deneb/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `deneb/plugin-sdk/channel-setup`,
  `deneb/plugin-sdk/channel-pairing`,
  `deneb/plugin-sdk/channel-reply-pipeline`,
  `deneb/plugin-sdk/secret-input`, and
  `deneb/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  wiring.
- Domain subpaths such as `deneb/plugin-sdk/channel-config-helpers`,
  `deneb/plugin-sdk/channel-config-schema`,
  `deneb/plugin-sdk/channel-policy`,
  `deneb/plugin-sdk/channel-runtime`,
  `deneb/plugin-sdk/config-runtime`,
  `deneb/plugin-sdk/agent-runtime`,
  `deneb/plugin-sdk/lazy-runtime`,
  `deneb/plugin-sdk/reply-history`,
...
- Avoid the root `deneb/plugin-sdk` barrel for new code.
...
`deneb/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Deneb writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Deneb's
...
- if both `catalog` and `discovery` are registered, Deneb uses `catalog`
...
- Read-only command paths such as `deneb status`, `deneb status --all`,
  `deneb channels status`, `deneb channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `deneb.extensions`:
...
Security guardrail: every `deneb.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `deneb plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `deneb.setupEntry` can point at a lightweight setup-only module.
When Deneb needs setup surfaces for a disabled channel plugin, or
...
Optional: `deneb.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Deneb load the
...
Channel plugins can advertise setup/discovery metadata via `deneb.channel` and
install hints via `deneb.install`. This keeps the core catalog data-free.
...
Deneb can also merge **external channel catalogs** (for example, an MPM
...
Or point `DENEB_PLUGIN_CATALOG_PATHS` (or `DENEB_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "deneb": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "deneb/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `DenebPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Deneb stays opinionated without becoming hardcoded to one

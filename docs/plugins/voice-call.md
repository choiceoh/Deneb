...
read_when:
  - You want to place an outbound voice call from Deneb
...
Voice calls for Deneb via a plugin. Supports outbound notifications and
...
- Use `deneb voicecall ...` or the `voice_call` tool
...
```bash
deneb plugins install ./extensions/voice-call
...
```bash
deneb voicecall call --to "+15555550123" --message "Hello from Deneb"
deneb voicecall start --to "+15555550123"   # alias for call
deneb voicecall continue --call-id <id> --message "Any questions?"
deneb voicecall speak --call-id <id> --message "One moment"
deneb voicecall end --call-id <id>
deneb voicecall status --call-id <id>
deneb voicecall tail
deneb voicecall latency                     # summarize turn latency from logs
deneb voicecall expose --mode funnel
```

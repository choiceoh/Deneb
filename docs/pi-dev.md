...
This guide summarizes a sane workflow for working on the pi integration in Deneb.
...
```bash
DENEB_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm deneb agent --message "Hello" --thinking low`
...
State lives under the Deneb state directory. Default is `~/.deneb`. If `DENEB_STATE_DIR` is set, use that directory instead.

To reset everything:

- `deneb.json` for config

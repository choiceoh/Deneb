---
summary: "CLI reference for `deneb message` (send + channel actions)"
read_when:
...
```
deneb message <subcommand> [flags]
```
...
- On cache miss, Deneb will attempt a live directory lookup when the provider supports it.
...
- `deneb message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Deneb does not force a `default` account SecretRef scope
...
```
deneb message send --channel discord \
...
```
deneb message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
deneb message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
deneb message poll --channel msteams \
...
```
deneb message react --channel slack \
...
```
deneb message react --channel signal \
...
```
deneb message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
deneb message send --channel telegram --target @mychat \

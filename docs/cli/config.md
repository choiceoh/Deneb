---
summary: "CLI reference for `deneb config` (get/set/unset/file/validate)"
read_when:
...
Config helpers for non-interactive edits in `deneb.json`: get/set/unset/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `deneb configure`).

## Examples

```bash
deneb config file
deneb config get browser.executablePath
...
deneb config set secrets.providers.vaultfile --provider-source file --provider-path /etc/deneb/secrets.json --provider-mode json
...
deneb config validate
deneb config validate --json
```
...
deneb config get agents.list[0].id
```
...
deneb config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
deneb config set agents.defaults.heartbeat.every "0m"
deneb config set gateway.port 19001 --strict-json
deneb config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`deneb config set` supports four assignment styles:

1. Value mode: `deneb config set <path> <value>`
2. SecretRef builder mode:

```bash
deneb config set channels.discord.token \
...
```bash
deneb config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/deneb-vault \
  --provider-arg read \
...
deneb config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/deneb/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `deneb.json`.
...
- `config file`: Print the active config file path (resolved from `DENEB_CONFIG_PATH` or default location).

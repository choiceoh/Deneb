---
summary: "CLI reference for `deneb plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
deneb plugins list
deneb plugins install <path-or-spec>
deneb plugins inspect <id>
deneb plugins enable <id>
deneb plugins disable <id>
deneb plugins uninstall <id>
deneb plugins doctor
deneb plugins update <id>
deneb plugins update --all
deneb plugins marketplace list <marketplace>
```

Bundled plugins ship with Deneb but start disabled. Use `plugins enable` to
activate them.

Native Deneb plugins must ship `deneb.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: deneb` or `Format: bundle`. Verbose list/info
...
```bash
deneb plugins install <path-or-spec>
deneb plugins install <npm-spec> --pin
deneb plugins install <plugin>@<marketplace>
deneb plugins install <plugin> --marketplace <marketplace>
```
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Deneb
...
```bash
deneb plugins install <plugin-name> --marketplace <marketplace-name>
deneb plugins install <plugin-name> --marketplace <owner/repo>
deneb plugins install <plugin-name> --marketplace ./my-marketplace
```
...
For local paths and archives, Deneb auto-detects:

- native Deneb plugins (`deneb.plugin.json`)
...
```bash
deneb plugins install -l ./my-plugin
```
...
```bash
deneb plugins uninstall <id>
deneb plugins uninstall <id> --dry-run
deneb plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$DENEB_STATE_DIR/extensions/<id>`). Use
...
deneb plugins update <id>
deneb plugins update --all
deneb plugins update <id> --dry-run
```

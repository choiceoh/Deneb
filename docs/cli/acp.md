...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Deneb Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`deneb acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Deneb gateway or agent instead.                                                                                                                                        |
...
# Remote Gateway
deneb acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
deneb acp --url wss://gateway-host:18789 --token-file ~/.deneb/gateway.token

# Attach to an existing session key
deneb acp --session agent:main:main

# Attach by label (must already exist)
deneb acp --session-label "support inbox"

# Reset the session key before the first prompt
deneb acp --session agent:main:main --reset-session
```
...
deneb acp client --server-args --url wss://gateway-host:18789 --token-file ~/.deneb/gateway.token

# Override the server command (default: deneb)
deneb acp client --server "node" --server-args deneb.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Deneb Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `deneb acp` over stdio.

Example config (persisted):

```bash
deneb config set gateway.remote.url wss://gateway-host:18789
deneb config set gateway.remote.token <token>
```
...
```bash
deneb acp --session agent:main:main
deneb acp --session agent:design:main
deneb acp --session agent:qa:bug-123
```
...
Deneb bot over ACP, use `acpx` with its built-in `deneb` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx deneb` at `deneb acp`.
3. Target the Deneb session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Deneb ACP session
acpx deneb exec "Summarize the active Deneb session state."

# Persistent named session for follow-up turns
acpx deneb sessions ensure --name codex-bridge
acpx deneb -s codex-bridge --cwd /path/to/repo \
  "Ask my Deneb work agent for recent context relevant to this repo."
```

If you want `acpx deneb` to target a specific Gateway and session key every
time, override the `deneb` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "deneb": {
      "command": "env DENEB_HIDE_BANNER=1 DENEB_SUPPRESS_NOTES=1 deneb acp --url ws://127.0.0.1:18789 --token-file ~/.deneb/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Deneb checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env DENEB_HIDE_BANNER=1 DENEB_SUPPRESS_NOTES=1 node deneb.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Deneb agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Deneb ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`DENEB_GATEWAY_TOKEN`, `DENEB_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`DENEB_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `DENEB_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `deneb acp client` sets `DENEB_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `deneb`).

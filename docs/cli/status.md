---
summary: "CLI reference for `deneb status` (diagnostics, probes, usage snapshots)"
read_when:
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `deneb update` (see [Updating](/install/updating)).

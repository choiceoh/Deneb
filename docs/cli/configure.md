---
summary: "CLI reference for `deneb configure` (interactive configuration prompts)"
read_when:
...
Tip: `deneb config` without a subcommand opens the same wizard. Use
`deneb config get|set|unset` for non-interactive edits.
...
```bash
deneb configure
deneb configure --section model --section channels
```

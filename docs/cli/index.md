---
summary: "Deneb CLI reference for `deneb` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.deneb-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.deneb-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `deneb update` (source installs only).
...
Deneb uses a lobster palette for CLI output.
...
```
deneb [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
- `deneb security audit` — audit config + local state for common security foot-guns.
- `deneb security audit --deep` — best-effort live Gateway probe.
- `deneb security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `deneb secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `deneb secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `deneb secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `deneb secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `deneb plugins list` — discover plugins (use `--json` for machine output).
- `deneb plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `deneb plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `deneb plugins marketplace list <marketplace>` — list marketplace entries before install.
- `deneb plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `deneb plugins doctor` — report plugin load errors.
...
- `deneb memory status` — show index stats.
- `deneb memory index` — reindex memory files.
- `deneb memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.deneb/workspace`).
...
Non-interactive config helpers (get/set/unset/file/validate). Running `deneb config` with no
...
- `config set --dry-run`: validate assignments without writing `deneb.json` (exec SecretRef checks are skipped by default).
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `deneb health` or `deneb status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `deneb doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Deneb moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
deneb channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
deneb channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
deneb channels remove --channel discord --account work --delete
...
- `deneb message send --target +15555550123 --message "Hi"`
- `deneb message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Deneb may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Deneb can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `deneb status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Deneb services are treated as first-class and aren't flagged as "extra".
...
`deneb models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `DENEB_GATEWAY_TOKEN` / `DENEB_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`deneb browser`](/cli/browser) and the [Browser tool](/tools/browser).

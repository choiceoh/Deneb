---
summary: "CLI reference for `deneb onboard` (interactive onboarding)"
read_when:
...
```bash
deneb onboard
deneb onboard --flow quickstart
deneb onboard --flow manual
deneb onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`DENEB_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `deneb gateway run`.
...
- Fastest first chat: `deneb dashboard` (Control UI, no channel setup).

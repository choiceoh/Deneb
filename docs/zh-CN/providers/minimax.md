---
read_when:
  - 你想在 Deneb 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Deneb 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
deneb plugins enable minimax  # 如果已加载则跳过。
deneb gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/deneb/deneb/tree/main/extensions/minimax)。
...
- 使用 `deneb models list` 和 `deneb models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `deneb configure` 并选择 **MiniMax M2.5**，或者

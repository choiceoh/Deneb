...
Deneb 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`deneb gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`deneb onboard --install-daemon`
- 直接安装：`deneb gateway install`
- 配置流程：`deneb configure` → 选择 **Gateway 服务**
- 修复/迁移：`deneb doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.deneb.gateway` 或 `ai.deneb.<profile>`；旧版为 `com.deneb.*`）
- Linux/WSL2：systemd 用户服务（`deneb-gateway[-<profile>].service`）

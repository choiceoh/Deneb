---
read_when:
  - 你想在 GCP 上 24/7 运行 Deneb
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 Deneb Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 Deneb（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 Deneb Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"Deneb 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 Deneb Gateway 网关
- 在主机上持久化 `~/.deneb` + `~/.deneb/workspace`（重启/重建后仍保留）
...
```bash
gcloud projects create my-deneb-project --name="Deneb Gateway"
gcloud config set project my-deneb-project
```
...
```bash
gcloud compute instances create deneb-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh deneb-gateway --zone=us-central1-a
```
...
```bash
DENEB_IMAGE=deneb:latest
DENEB_GATEWAY_TOKEN=change-me-now
DENEB_GATEWAY_BIND=lan
DENEB_GATEWAY_PORT=18789

DENEB_CONFIG_DIR=/home/$USER/.deneb
DENEB_WORKSPACE_DIR=/home/$USER/.deneb/workspace

GOG_KEYRING_PASSWORD=change-me-now
XDG_CONFIG_HOME=/home/node/.deneb
```
...
services:
  deneb-gateway:
    ...
    environment:
      ...
      - NODE_ENV=production
      - TERM=xterm-256color
      - DENEB_GATEWAY_BIND=${DENEB_GATEWAY_BIND}
      - DENEB_GATEWAY_PORT=${DENEB_GATEWAY_PORT}
      - DENEB_GATEWAY_TOKEN=${DENEB_GATEWAY_TOKEN}
      - GOG_KEYRING_PASSWORD=${GOG_KEYRING_PASSWORD}
      - XDG_CONFIG_HOME=${XDG_CONFIG_HOME}
      - PATH=/home/linuxbrew/.linuxbrew/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
    volumes:
      - ${DENEB_CONFIG_DIR}:/home/node/.deneb
      - ${DENEB_WORKSPACE_DIR}:/home/node/.deneb/workspace
    ports:
      # 推荐：在 VM 上保持 Gateway 网关仅绑定 loopback；通过 SSH 隧道访问。
      # 要公开暴露，移除 `127.0.0.1:` 前缀并相应配置防火墙。
      - "127.0.0.1:${DENEB_GATEWAY_PORT}:18789"
...
```bash
docker compose exec deneb-gateway which gog
docker compose exec deneb-gateway which goplaces
docker compose exec deneb-gateway which wacli
```
...
```bash
docker compose logs -f deneb-gateway
```
...
```bash
gcloud compute ssh deneb-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
Deneb 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.deneb/`           | 主机卷挂载    | 包括 `deneb.json`、令牌  |
| 模型认证配置文件 | `/home/node/.deneb/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.deneb/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.deneb/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.deneb/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.deneb/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
...
```bash
# 首先停止 VM
gcloud compute instances stop deneb-gateway --zone=us-central1-a

# 更改机器类型
gcloud compute instances set-machine-type deneb-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# 启动 VM
gcloud compute instances start deneb-gateway --zone=us-central1-a
```
...
1. 创建服务账户：
   ...
   ```bash
   gcloud iam service-accounts create deneb-deploy \
     --display-name="Deneb Deployment"
   ```

2. 授予 Compute Instance Admin 角色（或更窄的自定义角色）：
   ```bash
   gcloud projects add-iam-policy-binding my-deneb-project \
     --member="serviceAccount:deneb-deploy@my-deneb-project.iam.gserviceaccount.com" \

...
所有日志配置都在 `~/.deneb/deneb.json` 的 `logging` 下。
...
- Deneb 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `deneb plugins enable diagnostics-otel` 启用插件。
...
- `deneb.tokens`（计数器，属性：`deneb.token`、`deneb.channel`、`deneb.provider`、`deneb.model`）
- `deneb.cost.usd`（计数器，属性：`deneb.channel`、`deneb.provider`、`deneb.model`）
- `deneb.run.duration_ms`（直方图，属性：`deneb.channel`、`deneb.provider`、`deneb.model`）
- `deneb.context.tokens`（直方图，属性：`deneb.context`、`deneb.channel`、`deneb.provider`、`deneb.model`）

消息流：

- `deneb.webhook.received`（计数器，属性：`deneb.channel`、`deneb.webhook`）
- `deneb.webhook.error`（计数器，属性：`deneb.channel`、`deneb.webhook`）
- `deneb.webhook.duration_ms`（直方图，属性：`deneb.channel`、`deneb.webhook`）
- `deneb.message.queued`（计数器，属性：`deneb.channel`、`deneb.source`）
- `deneb.message.processed`（计数器，属性：`deneb.channel`、`deneb.outcome`）
- `deneb.message.duration_ms`（直方图，属性：`deneb.channel`、`deneb.outcome`）

队列 + 会话：

- `deneb.queue.lane.enqueue`（计数器，属性：`deneb.lane`）
- `deneb.queue.lane.dequeue`（计数器，属性：`deneb.lane`）
- `deneb.queue.depth`（直方图，属性：`deneb.lane` 或 `deneb.channel=heartbeat`）
- `deneb.queue.wait_ms`（直方图，属性：`deneb.lane`）
- `deneb.session.state`（计数器，属性：`deneb.state`、`deneb.reason`）
- `deneb.session.stuck`（计数器，属性：`deneb.state`）
- `deneb.session.stuck_age_ms`（直方图，属性：`deneb.state`）
- `deneb.run.attempt`（计数器，属性：`deneb.attempt`）
...
- `deneb.webhook.error`
  - `deneb.channel`、`deneb.webhook`、`deneb.chatId`、`deneb.error`
- `deneb.message.processed`
  - `deneb.channel`、`deneb.outcome`、`deneb.chatId`、`deneb.messageId`、`deneb.sessionKey`、`deneb.sessionId`、`deneb.reason`
- `deneb.session.stuck`
  - `deneb.state`、`deneb.ageMs`、`deneb.queueDepth`、`deneb.sessionKey`、`deneb.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `deneb doctor`。

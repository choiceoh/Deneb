...
summary: 设置指南：在保持最新的同时保持你的 Deneb 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.deneb/workspace`（工作区）+ `~/.deneb/deneb.json`（配置）。
...
- **配置：** `~/.deneb/deneb.json`（JSON/JSON5 格式）
- **工作区：** `~/.deneb/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm deneb setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Deneb.app**（菜单栏）。
...
- 运行 `deneb setup`，然后 `deneb channels login`，然后手动启动 Gateway 网关（`deneb gateway`）。
...
- **配对允许列表**：`~/.deneb/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.deneb/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.deneb/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.deneb/workspace` 和 `~/.deneb/` 保持为"你的东西"；不要将个人提示/配置放入 `deneb` 仓库。

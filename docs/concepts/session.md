...
All session state is **owned by the gateway** (the “master” Deneb). UI clients (macOS app, WebChat, etc.) must query the gateway for session lists and token counts instead of reading local files.

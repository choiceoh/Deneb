...
Deneb selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Deneb can use (plus aliases).
...
- If you omit the provider, Deneb treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
deneb models list
deneb models status
deneb models set <provider/model>
deneb models set-image <provider/model>

deneb models aliases list
deneb models aliases add <alias> <provider/model>
deneb models aliases remove <alias>

deneb models fallbacks list
deneb models fallbacks add <provider/model>
deneb models fallbacks remove <provider/model>
deneb models fallbacks clear

deneb models image-fallbacks list
deneb models image-fallbacks add <provider/model>
deneb models image-fallbacks remove <provider/model>
deneb models image-fallbacks clear
```

`deneb models` (no subcommand) is a shortcut for `models status`.
...
`deneb models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.deneb/agents/<agentId>/agent/models.json`). This file
...
This applies whenever Deneb regenerates `models.json`, including command-driven paths like `deneb agent`.

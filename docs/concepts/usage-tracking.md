...
- `/usage cost` in chats: local cost summary aggregated from Deneb session logs.
- CLI: `deneb status --usage` prints a full per-provider breakdown.
- CLI: `deneb channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

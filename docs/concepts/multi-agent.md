...
- **Session store** (chat history + routing state) under `~/.deneb/agents/<agentId>/sessions`.
...
available from `~/.deneb/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.deneb` (or `DENEB_STATE_DIR`)
- Workspace: `~/.deneb/workspace` (or `~/.deneb/workspace-<agentId>`)
- Agent dir: `~/.deneb/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.deneb/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Deneb runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.deneb/workspace` (or `~/.deneb/workspace-<profile>` when `DENEB_PROFILE` is set).
- State defaults to `~/.deneb/agents/main/agent`.
...
```bash
deneb agents add coding
deneb agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.deneb/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.deneb/workspace-main" },
      { id: "coding", workspace: "~/.deneb/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.deneb/workspace-main" },
      { id: "alerts", workspace: "~/.deneb/workspace-alerts" },
    ],
  },
  bindings: [
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.deneb/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.deneb/workspace-opus",

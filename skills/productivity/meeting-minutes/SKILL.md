...
게이트웨이가 Plaud 녹음을 이미 분석해 `프로젝트/<이름>/회의록/` 아래에 페이지를
남긴 경우가 많다 (`plaud_recordings.go`). 사용자가 "회의록 만들어줘"라고 해도
**같은 날짜·제목·Plaud 녹음 id 페이지가 있으면** 새로 쓰지 말고 그 페이지를
읽고 보완·정정하라. 중복 페이지를 만들지 않는다.
...
- **영속화는 위키 저장이 기본이자 전부다.** (구버전의 "본인 지메일 자동 전송" 스텝은
  은퇴 — 에이전트 표면에서 Gmail 발신 도구가 제거되어 `gmail(action="send")` 류 호출은
  불가능하다. 메일 읽기는 `mail_archive`로만 한다.)
...
- **참석자·외부 공유** — 에이전트 표면에는 메일 발신 도구가 없어 직접 발송은 불가하다.
  외부 공유 요청이면 전문 파일(`send_file`) 또는 본문 텍스트를 제공해 사용자가 직접

...
몇 개)을 돌려주고, 전체 회의록은 위키에 있다고 알린다. 원하면 원문을 파일·메일로
보낼 수 있음을 한 줄로 덧붙인다(아래). 그 외엔 캐묻지 마라.

## 전달 — 파일·메일

요약은 채팅으로, 전문은 위키에 둔다. 그 위에 사용자가 원하면 회의록 **원문을 파일로
받거나 메일로 보낼 수 있다.** 강요하지 말고, 요약 끝에 한 줄로 알린다("전문은 위키에
있어요 — 파일로 받거나 메일로 보내드릴까요?"). 요청이 오면 한 턴에 끝낸다.
...
- **메일로 보내기** — "회의록 메일로 보내줘"면 `gmail(action="send")`로 제목
  `[회의록] YYYY-MM-DD 주제`, 본문에 회의록 전문(길면 요약 + 핵심)을 담아 보낸다.
- **수신자 확인 필수** — 본인 메일로 보내는 게 기본이다. 참석자·외부로 보낼 땐 위키
  인물에서 이메일을 찾되, **보내기 전에 수신자를 확인받아라**(잘못된 외부 발송은
  되돌릴 수 없다). 사용자가 지정한 수신자만 넣고, 추측한 주소로 보내지 마라.

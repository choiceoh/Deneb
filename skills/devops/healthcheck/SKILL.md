...
metadata:
  { "deneb": { "emoji": "🏥", "tags": ["security", "hardening", "audit", "monitoring"] } }
---

# Deneb Host Hardening

## Overview

Assess and harden the host running Deneb, then align it to a user-defined risk tolerance without breaking access. Use Deneb security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Deneb changes the host firewall, SSH, or OS updates; it does not.
...
5. Deneb gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Deneb can access sensitive data.
...
- Credential hygiene notes (location of Deneb creds, prefer disk encryption)
...
Follow the durable-memory prompt format used by Deneb compaction:

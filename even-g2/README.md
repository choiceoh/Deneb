...
안경 마이크 → 게이트웨이 → 자막. 내장 Translate 를 흉내내는 게 아니라, **Deneb 이 아는 고유명사를 쓸 수 있는 경로**를 만드는 것이다.

```
audioControl(true, Glasses) → audioEvent.audioPcm → 3초 창 → POST /api/even/audio
                                                              ↓ MOSS ASR + tiny(qwen) 번역
                                                          자막 3줄 유지
```

- **시작·중지는 폰 화면에** 있다. 안경 제스처 4개가 이미 다 쓰였다(↓↑=커서/페이지, 탭=상세, 더블탭=종료/확인). 통역 중에는 안경 탭이 중지로 동작한다.
- **통역 중에는 폴링을 멈춘다.** glance 재그리기가 문장 중간에 자막을 지워버린다.
- **밀린 창은 버린다**(`sending` 인플라이트 가드). 네트워크가 말보다 느릴 때 큐를 쌓으면 문장마다 더 뒤처진다 — 최신 자막이 밀린 자막보다 낫다.
- 창 3초는 지연의 지배항이다(실측: 전사 0.3s + 번역 0.5s). 더 짧게 자르면 한 문장이 두 창에 걸려 양쪽 다 맥락 없이 잘못 전사된다.

07-26 실측 근거(영어·표준중국어·쓰촨 억양·광둥어)는 [[#번역-경로-선택-근거]] 아래.

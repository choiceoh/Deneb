...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `deneb-e2e-${name}-`));
  const configDir = path.join(homeDir, ".deneb");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          DENEB_SKIP_PROVIDERS: "1",
          DENEB_SKIP_GMAIL_WATCHER: "1",
          DENEB_SKIP_CRON: "1",
          DENEB_SKIP_BROWSER_CONTROL_SERVER: "1",
          DENEB_SKIP_CANVAS_HOST: "1",
          DENEB_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },

...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: DenebConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): DenebConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("deneb/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("deneb/plugin-sdk/acp-runtime")>(
    "deneb/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("deneb/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("deneb/plugin-sdk/reply-runtime")>(
...
vi.mock("deneb/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("deneb/plugin-sdk/config-runtime")>(
...
vi.mock("deneb/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("deneb/plugin-sdk/runtime-env")>(
    "deneb/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("deneb/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("deneb/plugin-sdk/infra-runtime")>(
    "deneb/plugin-sdk/infra-runtime",
  );
  return {
    ...actual,

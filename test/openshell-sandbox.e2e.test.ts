...
const DENEB_OPENSHELL_E2E = process.env.DENEB_E2E_OPENSHELL === "1";
const DENEB_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const DENEB_OPENSHELL_COMMAND = process.env.DENEB_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "deneb-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && DENEB_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: DENEB_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(DENEB_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-openshell-e2e-"));
      ...
      const gatewayName = `deneb-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `deneb-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: DENEB_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("deneb-openshell-e2e");

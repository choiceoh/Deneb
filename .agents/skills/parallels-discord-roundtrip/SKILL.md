...
```bash
export DENEB_PARALLELS_DISCORD_TOKEN="$(
...
- Use the `deneb` wrapper for guest `message send/read`; `node deneb.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/deneb-parallels-smoke.*`
...
- host inbound nonce appears in `deneb message read` output

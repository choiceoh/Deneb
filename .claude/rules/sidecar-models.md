...
| **VibeVoice-ASR** (9B) | 음성 전사 + 화자분리 + 타임스탬프 (최대 60분·50+개 언어·한국어) | `http://127.0.0.1:18013` (`POST /v1/transcribe`, OpenAI 비호환) | — (게이트웨이 미배선) | transformers+FastAPI 상주. 핫워드로 고유명사 교정. ↓ 상세 |
...
### 코드 통합 — 미배선 (TODO)
- 현재 게이트웨이 코드는 18013 을 **호출하지 않는다.** 음성/녹음 캡처 도구를 `chat/tools/` 에 추가 + `toolreg` 등록 + PaddleOCR 의 `DENEB_OCR_VL_URL` 패턴처럼 `DENEB_ASR_URL` env override + 서버 다운 시 graceful degradation 을 따르는 게 통합 단계.
- 참고: 안드로이드 음성 캡처(#1843)는 **온디바이스 시스템 STT** 를 쓰므로 이 서버와 무관. 서버측 전사가 필요할 때(긴 녹음·화자분리·핫워드 교정) 이 사이드카가 역할.

...
대표페이지 머리말 `kinds:` — 프로젝트가 사업적으로 무엇인지 (복수 허용, **고정 어휘 9개만**):

| 값 | 뜻 |
|---|---|
| 시공 | EPC·턴키·발전소 건설 |
| 모듈 / 인버터 / 케이블 / BESS | 기자재 공급 |
| 풍력 | 풍력 사업 |
| 개발 | 인허가·부지·RPS·가배치 |
| 용역 | 안전관리대행 등 서비스 |
| 협력 | NDA·전략제휴·R&D 검토 |

- 어휘 밖 값은 파싱·렌더에서 **드롭**(`page.go:normalizeKinds` — epc→시공 등 동의어는 자동 접힘). 집계 가능성이 목적이므로 enum 규율 엄수.
- 자가소비·루프탑 같은 **시장 구분은 kinds가 아니라 tags**.

...
| 1차 | 2차 | 뜻 |
|---|---|---|
| 시공 | 토지 · 루프탑 · 수상 · ESS | EPC·턴키 (2차=설치·플랜트 유형; **ESS 사업은 시공**) |
...
- 어휘 밖 값은 파싱·렌더에서 **드롭**(`page.go:normalizeKinds`). 동의어·구세대 flat 값은 자동 승격(모듈→기자재/모듈, 루프탑→시공/루프탑, BESS→시공/ESS, 용역→기타/용역, epc→시공) — 기존 저장 데이터는 손대지 않아도 읽기 시점에 새 체계로 수렴.

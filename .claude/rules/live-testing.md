...
| Command | Description |
|---|---|
| `concurrency` | **전체 동시성 테스트** (queue + fifo + interrupt + abort + multi-conn + rapid) |
| `concurrency queue` | 실행 중 메시지 전송 시 큐잉 검증 |
| `concurrency fifo` | 복수 큐잉된 메시지의 FIFO 순서 드레인 검증 |
| `concurrency interrupt` | `sessions.send`로 실행 중 인터럽트 검증 |
| `concurrency abort` | abort 시 큐 클리어 검증 (큐잉된 메시지 미실행 확인) |
| `concurrency multi-conn` | 같은 세션에 복수 WebSocket 연결 동시 전송 |
| `concurrency rapid` | 연속 빠른 메시지 버스트 (4연발) 안정성 |
...
`concurrency` 명령은 다음을 자동 검증한다:

| 검증 항목 | 설명 |
|---|---|
| **메시지 큐잉** | 실행 중 전송 시 `{status: "queued", reason: "active-run"}` 반환 |
| **큐 드레인** | 실행 완료 후 큐잉된 메시지가 자동 처리됨 |
| **FIFO 순서** | 복수 큐잉 시 입력 순서대로 처리 |
| **인터럽트** | `sessions.send`로 실행 중 작업 취소 + 새 작업 시작 |
| **abort + 큐 클리어** | abort 시 큐잉된 메시지도 함께 폐기 |
| **멀티 커넥션** | 같은 세션에 복수 WebSocket에서 전송해도 안전 |
| **버스트 안정성** | 연속 빠른 메시지 전송에 크래시 없음 |
...
# 세션/큐잉/인터럽트/abort 관련 수정했으면
scripts/dev-live-test.sh concurrency

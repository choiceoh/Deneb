...
- **Cursor 자동 워크트리 + CodeGraph**: `.cursor/hooks.json` SessionStart가 `scripts/dev/cursor-worktree-init.sh`로 `~/.cursor/worktrees/Deneb/<session-id>`를 만들고, `.cursor/mcp.json`이 CodeGraph MCP를 배선. 메인 체크아웃 편집은 `cursor-worktree-guard.sh`가 차단. **`move_agent_to_root`로 git worktree 진입 금지**(브랜치가 `main`으로 덮일 수 있음) — Shell `working_directory`와 Write 경로를 세션 워크트리로 스코핑.

...
**비서실장형 단일 AI 에이전트** (NVIDIA DGX Spark **4노드** 플릿 · 단일 사용자 · Korean-first — 게이트웨이·배포·웜홀=srv4, GPU 보조=srv1·srv2·srv3; 대형 모델은 4노드 전부를 TP=4로 묶어 서빙한다). 한 페르소나가 업무분석(깊이)과 업무비서(능동)를 동시에 수행한다 — 분석/비서 *페르소나* 분리 금지 (기기별 반응형 레이아웃은 허용). 좁고 깊게: 완성도·응집도 우선, 옵션보다 opinionated 기본값.
...
- **편집 레인은 배선+계측돼 있다**: implementer 프리셋이 `codegraph_*`를 허용목록에 갖고 impact·node를 턴1부터 호출 가능(`toolpreset.codegraphTools`), L4 배차 워크트리는 프로덕션 인덱스를 시드받고, 계약 첫 단계가 `codegraph impact`다. `pr.sh land`는 변경 파급 브리핑(`scripts/dev/impact_brief.py`)을 PR 본문에 자동 부착하고, RSI Bench `impact-first`(Process w8)가 "첫 편집보다 앞선 그래프 조회" 비율을 센다. 규율이 아니라 지표다.

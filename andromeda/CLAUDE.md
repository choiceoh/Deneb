...
pnpm gen:wire      # regen src/gen/miniappWire.ts from the gateway's //deneb:wire structs (needs Go; finds ../gateway-go in this monorepo)
...
| `computer.ts`          | Computer use: parses gateway `computer` push frames (the `computer` chat tool — host-OS screenshot/mouse/keyboard), gates on the 설정 › 일반 › 컴퓨터 조종 toggle (default OFF), executes through the Tauri `computer_action` command (`src-tauri/src/computer.rs`) and reports back via `miniapp.computer.result`. `ProactivePanel` intercepts the frame and shows a visible "컴퓨터 조종" nudge.     |
...
| `components/`          | `Workstation` (shell: tiled work area 1–3 panes + split-add strip), `CommandPalette` (⌘K / 레일 명령·검색 — 이동·분할·레이아웃·위키 quick-open·데네브에게 묻기), `Sidebar` (nav + Alt+클릭 분할), `AIPanel`, `ProactivePanel` (nudges + gateway workspace-command execution), `Grid` (+`GridNotice`, `RowBtn`), `Modal` (+`Field`, `Detail`), `ErrorBoundary`.                                         |

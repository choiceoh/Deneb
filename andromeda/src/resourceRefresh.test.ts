...
describe("resource refresh mapping", () => {
  it("when expands calendar resources because the dashboard and month pane use different lists", () => {
    expect(relatedResourcesForResource("calendar-range")).toEqual(["calendar", "calendar-range"]);
  });

  it("when expands wiki resources because memory pages feed search and notebooks", () => {
    expect(relatedResourcesForResource("wiki")).toEqual(["wiki", "search", "notebook"]);
  });

  it("when maps AI tool names to every resource cache they can affect", () => {
    expect(
      new Set(
  ...
  it("when maps direct file tools to the files cache", () => {

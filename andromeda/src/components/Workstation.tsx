...
export function Workstation({ cfg }: { cfg: GatewayConfig }) {
  const { view, setView, codeMode, connected, notebookFolded } = useWorkspace();
  ...
  // 노트북 상단(자료) 접힘 — NotebookPane의 바 버튼이 컨텍스트로 토글하고, 여기서 그리드
  // 상단 행을 auto(바 높이)로 줄여 확보된 높이를 하단 채팅에 넘긴다.
  const topFolded = bottomChat && notebookFolded;
  ...
  return (
    <div className={"workstation-shell" + (bottomChat ? " ws-bottom-chat" : "") + (topFolded ? " ws-top-folded" : "")}>

...
If you believe you've found a security issue in Deneb, please report it privately.
...
- Tested version details (Deneb version and/or commit SHA).
...
- Demonstrated impact tied to Deneb's documented trust boundaries.
- For exposed-secret reports: proof the credential is Deneb-owned (or grants access to Deneb-operated infrastructure/services).
...
- Telegram inbound webhook signature findings for paths not used by this repo's Telegram integration.
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Deneb. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Deneb is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Deneb does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Deneb can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Deneb, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Deneb's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Deneb's trusted computing base for a gateway.
...
- Using Deneb in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.deneb`, workspace files like `MEMORY.md` / `memory/*.md`)
...
Deneb security guidance assumes:

- The host where Deneb runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.deneb` state/config (including `deneb.json`) is effectively a trusted operator.
...
Deneb separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Deneb can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Deneb process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Deneb uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/deneb` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/deneb` (or `deneb-<uid>` on multi-user hosts).
...
For threat model + hardening guidance (including `deneb security audit --deep` and `--fix`), see:

- `https://docs.deneb.ai/gateway/security`
...
Deneb's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `deneb gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Deneb keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `deneb security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__deneb__/canvas/`, `/__deneb__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__deneb__/canvas/`, `/__deneb__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

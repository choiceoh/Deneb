...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://deneb.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://deneb.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://deneb.ai/abc-123", baseHref)).toBe(
      "blob:https://deneb.ai/abc-123",
    );
  });

...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as DenebApp);
}
...
async function sendChatMessageNow(
  ...
  const runId = await sendChatMessage(host as unknown as DenebApp, message, opts?.attachments);
...
async function clearChatHistory(host: ChatHost) {
  ...
  try {
    ...
    host.chatRunId = null;
    await loadChatHistory(host as unknown as DenebApp);
  } catch (err) {
    host.lastError = String(err);
  }
  scheduleChatScroll(host as unknown as Parameters<typeof scheduleChatScroll>[0]);
...
export async function refreshChat(host: ChatHost, opts?: { scheduleScroll?: boolean }) {
  await Promise.all([
    loadChatHistory(host as unknown as DenebApp),
    loadSessions(host as unknown as DenebApp, {
      activeMinutes: 0,
      limit: 0,
      includeGlobal: true,
      includeUnknown: true,

const SETTINGS_KEY_PREFIX = "deneb.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "deneb.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "deneb.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "deneb.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__DENEB_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__DENEB_CONTROL_UI_BASE_PATH__.trim();
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("deneb.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;

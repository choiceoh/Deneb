...
export async function handleWhatsAppStart(host: DenebApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: DenebApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: DenebApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: DenebApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: DenebApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: DenebApp): string {
...
function resolveGatewayHttpAuthHeader(host: DenebApp): string | null {
...
function buildGatewayHttpHeaders(host: DenebApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: DenebApp) {
...
export function handleNostrProfileToggleAdvanced(host: DenebApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: DenebApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: DenebApp) {
  const state = host.nostrProfileFormState;

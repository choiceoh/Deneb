...
go-ffi:
	cd gateway-go && $(GO_ENV) go build $(GO_TAGS) $(GO_LDFLAGS) ./...

# Build without Rust core (FFI fallbacks). CGo stays on for SQLite.
go-pure:
	cd gateway-go && $(GO_ENV) go build -tags 'no_ffi,fts5' $(GO_LDFLAGS) ./...

go-run: go
	cd gateway-go && $(GO_ENV) go run $(GO_TAGS) ./cmd/gateway/
...
go-dev:
	...
	@while true; do \
		if ! $(GO_ENV) go build -C gateway-go $(GO_TAGS) $(GO_LDFLAGS) -o /tmp/deneb-gateway-dev ./cmd/gateway/; then \
...
go-test:
	cd gateway-go && $(GO_ENV) go test $(GO_TAGS) -race -count=1 ./...

go-test-pure:
	cd gateway-go && $(GO_ENV) go test -tags 'no_ffi,fts5' -count=1 ./...

go-test-fuzz:
	cd gateway-go && $(GO_ENV) go test $(GO_TAGS) ./internal/bridge/ -fuzz=FuzzParseRequestFrame -fuzztime=10s

go-vet:
	cd gateway-go && $(GO_ENV) go vet $(GO_TAGS) ./...
...
go-binary: rust go
	cd gateway-go && $(GO_ENV) go build $(GO_TAGS) -trimpath $(GO_LDFLAGS) -o ../dist/deneb-gateway ./cmd/gateway/
...
go-bench:
	cd gateway-go && $(GO_ENV) go test $(GO_TAGS) -bench=. -benchmem -run='^$$' ./...

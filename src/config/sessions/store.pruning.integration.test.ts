...
// Keep integration tests deterministic: never read a real deneb.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.DENEB_SESSION_CACHE_TTL_MS;
    process.env.DENEB_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.DENEB_SESSION_CACHE_TTL_MS;
    } else {
      process.env.DENEB_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");

...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): DenebConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: DenebConfig, env: NodeJS.ProcessEnv) =>
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".deneb/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".deneb/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });

...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ DENEB_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-deneb",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-deneb#general");
  });
  ...
  it("derives session transcripts dir from DENEB_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { DENEB_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when DENEB_STATE_DIR differs from stored paths", () => {

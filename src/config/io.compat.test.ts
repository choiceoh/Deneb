...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.deneb/deneb.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".deneb", 19001);
  ...
  it("defaults to ~/.deneb/deneb.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".deneb", "deneb.json"));
    });
  });

  it("uses DENEB_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { DENEB_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".deneb", "deneb.json"));
    });
  });

  it("honors explicit DENEB_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".deneb", 20002, "custom.json");
      const io = createIoForHome(home, { DENEB_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".deneb", 20003, "legacy-custom.json");

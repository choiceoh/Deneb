...
const ETC_DENEB_DIR = path.join(ROOT_DIR, "etc", "deneb");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "deneb.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcDenebPath(...parts: string[]) {
  return path.join(ETC_DENEB_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcDenebPath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-includes-symlink-"));
      try {
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-includes-hardlink-"));
      try {
    ...
    it("rejects oversized include files", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-includes-big-"));
      try {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "deneb.json")),

...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  deneb config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  deneb config set ${policyPath} "pairing"`,
  ].join("\n");
}

export function isNumericPathSegment(raw: string): boolean {
  return /^[0-9]+$/.test(raw);
}

export function isWritePlainObject(value: unknown): value is Record<string, unknown> {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}

export function hasOwnObjectKey(value: Record<string, unknown>, key: string): boolean {
...
export const WRITE_PRUNED_OBJECT: unique symbol = Symbol("write-pruned-object");
...
export function unsetPathForWriteAt(
  value: unknown,
  pathSegments: string[],
  depth: number,
...
export function coerceConfig(value: unknown): DenebConfig {
...
export function unsetPathForWrite(
  root: DenebConfig,
  pathSegments: string[],
): { changed: boolean; next: DenebConfig } {

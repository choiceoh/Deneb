...
import type { ConfigFileSnapshot } from "./types.deneb.js";
import { DenebSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(DenebSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.deneb/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/deneb/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/deneb/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/deneb/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/deneb/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = DenebSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "DenebConfig";
    const hints = mainSchemaHints;

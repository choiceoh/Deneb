...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("deneb-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "deneb-sandbox-browser-global" },
      agentBrowser: { network: "deneb-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("deneb-sandbox-browser-agent");
  });

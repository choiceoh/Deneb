...
import { buildWebSearchProviderConfig, withTempHome, writeDenebConfig } from "./test-helpers.js";
import { DenebSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = DenebSchema.safeParse({
      $schema: "https://deneb.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://deneb.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = DenebSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = DenebSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeDenebConfig(home, {
        gateway: { bind: "${DENEB_BIND}" },
      });

      const prev = process.env.DENEB_BIND;
      process.env.DENEB_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.DENEB_BIND;
        } else {
          process.env.DENEB_BIND = prev;
        }
      }
    });
  });

/**
 * Native bridge for the Aurora context engine.
 *
...
import { resolveAuroraConfig } from "./src/db/config.js";
import type { AuroraDependencies } from "./src/types.js";

const log = createSubsystemLogger("aurora");
...
function nativeResolveModel(
  ...
  if (!raw) {
    throw new Error("No model configured for Aurora summarization.");
  }
...
async function nativeCallGateway(params: {
  ...
  switch (params.method) {
    ...
    default:
      throw new Error(`Unsupported gateway method in Aurora native bridge: ${params.method}`);
  }
}
...
export function createNativeAuroraDependencies(): AuroraDependencies {
  const cfg = loadConfig();

  // Read Aurora config from plugins.entries.aurora.config, with fallback to the
  ...
  const auroraConfig = (entries?.["aurora"]?.config as Record<string, unknown> | undefined) ?? {};
  const legacyConfig =
    (entries?.["lossless-claw"]?.config as Record<string, unknown> | undefined) ?? {};
  const pluginConfig = { ...legacyConfig, ...auroraConfig };
  const config = resolveAuroraConfig(process.env, pluginConfig);
  ...
  return {
    ...
    onCompaction: (event) => {
      ...
      nativeCallGateway({
        method: "agent",
        params: {
          sessionKey: process.env.AURORA_NOTIFY_SESSION || "agent:main",
          message: text,
          silent: true,
        },
        timeoutMs: 5000,

...
import { getAuroraDbFeatures } from "./features.js";
...
export function runAuroraMigrations(db: DatabaseSync, options?: { fts5Available?: boolean }): void {
  db.exec(`
  ...
  const fts5Available = options?.fts5Available ?? getAuroraDbFeatures(db).fts5Available;
  if (!fts5Available) {
    return;
  }

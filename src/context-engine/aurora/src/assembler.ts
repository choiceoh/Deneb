...
/**
 * Build Aurora usage guidance for the runtime system prompt.
 *
...
function buildSystemPromptAddition(summarySignals: SummaryPromptSignal[]): string | undefined {
  ...
  sections.push(
    ...
    "",
    "**Recall priority:** Aurora tools first, then vega (for Granola/Limitless/pre-Aurora data), then memory_search as last resort.",
    "",
    "**Tool escalation:**",
    "1. `aurora_grep` — search by regex or full-text across messages and summaries",
    "2. `aurora_describe` — inspect a specific summary (cheap, no sub-agent)",
    "3. `aurora_expand_query` — deep recall: spawns bounded sub-agent, expands DAG, returns answer with cited summary IDs (~120s, don't ration it)",
    "",
    "**`aurora_expand_query` usage** — two patterns (always requires `prompt`):",
    '- With IDs: `aurora_expand_query(summaryIds: ["sum_xxx"], prompt: "What config changes were discussed?")`',
    '- With search: `aurora_expand_query(query: "database migration", prompt: "What strategy was decided?")`',
    "- Optional: `maxTokens` (default 2000), `conversationId`, `allConversations: true`",
    "",
    '**Summaries include "Expand for details about:" footers** listing compressed specifics. Use `aurora_expand_query` with that summary\'s ID to retrieve them.',
  );
  ...
  if (heavilyCompacted) {
    sections.push(
      ...
      "1) `aurora_grep` to locate relevant summary/message IDs",
      "2) `aurora_expand_query` with a focused prompt",

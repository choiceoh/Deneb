...
export type AuroraSummarizeOptions = {
...
export type AuroraSummarizeFn = (
  text: string,
  aggressive?: boolean,
  options?: AuroraSummarizeOptions,
) => Promise<string>;

export type AuroraSummarizerParams = {
  provider?: unknown;
  model?: unknown;
  config?: unknown;
  agentDir?: unknown;
...
const AURORA_SUMMARIZER_SYSTEM_PROMPT =
...
function buildDeterministicFallbackSummary(text: string, targetTokens: number): string {
  ...
  return `${trimmed.slice(0, maxChars)}\n[Aurora fallback summary; truncated for context management]`;
}

/**
 * Builds a model-backed Aurora summarize callback from runtime params.
 *
 * Returns `undefined` when model/provider context is unavailable so callers can
 * choose a fallback summarizer.
 */
export async function createAuroraSummarize(params: {
  deps: AuroraDependencies;
  runtimeParams: AuroraSummarizerParams;
  customInstructions?: string;
}): Promise<AuroraSummarizeFn | undefined> {
  ...
  // Prefer aurora entry; fall back to lossless-claw for users who haven't run `deneb doctor`.
  const nestedPluginConfig =
    runtimeConfig?.plugins?.entries?.["aurora"]?.config ??
    runtimeConfig?.plugins?.entries?.["lossless-claw"]?.config;

  const summaryLevels = [
    {
      levelName: "aurora config",
    ...
    },
    {
      ...
      model: process.env.AURORA_SUMMARY_MODEL?.trim() ?? "",
      provider: process.env.AURORA_SUMMARY_PROVIDER?.trim() ?? "",
    },
    {
  ...
  for (const level of summaryLevels) {
    ...
    params.deps.log.warn(
      `[aurora] summaryModel "${level.model}" at "${level.levelName}" has no summaryProvider or provider prefix. Will attempt resolution without provider.`,
    );
    resolvedSummary = { model: level.model, provider: undefined };
    break;
  }
  ...
  try {
    ...
    console.error(
      `[aurora] createAuroraSummarize: resolveModel FAILED:`,
  ...
  return async (
    ...
    const result = await params.deps.complete({
      ...
      agentDir,
      runtimeConfig: params.runtimeParams.config,
      system: AURORA_SUMMARIZER_SYSTEM_PROMPT,
      messages: [
        {
          role: "user",
          content: prompt,
    ...
    if (!summary) {
      ...
      if (envelopeNormalized.summary) {
        ...
        console.error(
          `[aurora] recovered summary from response envelope; provider=${provider}; model=${model}; ` +
    ...
    if (!summary) {
      ...
      const diagParts = [
        `[aurora] empty normalized summary on first attempt`,
        `provider=${provider}`,
        `model=${model}`,
      ...
      try {
        ...
        if (summary) {
          summarySource = "retry";
          console.error(
            `[aurora] retry succeeded; provider=${provider}; model=${model}; ` +
          ...
          const retryParts = [
            `[aurora] retry also returned empty summary`,
            `provider=${provider}`,
            `model=${model}`,
        ...
        console.error(
          `[aurora] retry failed; provider=${provider} model=${model}; error=${
    ...
    if (!summary) {
      ...
      console.error(
        `[aurora] all extraction attempts exhausted; provider=${provider}; model=${model}; source=fallback`,
      );
    ...
    if (summarySource !== "content") {
      console.error(
        `[aurora] summary resolved via non-content path; provider=${provider}; model=${model}; source=${summarySource}`,
      );
    }

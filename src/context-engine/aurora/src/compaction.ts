...
const log = createSubsystemLogger("aurora-compaction");
...
export class CompactionEngine {
  ...
  private async leafPass(
    ...
    // Wrap all DB mutations in a single transaction to prevent orphaned
    // records on partial failure (matching tryObserverFastPath behavior).
    await this.conversationStore.withTransaction(async () => {
      await this.summaryStore.insertSummary({
        summaryId,
        conversationId,
  ...
  private async persistCompactionEvent(input: {
    ...
    const content = `Aurora compaction ${input.pass} pass (${input.level}): ${input.tokensBefore} -> ${input.tokensAfter}`;

...
const AuroraExpandQuerySchema = Type.Object({
  ...
  tokenCap: Type.Optional(
    Type.Number({
      description:
        "Expansion retrieval token budget across all delegated aurora_expand calls for this query.",
      minimum: 1,
    }),
  ),
});
...
function buildDelegatedExpandQueryTask(params: {
  ...
  return [
    "You are an autonomous Aurora retrieval navigator. Plan and execute retrieval before answering.",
    "",
    "Available tools: aurora_describe, aurora_expand, aurora_grep",
    ...
    "",
    "Strategy:",
    "1. Start with `aurora_describe` on seed summaries to inspect subtree manifests and branch costs.",
    "2. If additional candidates are needed, use `aurora_grep` scoped to summaries.",
    "3. Select branches that fit remaining budget; prefer high-signal paths first.",
    "4. Call `aurora_expand` selectively (do not expand everything blindly).",
    "5. Keep includeMessages=false by default; use includeMessages=true only for specific leaf evidence.",
    `6. Stay within ${params.tokenCap} total expansion tokens across all aurora_expand calls.`,
    "",
    ...
    "",
    "Rules:",
    "- In delegated context, call `aurora_expand` directly for source retrieval.",
    "- DO NOT call `aurora_expand_query` from this delegated session.",
...
async function resolveSummaryCandidates(params: {
  aurora: AuroraContextEngine;
  explicitSummaryIds: string[];
  query?: string;
  conversationId?: number;
}): Promise<SummaryCandidate[]> {
  const retrieval = params.aurora.getRetrieval();
...
export function createAuroraExpandQueryTool(input: {
  deps: AuroraDependencies;
  aurora: AuroraContextEngine;
  /** Session id used for Aurora conversation scoping. */
  sessionId?: string;
  ...
  return {
    ...
    label: "Aurora Expand Query",
    description:
      "Answer a focused question using delegated Aurora expansion. " +
    ...
    parameters: AuroraExpandQuerySchema,
    async execute(_toolCallId, params) {
      ...
      const conversationScope = await resolveAuroraConversationScope({
        aurora: input.aurora,
        deps: input.deps,
      ...
      if (!conversationScope.allConversations && scopedConversationId == null) {
        return jsonResult({
          error:
            "No Aurora conversation found for this session. Provide conversationId or set allConversations=true.",
        });
      }
      ...
      try {
        ...
        const response = (await input.deps.callGateway({
          ...
          params: {
            ...
            extraSystemPrompt: input.deps.buildSubagentSystemPrompt({
              depth: 1,
              maxDepth: 8,
              taskSummary: "Run aurora_expand and return prompt-focused JSON answer",
            }),
          },
        ...
        if (status === "timeout") {
          ...
          return jsonResult({
            error: "aurora_expand_query timed out waiting for delegated expansion (120s).",
          });
        }
        if (status !== "ok") {
          return jsonResult({

...
export type AuroraConversationScope = {
...
/**
 * Resolve Aurora conversation scope for tool calls.
 *
 * Priority:
 * 1. Explicit conversationId parameter
 * 2. allConversations=true (cross-conversation mode)
 * 3. Current session's Aurora conversation
 */
export async function resolveAuroraConversationScope(input: {
  aurora: AuroraContextEngine;
  params: Record<string, unknown>;
  sessionId?: string;
  sessionKey?: string;
  deps?: Pick<AuroraDependencies, "resolveSessionIdFromSessionKey">;
}): Promise<AuroraConversationScope> {
  const { aurora, params } = input;
  ...
  const conversation = await aurora

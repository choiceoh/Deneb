...
type AuroraDelegatedRunReference = {
  pass: number;
...
): AuroraDelegatedRunReference[] | undefined {
  if (!delegated) {
    return undefined;
  }
...
function buildOrchestrationObservability(input: {
  policy: ReturnType<typeof decideAuroraExpansionRouting>;
...
/**
 * Build the runtime Aurora expansion tool with route-vs-delegate orchestration.
 */
export function createAuroraExpandTool(input: {
  ...
  return {
    ...
    label: "Aurora Expand",
    description:
      "Expand compacted conversation summaries from Aurora context engine. " +
    ...
    parameters: AuroraExpandSchema,
    async execute(_toolCallId, params) {
      const retrieval = input.aurora.getRetrieval();
      ...
      if (!input.deps.isSubagentSessionKey(sessionKey)) {
        return jsonResult({
          error:
            "aurora_expand is only available in sub-agent sessions. Use aurora_expand_query to ask a focused question against expanded summaries, or aurora_describe/aurora_grep for lighter lookups.",
        });
      }
      ...
      if (query) {
        try {
          if (resolvedConversationId == null) {
            ...
            const policy = decideAuroraExpansionRouting({
              intent: "query_probe",
              query,

...
function buildExpansionRecursionRecoveryGuidance(originSessionKey: string): string {
  return (
    "Recovery: In delegated sub-agent sessions, call `aurora_expand` directly and synthesize " +
    "your answer from that result. Do NOT call `aurora_expand_query` from delegated context. " +
    `If deeper delegation is required, return to the origin session (${originSessionKey}) ` +
    "and call `aurora_expand_query` there."
  );
}
...
export function recordExpansionDelegationTelemetry(params: {
  deps: Pick<AuroraDependencies, "log">;
  component: string;
  ...
  const line = `[aurora][expansion_delegation] ${JSON.stringify(payload)}`;

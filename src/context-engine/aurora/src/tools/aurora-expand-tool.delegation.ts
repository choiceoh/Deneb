...
function buildDelegatedExpansionTask(params: {
  ...
  return [
    "Run Aurora expansion and report distilled findings.",
    params.query ? `Original query: ${params.query}` : undefined,
    `Pass ${params.pass}`,
    "",
    "Call `aurora_expand` using exactly this JSON payload:",
    ...
    "",
    "Rules:",
    "- In delegated context, use `aurora_expand` directly for retrieval.",
    "- DO NOT call `aurora_expand_query` from this delegated session.",
    "- Keep summary concise and factual.",
    "- Synthesize findings from the `aurora_expand` result before returning.",
...
/**
 * Resolve the requester's active Aurora conversation ID from the session store.
...
export async function resolveRequesterConversationScopeId(params: {
  deps: Pick<AuroraDependencies, "resolveSessionIdFromSessionKey">;
  ...
  try {
    ...
    const conversation = await params.aurora
    ...
    const response = (await params.deps.callGateway({
      ...
      params: {
        ...
        extraSystemPrompt: params.deps.buildSubagentSystemPrompt({
          depth: 1,
          maxDepth: 8,
          taskSummary: "Run aurora_expand and return JSON findings",
        }),
      },
      timeoutMs: 10_000,
    })) as { runId?: string };

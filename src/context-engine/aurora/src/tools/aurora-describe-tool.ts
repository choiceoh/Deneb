...
const AuroraDescribeSchema = Type.Object({
  id: Type.String({
    description: "The Aurora ID to look up. Use sum_xxx for summaries, file_xxx for files.",
  }),
...
export function createAuroraDescribeTool(input: {
  ...
  return {
    ...
    label: "Aurora Describe",
    description:
      "Look up metadata and content for an Aurora item by ID. " +
    ...
    parameters: AuroraDescribeSchema,
    async execute(_toolCallId, params) {
      const retrieval = input.aurora.getRetrieval();
      const timezone = input.aurora.timezone;

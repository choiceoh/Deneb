...
const AuroraExpansionSchema = Type.Object({
...
/**
 * Build a tool definition object for Aurora expansion that can be registered as
 * an agent tool. Follows the pattern used in `src/agents/tools/`.
 *
 * Requires an already-initialised ExpansionOrchestrator and an AuroraConfig
...
export function buildExpansionToolDefinition(options: {
  ...
  return {
    ...
    parameters: AuroraExpansionSchema,
    execute: async (
      _toolCallId: string,

...
export type AuroraExpansionRoutingInput = {
  intent: AuroraExpansionRoutingIntent;
  query?: string;
...
export type AuroraExpansionRoutingDecision = {
  action: AuroraExpansionRoutingAction;
  normalizedMaxDepth: number;
  candidateSummaryCount: number;
  estimatedTokens: number;
  tokenCap: number;
  tokenRiskRatio: number;
  tokenRiskLevel: AuroraExpansionTokenRiskLevel;
  indicators: {
...
export function classifyExpansionTokenRisk(input: { estimatedTokens: number; tokenCap: number }): {
  ratio: number;
  level: AuroraExpansionTokenRiskLevel;
} {
...
/**
 * Decide deterministic route-vs-delegate policy for Aurora expansion orchestration.
 *
 ...
 */
export function decideAuroraExpansionRouting(
  input: AuroraExpansionRoutingInput,
): AuroraExpansionRoutingDecision {
  const normalizedMaxDepth = normalizeDepth(input.requestedMaxDepth);
  const candidateSummaryCount = Math.max(0, Math.trunc(input.candidateSummaryCount));
  const tokenCap = normalizeTokenCap(input.tokenCap);
  ...
  const action: AuroraExpansionRoutingAction = shouldDirect
    ? "answer_directly"
    : shouldDelegate
      ? "delegate_traversal"
      : "expand_shallow";

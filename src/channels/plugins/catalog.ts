...
import type { DenebPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, DenebPackageManifest>>;

const ENV_CATALOG_PATHS = ["DENEB_PLUGIN_CATALOG_PATHS", "DENEB_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<DenebPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  origin?: PluginOrigin;
  workspaceDir?: string;
  packageManifest?: DenebPackageManifest;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.deneb,
    });
    if (entry) {
      entries.push(entry);
    }

...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as DenebConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as DenebConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as DenebConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as DenebConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DenebConfig, channel: "discord" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DenebConfig, channel: "telegram" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as DenebConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as DenebConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as DenebConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as DenebConfig,
        channel: "discord",
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as DenebConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as DenebConfig)).toEqual([]);

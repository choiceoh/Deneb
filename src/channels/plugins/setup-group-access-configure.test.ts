...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: DenebConfig, policy: ChannelAccessPolicy) => DenebConfig;
  resolveAllowlist: (params: { cfg: DenebConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: DenebConfig; resolved: TResolved }) => DenebConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: DenebConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: DenebConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: DenebConfig, policy: ChannelAccessPolicy): DenebConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: DenebConfig, policy: ChannelAccessPolicy): DenebConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: DenebConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: DenebConfig; resolved: string[] }) => {
      calls.push("apply");

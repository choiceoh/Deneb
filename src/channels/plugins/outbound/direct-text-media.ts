import { resolveOutboundMediaUrls } from "deneb/plugin-sdk/reply-payload";
...
export function resolveScopedChannelMediaMaxBytes(params: {
  cfg: DenebConfig;
  accountId?: string | null;
  resolveChannelLimitMb: (params: { cfg: DenebConfig; accountId: string }) => number | undefined;
}): number | undefined {
...
export function createScopedChannelMediaMaxBytesResolver(channel: "imessage" | "signal") {
  return (params: { cfg: DenebConfig; accountId?: string | null }) =>
    resolveScopedChannelMediaMaxBytes({
      cfg: params.cfg,
      accountId: params.accountId,
      resolveChannelLimitMb: ({ cfg, accountId }) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic channel config access

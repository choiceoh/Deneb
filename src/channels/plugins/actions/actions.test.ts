...
function telegramCfg(): DenebConfig {
  return { channels: { telegram: { botToken: "tok" } } } as DenebConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: DenebConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as DenebConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as DenebConfig;
...
function createSignalAccountOverrideCfg(): DenebConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as DenebConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as DenebConfig,
        accountId: undefined,
        params: {

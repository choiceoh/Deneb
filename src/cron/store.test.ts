...
const { makeStorePath } = createCronStoreHarness({ prefix: "deneb-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses DENEB_HOME for tilde expansion", () => {
    vi.stubEnv("DENEB_HOME", "/srv/deneb-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/deneb-home", "cron", "jobs.json"));
  });
});

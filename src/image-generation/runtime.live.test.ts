...
function withPluginsEnabled(cfg: DenebConfig): DenebConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
...
describeLive("image generation live (provider sweep)", () => {
  it("generates images for every configured image-generation variant with available auth", async () => {
    ...
    const providerFilter = parseCsvFilter(process.env.DENEB_LIVE_IMAGE_GENERATION_PROVIDERS);
    const caseFilter = parseCaseFilter(process.env.DENEB_LIVE_IMAGE_GENERATION_CASES);
    const envModelMap = parseProviderModelMap(process.env.DENEB_LIVE_IMAGE_GENERATION_MODELS);

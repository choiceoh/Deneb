...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when DENEB_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".deneb", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when DENEB_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", DENEB_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".deneb-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers DENEB_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        DENEB_PROFILE: "rescue",
        DENEB_STATE_DIR: "C:\\State\\deneb",
      },
      expected: path.join("C:\\State\\deneb", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", DENEB_PROFILE: "default" },
      expected: path.join("/home/test", ".deneb", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\deneb",
          environment: {
            NODE_ENV: "production",
            DENEB_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\deneb\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\deneb\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Deneb Share\\node.exe" "\\\\fileserver\\Deneb Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Deneb Share\\node.exe",
            "\\\\fileserver\\Deneb Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from DENEB_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ DENEB_STATE_DIR: path.join(tmpDir, "custom-state") }),

...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds DENEB_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `deneb gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.DENEB_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.DENEB_GATEWAY_TOKEN?.trim() || undefined;
}
...
function auditGatewayServicePath(
  ...
  const expected = getMinimalServicePathPartsFromEnv({ platform: "linux", env });
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `deneb gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `deneb-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      DENEB_LAUNCHD_LABEL: `ai.deneb.launchd-int-${testId}`,
      DENEB_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

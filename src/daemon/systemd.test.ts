...
const TEST_MANAGED_HOME = "/tmp/deneb-test-home";
const GATEWAY_SERVICE = "deneb-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/deneb gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/deneb", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/deneb-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "deneb-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "deneb-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/deneb-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled deneb-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled deneb-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when DENEB_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/deneb-gateway.service",
    },
    {
      name: "uses profile-specific service name when DENEB_PROFILE is set to a custom value",
      env: { HOME: "/home/test", DENEB_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/deneb-gateway-jbphoenix.service",
    },
    {
      name: "prefers DENEB_SYSTEMD_UNIT over DENEB_PROFILE",
      env: {
        HOME: "/home/test",
        DENEB_PROFILE: "jbphoenix",
        DENEB_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles DENEB_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        DENEB_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from DENEB_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        DENEB_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/deneb gateway start --name "My Bot"')).toEqual([
      "/usr/bin/deneb",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('deneb --name "My \\"Bot\\"" --foo bar', {
...
describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/deneb gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads DENEB_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/deneb gateway run", "EnvironmentFile=%h/.deneb/.env"],
      { [`${TEST_SERVICE_HOME}/.deneb/.env`]: "DENEB_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DENEB_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/deneb gateway run",
        "EnvironmentFile=%h/.deneb/.env",
        'Environment="DENEB_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.deneb/.env`]: "DENEB_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DENEB_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.DENEB_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.deneb/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/deneb-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/deneb gateway run",
          'EnvironmentFile=%h/.deneb/first.env "%h/.deneb/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.deneb/first.env") {
        return "DENEB_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.deneb/second env.env") {
        return 'DENEB_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      DENEB_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "DENEB_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "DENEB_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "DENEB_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DENEB_GATEWAY_TOKEN: "override-token",
      DENEB_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/deneb-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/deneb gateway run",
          "EnvironmentFile=%h/.deneb/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.deneb/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'DENEB_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "DENEB_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DENEB_GATEWAY_TOKEN: "quoted token",
      DENEB_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      DENEB_GATEWAY_TOKEN: "file",
      DENEB_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "deneb-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ DENEB_PROFILE: "work" });
  });

...
describe("usage-format", () => {
  const originalAgentDir = process.env.DENEB_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-usage-format-"));
    process.env.DENEB_AGENT_DIR = agentDir;
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  });
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.DENEB_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over deneb config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to deneb config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {

...
describe("shared/frontmatter", () => {
  ...
  test("resolveDenebManifestBlock reads current manifest keys and custom metadata fields", () => {
    expect(
      resolveDenebManifestBlock({
        frontmatter: {
          metadata: "{ deneb: { foo: 1, bar: 'baz' } }",
        },
      }),
  ...
  test("resolveDenebManifestBlock returns undefined for invalid input", () => {
    ...
    expect(
      resolveDenebManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
  ...
  it("parses and applies install common fields", () => {
    const parsed = parseDenebManifestInstallBase(
      {
        type: " Brew ",
        id: "brew.git",
        label: "Git",
    ...
    expect(parseDenebManifestInstallBase({ kind: "bad" }, ["brew"])).toBeUndefined();
    expect(
      applyDenebManifestInstallCommonFields<{
        extra: boolean;
        id?: string;

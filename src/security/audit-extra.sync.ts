...
function summarizeGroupPolicy(cfg: DenebConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: DenebConfig): boolean {
...
function collectModels(cfg: DenebConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: DenebConfig): Set<string> {
...
function isWebSearchEnabled(cfg: DenebConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: DenebConfig): boolean {
...
function isBrowserEnabled(cfg: DenebConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: DenebConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: DenebConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: DenebConfig): {

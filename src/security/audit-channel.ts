...
function extractAccountConfig(account: unknown): ChannelAccountConfig {
  return (account as { config?: ChannelAccountConfig } | null)?.config ?? {};
}
...
async function collectDiscordFindings(ctx: {
  cfg: DenebConfig;
  account: unknown;
  accountId: string;
  ...
  const discordPathPrefix =
    ctx.orderedAccountIds.length > 1 || ctx.hasExplicitAccountPath
      ? `channels.discord.accounts.${ctx.accountId}`
      : "channels.discord";
  ...
  addDiscordNameBasedEntries({
    ...
    source: "~/.deneb/credentials/discord-allowFrom.json",
    isDiscordMutableAllowEntry,
  });
  ...
  const nativeEnabled = resolveNativeCommandsEnabled({
    ...
    globalSetting: ctx.cfg.commands?.native,
  });
  const nativeSkillsEnabled = resolveNativeSkillsEnabled({
    ...
    globalSetting: ctx.cfg.commands?.nativeSkills,
  });
  const slashEnabled = nativeEnabled || nativeSkillsEnabled;
  if (slashEnabled) {
    const defaultGroupPolicy = ctx.cfg.channels?.defaults?.groupPolicy;
    const groupPolicy =
      (discordCfg.groupPolicy as string | undefined) ?? defaultGroupPolicy ?? "allowlist";
    const guildsConfigured = Object.keys(discordGuildEntries).length > 0;
    const hasAnyUserAllowlist = Object.values(discordGuildEntries).some((guild) => {
...
async function collectSlackFindings(ctx: {
  cfg: DenebConfig;
  account: unknown;
  accountId: string;
}): Promise<SecurityAuditFinding[]> {
  ...
  const legacyAllowFromRaw = (ctx.account as { dm?: { allowFrom?: unknown } } | null | undefined)
    ?.dm?.allowFrom;
...
async function collectTelegramFindings(ctx: {
  cfg: DenebConfig;
  account: unknown;
  accountId: string;
}): Promise<SecurityAuditFinding[]> {
  const findings: SecurityAuditFinding[] = [];
  const allowTextCommands = ctx.cfg.commands?.text !== false;
  if (!allowTextCommands) {
    return findings;
  }
  const telegramCfg = extractAccountConfig(ctx.account);
  const defaultGroupPolicy = ctx.cfg.channels?.defaults?.groupPolicy;
  const groupPolicy =
  ...
  const warnDmPolicy = async (input: {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      findings.push({
        ...
        remediation:
          "Run: " +
          formatCliCommand('deneb config set session.dmScope "per-channel-peer"') +

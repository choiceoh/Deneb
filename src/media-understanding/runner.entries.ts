...
import { requireApiKey, resolveApiKeyForProvider } from "../agents/models/model-auth.js";
...
function extractSherpaOnnxText(raw: string): string | null {
  const tryParse = (value: string): string | null => {
    ...
    try {
      ...
      // JSON parse failed; fall through to line-by-line extraction below.
    }
    return null;
  };
...
async function resolveCliOutput(params: {
  ...
  if (fileOutput && (await fileExists(fileOutput))) {
    try {
      ...
      // Output file exists but is unreadable; fall through to stdout/stderr extraction.
    }
  }

...
function createOpenAiAudioCfg(extra?: Partial<DenebConfig>): DenebConfig {
  return {
    models: {
      providers: {
        openai: {
...
async function runAutoAudioCase(params: {
  transcribeAudio: (req: { model?: string }) => Promise<{ text: string; model: string }>;
  cfgExtra?: Partial<DenebConfig>;
}) {
  let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
  await withAudioFixture("deneb-auto-audio", async ({ ctx, media, cache }) => {
...
describe("runCapability auto audio entries", () => {
  ...
  it("uses mistral when only mistral key is configured", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-audio-agent-"));
    let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          await withAudioFixture("deneb-auto-audio-mistral", async ({ ctx, media, cache }) => {

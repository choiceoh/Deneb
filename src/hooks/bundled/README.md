...
This directory contains hooks that ship with Deneb. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.deneb/deneb.json`:
...
2. Restart gateway: `pkill -9 -f 'deneb.*gateway' && pnpm deneb gateway`
3. Enable the hook: `deneb hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.deneb.ai/automation/hooks

...
homepage: https://docs.deneb.ai/automation/hooks#command-logger
metadata:
  {
    "deneb":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.deneb/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.deneb/logs/commands.log ~/.deneb/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/deneb`:
   ```
   /home/username/.deneb/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

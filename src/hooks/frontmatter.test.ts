...
`;
    ...
    expect(parsed.deneb.emoji).toBe("💾");
    expect(parsed.deneb.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.deneb.emoji).toBe("📝");
    expect(parsed.deneb.events).toEqual(["command"]);
    expect(parsed.deneb.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.deneb.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"deneb": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"deneb": {"events": ["test"]}}');
  });
...
describe("resolveDenebMetadata", () => {
  it("extracts deneb metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDenebMetadata(frontmatter);
  ...
  it("returns undefined when deneb key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        deneb: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Deneb" },
            { id: "npm", kind: "npm", package: "@deneb/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@deneb/hook");
  });
...
`;
    ...
    const deneb = resolveDenebMetadata(frontmatter);
    expect(deneb).toBeDefined();
    expect(deneb?.emoji).toBe("💾");
    expect(deneb?.events).toEqual(["command:new", "command:reset"]);
    expect(deneb?.requires?.config).toEqual(["workspace.dir"]);
    expect(deneb?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const deneb = resolveDenebMetadata(frontmatter);
    expect(deneb?.emoji).toBe("disk");
    expect(deneb?.events).toEqual(["command:new"]);
  });
});

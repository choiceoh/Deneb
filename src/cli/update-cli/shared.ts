...
const DENEB_REPO_URL = "https://github.com/deneb/deneb.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "deneb";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["deneb", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.DENEB_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", DENEB_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `DENEB_GIT_DIR points at a non-git directory: ${params.dir}. Set DENEB_GIT_DIR to an empty folder or an deneb checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`DENEB_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "deneb.mjs");

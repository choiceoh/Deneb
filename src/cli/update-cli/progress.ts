...
const STEP_LABELS: Record<string, string> = {
  ...
  build: "Building",
  "deneb doctor entry": "Checking doctor entrypoint",
  "deneb doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g deneb@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g deneb@latest --omit=optional");
  }

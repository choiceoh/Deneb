...
function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.DENEB_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.DENEB_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.DENEB_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.DENEB_PROFILE);
}
...
`;
      ...
      filename = `deneb-restart-${timestamp}.bat`;

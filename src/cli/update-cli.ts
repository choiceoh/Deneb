...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Deneb and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["deneb update", "Update a source checkout (git)"],
        ["deneb update --channel beta", "Switch to beta channel (git + npm)"],
        ["deneb update --channel dev", "Switch to dev channel (git + npm)"],
        ["deneb update --tag beta", "One-off update to a dist-tag or version"],
        ["deneb update --tag main", "One-off package install from GitHub main"],
        ["deneb update --dry-run", "Preview actions without changing anything"],
        ["deneb update --no-restart", "Update without restarting the service"],
        ["deneb update --json", "Output result as JSON"],
        ["deneb update --yes", "Non-interactive (accept downgrade prompts)"],
        ["deneb update wizard", "Interactive update wizard"],
        ["deneb --update", "Shorthand for deneb update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run deneb update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.deneb.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb update status", "Show channel + version status."],
          ["deneb update status --json", "JSON output."],
          ["deneb update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.deneb.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    DENEB_STATE_DIR: "/tmp/deneb-daemon",
    DENEB_CONFIG_PATH: "/tmp/deneb-daemon/deneb.json",
  },
}));
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/deneb-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DENEB_STATE_DIR = "/tmp/deneb-cli";
    process.env.DENEB_CONFIG_PATH = "/tmp/deneb-cli/deneb.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.DENEB_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.DENEB_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.DENEB_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "deneb-gateway" }],
        hints: [],
      },
      healthy: false,

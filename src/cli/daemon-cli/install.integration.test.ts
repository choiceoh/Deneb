...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("deneb-daemon-install-int-");
    configPath = path.join(tempHome, "deneb.json");
    process.env.HOME = tempHome;
    process.env.DENEB_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.DENEB_GATEWAY_TOKEN).toBeUndefined();
  });
});

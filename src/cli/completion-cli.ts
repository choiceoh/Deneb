...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Deneb Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(deneb completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(deneb completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $DENEB_STATE_DIR/completions (no stdout)",
    )

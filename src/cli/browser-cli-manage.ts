...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "deneb";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "deneb" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (deneb|existing-session). Default: deneb")
    .action(
      async (
        opts: {
          name: string;

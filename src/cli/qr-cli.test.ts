...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("deneb devices approve <requestId>");
  });
  ...
  it("uses DENEB_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("DENEB_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),

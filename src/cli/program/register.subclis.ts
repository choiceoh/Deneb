...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.DENEB_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.DENEB_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration = async (): Promise<DenebConfig | null> => {

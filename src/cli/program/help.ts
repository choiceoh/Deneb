...
const EXAMPLES = [
  ...
  ["deneb gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["deneb --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["deneb gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["deneb gateway ...", "Gateway control via WebSocket."],
  [
    'deneb agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'deneb message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.deneb-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates DENEB_STATE_DIR/DENEB_CONFIG_PATH under ~/.deneb-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.deneb.ai/cli");

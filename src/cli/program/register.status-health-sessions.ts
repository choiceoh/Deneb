...
export function registerStatusHealthSessionsCommands(program: Command) {
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb status", "Show channel health + session summary."],
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.deneb.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.deneb.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb sessions", "List all sessions."],
          ["deneb sessions --agent work", "List sessions for one agent."],
          ["deneb sessions --all-agents", "Aggregate sessions across agents."],
          ["deneb sessions --active 120", "Only last 2 hours."],
          ["deneb sessions --json", "Machine-readable output."],
          ["deneb sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.deneb.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "deneb sessions cleanup --dry-run --fix-missing",

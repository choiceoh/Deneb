...
${formatHelpExamples([
  ['deneb agent --to +15555550123 --message "status update"', "Start a new session."],
  ['deneb agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'deneb agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'deneb agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['deneb agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'deneb agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.deneb.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.deneb.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "deneb agents set-identity --identity-file ~/.deneb/workspace/IDENTITY.md --agent main",

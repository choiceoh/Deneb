...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-bundled-skills-test-"));
    process.env.DENEB_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

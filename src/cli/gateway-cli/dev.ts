...
import { resolveWorkspaceTemplateDir } from "../../agents/workspace/workspace-templates.js";
...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.DENEB_PROFILE?.trim().toLowerCase();
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - Deneb Dev Workspace\n\nDefault dev workspace for deneb gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",

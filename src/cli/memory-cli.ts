    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb memory status", "Show index and provider status."],
          ["deneb memory status --deep", "Probe embedding provider readiness."],
          ["deneb memory index --force", "Force a full reindex."],
          ['deneb memory search "meeting notes"', "Quick search using positional query."],
          [
            'deneb memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["deneb memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.deneb.ai/cli/memory")}\n`,
    );

...
describe("config cli integration", () => {
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-config-cli-int-exec-skip-"));
    const configPath = path.join(tempDir, "deneb.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["DENEB_CONFIG_PATH", "DENEB_TEST_FAST"]);
    try {
      fs.writeFileSync(
        configPath,
        `${JSON.stringify(
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-config-cli-int-exec-allow-"));
    const configPath = path.join(tempDir, "deneb.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["DENEB_CONFIG_PATH", "DENEB_TEST_FAST"]);

...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["deneb security audit", "Run a local security audit."],
          ["deneb security audit --deep", "Include best-effort live Gateway probe checks."],
          ["deneb security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "deneb security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["deneb security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["deneb security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.deneb.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Deneb security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("deneb security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("deneb security audit --fix")}`));
        if (!fixResult) {

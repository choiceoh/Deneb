...
import type { GatewayBindMode, GatewayTailscaleMode, DenebConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.DENEB_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.DENEB_GATEWAY_TOKEN) ??
          "",
      });

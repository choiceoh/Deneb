...
function prefersSips(): boolean {
  return (
    process.env.DENEB_IMAGE_BACKEND === "sips" ||
    (process.env.DENEB_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin")
  );
}
...
function readJpegExifOrientation(buffer: Buffer): number | null {
  return loadNative().readJpegExifOrientation(buffer);
}

async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-img-"));
  try {
    return await fn(dir);
  } finally {

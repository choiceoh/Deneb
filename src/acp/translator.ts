...
function buildSessionPresentation(params: {
  ...
  const configOptions: SessionConfigOption[] = [
    buildSelectConfigOption({
      ...
      description: "Controls how much deliberate reasoning Deneb requests from the Gateway model.",
    ...
    buildSelectConfigOption({
      ...
      name: "Tool verbosity",
      description:
        "Controls how much tool progress and output detail Deneb keeps enabled for the session.",
    ...
    buildSelectConfigOption({
      ...
      name: "Usage detail",
      description:
        "Controls how much usage information Deneb attaches to responses for the session.",
...
export class AcpGatewayAgent implements Agent {
  ...
  private async handleChatEvent(evt: EventFrame): Promise<void> {
    ...
    if (state === "error") {
      // ChatEventSchema now provides errorKind ("timeout" | "rate_limit" |
      // "overloaded" | "server_error") for structured error classification.
      // ACP has no matching stop reasons yet, so all errors resolve as "end_turn"
      // to avoid clients treating transient backend issues as deliberate refusals.
  ...
  private assertSupportedSessionSetup(mcpServers: ReadonlyArray<unknown>): void {
    ...
    throw new Error(
      "ACP bridge mode does not support per-session MCP servers. Configure MCP on the Deneb gateway or agent instead.",
    );
  }

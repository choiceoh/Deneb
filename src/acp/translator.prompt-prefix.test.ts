...
describe("acp prompt cwd prefix", () => {
  ...
  async function runPromptAndCaptureRequest(
    ...
    sessionStore.createSession({
      ...
      cwd: options.cwd ?? path.join(os.homedir(), "deneb-test"),
    });
  ...
  async function runPromptWithCwd(cwd: string) {
    ...
    const previousDenebHome = process.env.DENEB_HOME;
    ...
    try {
      ...
      if (previousDenebHome === undefined) {
        delete process.env.DENEB_HOME;
      } else {
        process.env.DENEB_HOME = previousDenebHome;
      }
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "deneb-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]deneb-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\deneb-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\deneb-test]"),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        systemProvenanceReceipt: expect.stringContaining("bridge=deneb-acp"),
      }),
      { expectFinal: true },
    );
    expect(requestSpy).toHaveBeenCalledWith(

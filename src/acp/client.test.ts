...
const createTempDir = () => tempDirs.make("deneb-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets DENEB_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "deneb",
    });

    expect(env.DENEB_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("deneb");
  });

  it("overrides pre-existing DENEB_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      DENEB_SHELL: "wrong",
    });
    expect(env.DENEB_SHELL).toBe("acp-client");
  ...
  it("preserves DENEB_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        DENEB_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["DENEB_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Deneb bridge", () => {
    ...
    expect(env.DENEB_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.DENEB_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("DENEB_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "deneb", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "deneb", "dist", "entry.js");
    const shimPath = path.join(dir, "deneb.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\deneb\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/deneb-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/deneb-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/deneb-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);

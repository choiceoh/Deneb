...
const APPROVALS_PATH = "~/.deneb/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "deneb-app-server:thread:",
  "deneb-codex-app-server:thread:",
] as const;
...
const pluginBindingGlobalStateKey = Symbol.for("deneb.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Deneb instead.${buildDetachHintSuffix(binding.detachHint)}`;
}

...
function makeTempDir() {
  return makeTrackedTempDir("deneb-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "deneb.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "deneb.plugin.json");
  const linkedManifest = path.join(rootDir, "deneb.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        DENEB_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        DENEB_HOME: undefined,
        DENEB_STATE_DIR: path.join(homeB, ".state"),
      },
    });

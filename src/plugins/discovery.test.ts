...
import { clearPluginDiscoveryCache, discoverDenebPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("deneb-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverDenebPlugins>[0],
) {
  return discoverDenebPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      deneb: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverDenebPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".deneb", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".deneb", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverDenebPlugins({
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@deneb/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@deneb/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@deneb/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@deneb/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("deneb");
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverDenebPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DENEB_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverDenebPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DENEB_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverDenebPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DENEB_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },

...
describe("updateNpmInstalledPlugins", () => {
      ...
      config: {
        plugins: {
          installs: {
            "opik-deneb": {
              source: "npm",
              spec: "@opik/opik-deneb",
              integrity: "sha512-old",
              installPath: "/tmp/opik-deneb",
            },
          },
        },
      },
      pluginIds: ["opik-deneb"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @deneb/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @deneb/missing.",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@deneb/voice-call",
      targetDir: "/tmp/deneb-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@deneb/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@deneb/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@deneb/voice-call");
    expect(result.config.plugins?.entries?.["@deneb/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@deneb/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@deneb/voice-call",
      installPath: "/tmp/deneb-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/deneb-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",

...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-session-suite-"));
});
...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const root = await makeCaseDir("deneb-thread-session-");
  ...
  it("forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const root = await makeCaseDir("deneb-thread-session-existing-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork and creates fresh session when parent tokens exceed threshold", async () => {
    const root = await makeCaseDir("deneb-thread-session-overflow-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("respects session.parentForkMaxTokens override", async () => {
    const root = await makeCaseDir("deneb-thread-session-overflow-override-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await makeCaseDir("deneb-topic-session-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("uses RawBody for command extraction and reset triggers when Body contains wrapped context", async () => {
    const root = await makeCaseDir("deneb-rawbody-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await makeCaseDir("deneb-rawbody-reset-case-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("does not rotate local session state for /new on bound ACP sessions", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-reset-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("does not rotate local session state for ACP /new when conversation IDs are unavailable", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-reset-no-conversation-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps custom reset triggers working on bound ACP sessions", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-custom-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps normal /new behavior for unbound ACP-shaped session keys", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-unbound-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active conversation binding points to a non-ACP session", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-nonacp-binding-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active target session key is non-ACP even with configured ACP binding", async () => {
    const root = await makeCaseDir("deneb-rawbody-acp-configured-fallback-target-");
    const storePath = path.join(root, "sessions.json");
    const channelId = "1478836151241412759";
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await makeCaseDir("deneb-session-store-default-");
    const stateDir = path.join(root, ".deneb");
    ...
    vi.stubEnv("DENEB_STATE_DIR", stateDir);
    try {
      await fs.mkdir(path.dirname(storePath), { recursive: true });
      await writeSessionStoreFast(storePath, {
        [sessionKey]: {
...
describe("initSessionState reset policy", () => {
  ...
  it("defaults to daily reset at 4am local time", async () => {
    ...
    const root = await makeCaseDir("deneb-reset-daily-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    const root = await makeCaseDir("deneb-reset-daily-edge-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    const root = await makeCaseDir("deneb-reset-idle-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("uses per-type overrides for thread sessions", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("deneb-reset-thread-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("detects thread sessions without thread key suffix", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("deneb-reset-thread-nosuffix-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("deneb-reset-type-default-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("deneb-reset-legacy-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    const root = await makeCaseDir("deneb-channel-idle-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState reset triggers in WhatsApp groups", () => {
  ...
  function makeCfg(params: { storePath: string; allowFrom: string[] }): DenebConfig {
    return {
  ...
  it("applies WhatsApp group reset authorization across sender variants", async () => {
    ...
    const storePath = await createStorePath("deneb-group-reset");
    const cases = [
      {
...
describe("initSessionState reset triggers in Slack channels", () => {
  ...
  it("supports mention-prefixed Slack reset commands and preserves args", async () => {
    ...
    const storePath = await createStorePath("deneb-slack-channel-new-");
    await seedSessionStore({
      storePath,
      sessionKey,
      sessionId: existingSessionId,
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("preserves behavior overrides across /new and /reset", async () => {
    const storePath = await createStorePath("deneb-reset-overrides-");
  ...
  it("preserves selected auth profile overrides across /new and /reset", async () => {
    const storePath = await createStorePath("deneb-reset-model-auth-");
  ...
  it("archives the old session store entry on /new", async () => {
    const storePath = await createStorePath("deneb-archive-old-");
  ...
  it("archives the old session transcript on daily/scheduled reset (stale session)", async () => {
    ...
    try {
      ...
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("deneb-stale-archive-");
  ...
  it("idle-based new session does NOT preserve overrides (no entry to read)", async () => {
    const storePath = await createStorePath("deneb-idle-no-preserve-");
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("uses lastCallUsage for totalTokens when provided", async () => {
    const storePath = await createStorePath("deneb-usage-");
  ...
  it("uses lastCallUsage cache counters when available", async () => {
    const storePath = await createStorePath("deneb-usage-cache-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("accumulates estimatedCostUsd across persisted usage updates", async () => {
    const storePath = await createStorePath("deneb-usage-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("persists zero estimatedCostUsd for free priced models", async () => {
    const storePath = await createStorePath("deneb-usage-free-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,

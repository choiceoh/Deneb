...
import { scheduleGatewaySigusr1Restart, triggerDenebRestart } from "../../infra/restart.js";
...
export const handleUsageCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (rawArgs.toLowerCase().startsWith("cost")) {
    return {
      shouldContinue: false,
      reply: { text: "Cost tracking is not available." },
    };
  }
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Deneb in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDenebRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Deneb via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

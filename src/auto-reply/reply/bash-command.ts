...
import {
  ...
  getSession,
  markExited,
} from "../../agents/bash-tools/bash-process-registry.js";
import { createExecTool } from "../../agents/bash-tools/bash-tools.js";
...
function resolveForegroundMs(cfg: DenebConfig): number {
...
export async function handleBashChatCommand(params: {
  ...
  if (!isCommandFlagEnabled(params.cfg, "bash")) {
    return buildDisabledCommandReply({
      label: "bash",
      configKey: "bash",
      docsUrl: "https://docs.deneb.ai/tools/slash-commands#config",
    });
  }

...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop deneb!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@deneb_bot", { botUsername: "deneb_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@deneb_bot", { botUsername: "deneb_bot" })).toBe(true);

...
const workspaceHarness = createCommandWorkspaceHarness("deneb-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".deneb", "extensions", params.pluginId);
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("deneb-command-plugins-home-", async () => {

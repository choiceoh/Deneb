...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-commands-config-"));
  const configPath = path.join(dir, "deneb.json");
  const previous = process.env.DENEB_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.DENEB_CONFIG_PATH = previous;
    }
...
function buildParams(commandBody: string, cfg: DenebConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/deneb-session-store.json" },
    } as DenebConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/deneb-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<DenebConfig>(configPath);

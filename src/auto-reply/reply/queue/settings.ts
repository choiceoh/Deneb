...
import { normalizeQueueMode } from "./normalize.js";
...
export function resolveQueueSettings(params: ResolveQueueSettingsParams): QueueSettings {
  ...
  const capRaw = params.inlineOptions?.cap ?? params.sessionEntry?.queueCap ?? DEFAULT_QUEUE_CAP;
  const dropRaw =
    params.inlineOptions?.dropPolicy ?? params.sessionEntry?.queueDrop ?? DEFAULT_QUEUE_DROP;
  return {
    mode: resolvedMode,

...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): DenebConfig {
    const defaults: NonNullable<NonNullable<DenebConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: DenebConfig) {

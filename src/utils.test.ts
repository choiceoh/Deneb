...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("deneb-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("deneb-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("deneb-lid-a-", async (first) => {
      await withTempDir("deneb-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.deneb when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "deneb-config-dir-"));
    try {
  ...
  it("expands DENEB_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/deneb-home",
      DENEB_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/deneb-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers DENEB_HOME over HOME", () => {
    vi.stubEnv("DENEB_HOME", "/srv/deneb-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/deneb-home"));
...
describe("shortenHomePath", () => {
  it("uses $DENEB_HOME prefix when DENEB_HOME is set", () => {
    vi.stubEnv("DENEB_HOME", "/srv/deneb-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/deneb-home")}/.deneb/deneb.json`)).toBe(
      "$DENEB_HOME/.deneb/deneb.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $DENEB_HOME replacement when DENEB_HOME is set", () => {
    vi.stubEnv("DENEB_HOME", "/srv/deneb-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/deneb-home")}/.deneb/deneb.json`),
    ).toBe("config: $DENEB_HOME/.deneb/deneb.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/deneb", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "deneb"),
    );
  });
  ...
  it("prefers DENEB_HOME for tilde expansion", () => {
    vi.stubEnv("DENEB_HOME", "/srv/deneb-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/deneb")).toBe(path.resolve("/srv/deneb-home", "deneb"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/deneb", env)).toBe(path.resolve("/srv/deneb-home", "deneb"));
  });

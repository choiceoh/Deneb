...
type PromptDefaultModelResult = { model?: string; config?: DenebConfig };
...
function resolveConfiguredModelRaw(cfg: DenebConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: DenebConfig): string[] {
...
export function applyModelAllowlist(cfg: DenebConfig, models: string[]): DenebConfig {

 ...
 * Runs as part of `deneb doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("deneb memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("deneb config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("deneb memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("deneb configure --section model")}`,
        `- To disable: ${formatCliCommand("deneb config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("deneb memory status --deep")}`,
      ].join("\n"),
      "Memory search",

...
import type { DenebConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: DenebConfig): string[] {
...
export async function noteSecurityWarnings(cfg: DenebConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("deneb security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.deneb/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("deneb approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.deneb.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("deneb configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("deneb config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("deneb doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("deneb config set gateway.bind loopback")}`,

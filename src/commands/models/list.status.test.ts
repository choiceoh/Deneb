...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDenebAgentDir: vi.fn().mockReturnValue("/tmp/deneb-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/deneb-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/deneb-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/deneb-dev/deneb.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDenebAgentDir: mocks.resolveDenebAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/deneb-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveDenebAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/deneb-dev/deneb.json");
    expect(payload.auth.storePath).toBe("/tmp/deneb-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/deneb-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/deneb-agent-custom");

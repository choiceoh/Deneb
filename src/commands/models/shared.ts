...
export async function loadValidConfigOrThrow(): Promise<DenebConfig> {
...
export async function updateConfig(
  mutator: (cfg: DenebConfig) => DenebConfig,
): Promise<DenebConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: DenebConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DenebConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("deneb agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

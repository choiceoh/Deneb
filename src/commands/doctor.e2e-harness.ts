...
export const resolveDenebPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadDenebPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveDenebPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DENEB_STATE_DIR;
  originalUpdateInProgress = process.env.DENEB_UPDATE_IN_PROGRESS;
  process.env.DENEB_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.DENEB_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {

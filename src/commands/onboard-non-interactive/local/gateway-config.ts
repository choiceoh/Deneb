...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.DENEB_GATEWAY_TOKEN);
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like DENEB_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }

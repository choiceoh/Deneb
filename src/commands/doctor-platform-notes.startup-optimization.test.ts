...
describe("noteStartupOptimizationHints", () => {
  ...
  it("warns when compile cache is under /tmp and no-respawn is not set", () => {
    ...
    expect(message).toContain("DENEB_NO_RESPAWN is not set to 1");
    expect(message).toContain("export NODE_COMPILE_CACHE=/var/tmp/deneb-compile-cache");
    expect(message).toContain("export DENEB_NO_RESPAWN=1");
  });

...
const SEARCH_TOOL = "https://docs.deneb.ai/mcp.SearchDeneb";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.deneb.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('deneb docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.deneb.ai/");
      runtime.log(`Search: ${formatCliCommand('deneb docs "your query"')}`);
    }
    return;
  }

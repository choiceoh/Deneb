...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "deneb.json");
  ...
  try {
    await withEnvAsync(
      {
        HOME: tempHome,
        DENEB_STATE_DIR: tempHome,
        DENEB_CONFIG_PATH: configPath,
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("deneb-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("deneb-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("deneb-onboard-zai-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("deneb-onboard-zai-cn-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI Coding Plan global endpoint with GLM-5 when available", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("deneb-onboard-zai-coding-global-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-xai-", async (env) => {
  ...
  it("infers Mistral auth choice from --mistral-api-key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("deneb-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("deneb-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "deneb-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
    ...
    {
      name: "openai",
      prefix: "deneb-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
    ...
    {
      name: "openrouter",
      prefix: "deneb-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "deneb-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
    ...
    {
      name: "volcengine",
      prefix: "deneb-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
    ...
    {
      name: "byteplus",
      prefix: "deneb-onboard-ref-flag-byteplus-",
  ...
  it("stores the detected env alias as keyRef for both OpenCode runtime providers", async () => {
    await withOnboardEnv("deneb-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("configures vLLM via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("deneb-onboard-vllm-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "vllm",
  ...
  it("configures SGLang via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("deneb-onboard-sglang-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "sglang",
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "deneb-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "deneb-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers Model Studio auth choice from --modelstudio-api-key and sets default model", async () => {
    await withOnboardEnv("deneb-onboard-modelstudio-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("deneb-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveSetupWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "deneb-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "deneb-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "deneb-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key"; // pragma: allowlist secret
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref", // pragma: allowlist secret
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv("deneb-onboard-custom-provider-ref-flag-", async ({ runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "deneb-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("deneb-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveSetupWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",

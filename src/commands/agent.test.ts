...
import * as modelSelectionModule from "../agents/models/model-selection.js";
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "deneb-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<DenebConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<DenebConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as DenebConfig },
    writeOptions: {},

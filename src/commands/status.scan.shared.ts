...
export function hasExplicitMemorySearchConfig(cfg: DenebConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: DenebConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: DenebConfig, agentId: string) => { store: { path: string } } | null;

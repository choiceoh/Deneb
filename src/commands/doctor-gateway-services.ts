...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service DENEB_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (needsNodeRuntime && !systemNodePath) {
    ...
    note(
      "System Node 22 LTS (22.16+) or Node 24 not found. Install via apt and rerun doctor to migrate off Bun/version managers.",
      "Gateway runtime",
    );
  }
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Deneb gateway next.");
      }
    }
  }

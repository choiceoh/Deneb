...
type EnvSnapshot = {
  HOME?: string;
  DENEB_HOME?: string;
  DENEB_STATE_DIR?: string;
  DENEB_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    DENEB_HOME: process.env.DENEB_HOME,
    DENEB_STATE_DIR: process.env.DENEB_STATE_DIR,
    DENEB_OAUTH_DIR: process.env.DENEB_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: DenebConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: DenebConfig) {
...
async function runStateIntegrityText(cfg: DenebConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.DENEB_HOME = tempHome;
    process.env.DENEB_STATE_DIR = path.join(tempHome, ".deneb");
    delete process.env.DENEB_OAUTH_DIR;
    fs.mkdirSync(process.env.DENEB_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when DENEB_OAUTH_DIR is explicitly configured", async () => {
    process.env.DENEB_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: DenebConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
  ...
  it("prints deneb-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: DenebConfig = {};
    ...
    expect(text).toMatch(/deneb sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/deneb sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /deneb sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );

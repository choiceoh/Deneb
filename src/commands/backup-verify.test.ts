...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-deneb-backup";
...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("deneb-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".deneb");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "deneb.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-deneb-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.deneb",
            archivePath: `${rootName}/payload/posix/tmp/.deneb`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "deneb-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "deneb-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".deneb");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-backup-verify-out-"));
    try {
      process.env.DENEB_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.deneb/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "deneb-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "deneb-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [

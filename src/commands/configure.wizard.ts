    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from deneb.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Deneb update wizard" : "Deneb configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: DenebConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("deneb doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }

...
beforeAll(() => {
  envSnapshot = captureEnv(["DENEB_PROFILE"]);
  process.env.DENEB_PROFILE = "isolated";
});
...
vi.mock("../infra/deneb-root.js", () => ({
  resolveDenebPackageRoot: vi.fn().mockResolvedValue("/tmp/deneb"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/deneb/pnpm-lock.yaml",
      markerPath: "/tmp/deneb/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.deneb.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.deneb.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("deneb status --all") ||
          line.includes("deneb --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("DENEB_GATEWAY_TOKEN", "abcd1234", async () => {

...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByDeneb ? "installed · " : "";
  ...
  runtime.log(theme.heading("Deneb status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`deneb devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("deneb devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("deneb devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("deneb security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("deneb security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.deneb.ai/faq");
  runtime.log("Troubleshooting: https://docs.deneb.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("deneb status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("deneb logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("deneb status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("deneb gateway probe")}`);
  }
}

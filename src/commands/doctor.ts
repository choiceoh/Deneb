...
function resolveMode(cfg: DenebConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: DenebConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("deneb configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("deneb config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("deneb setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("deneb doctor --fix")}" to apply changes.`);
  }

...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Deneb", ".deneb");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/deneb-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".deneb");
  ...
  it("anchors cloud detection to OS homedir when DENEB_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".deneb");
    const originalDenebHome = process.env.DENEB_HOME;
    process.env.DENEB_HOME = "/tmp/deneb-home-override";
    ...
    try {
      ...
      if (originalDenebHome === undefined) {
        delete process.env.DENEB_HOME;
      } else {
        process.env.DENEB_HOME = originalDenebHome;
      }
    }
  });

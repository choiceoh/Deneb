...
import { DenebSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: DenebConfig): {
  config: DenebConfig;
  removed: string[];
} {
  const parsed = DenebSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: DenebConfig): void {

...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under DENEB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("deneb-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-oauth-sync-"));
    ...
    process.env.DENEB_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-oauth-nosync-"));
    process.env.DENEB_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside DENEB_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-oauth-external-"));
    process.env.DENEB_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* DENEB_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to DENEB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("deneb-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

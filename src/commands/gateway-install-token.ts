...
import { readConfigFileSnapshot, writeConfigFile, type DenebConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("deneb config set gateway.auth.mode token")} or ${formatCliCommand("deneb config set gateway.auth.mode password")}.`,
  ].join(" ");
}

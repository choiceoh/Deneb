...
function resolveAgentName(cfg: DenebConfig, agentId: string) {
...
function resolveAgentModel(cfg: DenebConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DenebConfig): AgentSummary[] {

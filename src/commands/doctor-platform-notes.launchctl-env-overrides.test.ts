...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "DENEB_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("DENEB_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv DENEB_GATEWAY_TOKEN");
    expect(message).not.toContain("DENEB_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "DENEB_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
          password: { source: "env", provider: "default", id: "DENEB_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        providers: {
    ...
    expect(message).toContain("DENEB_GATEWAY_PASSWORD");
  });

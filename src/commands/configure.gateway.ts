...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "DENEB_GATEWAY_TOKEN",
          placeholder: "DENEB_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like DENEB_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Deneb will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Deneb trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.deneb.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

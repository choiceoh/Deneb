...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "deneb", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "deneb", version: "2.3.4" });
  ...
  it("ignores non-deneb package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers DENEB_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        DENEB_VERSION: "9.9.9",
        DENEB_SERVICE_VERSION: "2.2.2",

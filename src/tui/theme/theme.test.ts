...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      DENEB_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when DENEB_THEME=light", async () => {
    const mod = await importThemeWithEnv({ DENEB_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when DENEB_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ DENEB_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats DENEB_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ DENEB_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("DENEB_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      DENEB_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ DENEB_THEME: "light" });
    const darkMod = await importThemeWithEnv({ DENEB_THEME: "dark" });

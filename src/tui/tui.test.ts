...
describe("resolveInitialTuiAgentId", () => {
  const cfg: DenebConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/deneb" },
        { id: "ops", workspace: "/tmp/deneb/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/deneb/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run deneb devices list");
    expect(state.pairingHint).toContain("deneb devices list");
  });

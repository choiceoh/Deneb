...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("deneb-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.DENEB_STATE_DIR).toBe(path.join(tempHome.home, ".deneb"));
    ...
    expect(process.env.DENEB_STATE_DIR).toBe(previousStateDir);

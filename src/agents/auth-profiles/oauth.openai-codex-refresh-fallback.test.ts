...
describe("resolveApiKeyForProfile openai-codex refresh fallback", () => {
  const envSnapshot = captureEnv(["DENEB_STATE_DIR", "DENEB_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tempRoot = "";
  let agentDir = "";
  ...
  beforeEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-codex-refresh-fallback-"));
    agentDir = path.join(tempRoot, "agents", "main", "agent");
    await fs.mkdir(agentDir, { recursive: true });
    process.env.DENEB_STATE_DIR = tempRoot;

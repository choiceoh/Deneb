...
function resolveAttachmentLimits(config: DenebConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".deneb", "attachments");
  const relDir = path.posix.join(".deneb", "attachments", attachmentId);

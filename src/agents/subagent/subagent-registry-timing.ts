...
export function findSessionEntryByKey(store: Record<string, SessionEntry>, sessionKey: string) {
...
/**
 * Reconcile a single orphaned subagent run. Returns `true` if the run was
 * modified or removed.
 *
 * Callers must pass the module-level `subagentRuns` and `resumedRuns`
 * collections so this helper stays free of module-level mutable state.
 */
export function reconcileOrphanedRun(params: {
  ...
  const removed = params.subagentRuns.delete(params.runId);
...
/**
 * Reconcile all orphaned runs after restoring from disk.
 *
 * Callers must pass the module-level `subagentRuns` and `resumedRuns`
 * collections so this helper stays free of module-level mutable state.
 */
export function reconcileOrphanedRestoredRuns(params: {
  ...
  let changed = false;
  for (const [runId, entry] of params.subagentRuns.entries()) {
...
export function resolveSubagentSessionStartedAt(

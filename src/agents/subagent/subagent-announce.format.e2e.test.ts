...
let configOverride: DenebConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: DenebConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/deneb/deneb/issues/31298
    previousFastTestEnv = process.env.DENEB_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.DENEB_TEST_FAST;
      return;
    }
    process.env.DENEB_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // DENEB_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Deneb runtime context (internal):");

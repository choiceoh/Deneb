...
const FAST_TEST_MODE = process.env.DENEB_TEST_FAST === "1";
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`subagents` applies to Deneb sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`deneb ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for Deneb subagents (`runtime: "subagent"`).',

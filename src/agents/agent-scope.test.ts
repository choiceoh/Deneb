...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DenebConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: DenebConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: DenebConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: DenebConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DenebConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: DenebConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/deneb");
  });

  it("uses DENEB_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "deneb-home");
    vi.stubEnv("DENEB_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as DenebConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".deneb", "workspace"));
  });

  it("uses DENEB_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "deneb-home");
    vi.stubEnv("DENEB_HOME", home);
    // Clear state dir so it falls back to DENEB_HOME
    vi.stubEnv("DENEB_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as DenebConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".deneb", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/deneb-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/deneb-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-agent-scope-"));

...
function redactSessionsSpawnAttachmentsArgs(value: unknown): unknown {
  ...
  const next = raw.map((item) => {
    ...
    return { ...rest, content: "__DENEB_REDACTED__" };
  });
...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[deneb] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolCallInputs(
  ...
  try {
    ...
    // Defensive: if repair itself fails on malformed data, return original messages
    // so the session continues rather than crashing.
    return { messages, droppedToolCalls: 0, droppedAssistantMessages: 0 };
  }
}
...
export function repairToolUseResultPairing(
  ...
  try {
    ...
    // Defensive: if repair itself fails on malformed transcript data, return
    // original messages so the session continues rather than crashing.
    return {
      messages,
      added: [],

...
const DIRECT_ENABLED = Boolean(process.env.DENEB_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.DENEB_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress("[live-models] skipping (set DENEB_LIVE_MODELS=modern|all|<list>; all=modern)");
        return;
      }
      ...
      const rawModels = process.env.DENEB_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.DENEB_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.DENEB_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.DENEB_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via DENEB_LIVE_MAX_MODELS=${maxModels}`,
        );
      }

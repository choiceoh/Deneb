...
const GOOGLE_LIVE_SINGLE_KEY = "DENEB_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "DENEB_LIVE_ANTHROPIC_KEY",
    listVar: "DENEB_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "DENEB_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `DENEB_LIVE_${base}_KEY`;

...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    deneb: process.env.DENEB_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.deneb === undefined) {
      delete process.env.DENEB_GATEWAY_TOKEN;
    } else {
      process.env.DENEB_GATEWAY_TOKEN = envSnapshot.deneb;
    }
  ...
  it("uses DENEB_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.DENEB_GATEWAY_TOKEN = "local-env-token";

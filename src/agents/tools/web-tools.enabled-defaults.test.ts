...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "deneb" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [{ title: "Deneb", url: "https://deneb.ai/docs", content: "docs" }],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest deneb release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://deneb.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://deneb.ai/docs"]);

...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses DENEB_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      DENEB_HOME: "/srv/deneb-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/deneb-home"), ".deneb", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".deneb", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("deneb-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("deneb-workspace-");
    await fs.mkdir(path.join(tempDir, ".deneb"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-workspace-hardlink-"));
    try {

...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const denebTmpDir = path.resolve(resolvePreferredDenebTmpDir());
  if (!isPathInside(denebTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: denebTmpDir });
  return resolved;
}

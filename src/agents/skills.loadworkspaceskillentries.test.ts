...
async function createTempWorkspaceDir() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-"));
...
async function setupWorkspaceWithProsePlugin() {
  ...
  const pluginRoot = path.join(workspaceDir, ".deneb", "extensions", "open-prose");
...
async function setupWorkspaceWithDiffsPlugin() {
  ...
  const managedDir = path.join(workspaceDir, ".managed");
  const bundledDir = path.join(workspaceDir, ".bundled");
  const pluginRoot = path.join(workspaceDir, ".deneb", "extensions", "diffs");

...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["DENEB_STATE_DIR"]);
  ...
  it("uses isolated temp state when DENEB_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "deneb-test-state"));
  });
});

...
const originalBundledDir = process.env.DENEB_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("deneb-bundled-");
  const workspaceDir = await createTempDir("deneb-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: DenebConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.DENEB_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.DENEB_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(

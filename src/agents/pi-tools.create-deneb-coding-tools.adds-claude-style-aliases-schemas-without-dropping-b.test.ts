...
const defaultTools = createDenebCodingTools({ senderIsOwner: true });
...
describe("createDenebCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: DenebConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createDenebCodingTools({
    ...
    const anthropicTools = createDenebCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: DenebConfig = {
      tools: {
        exec: {
    ...
    const allowed = createDenebCodingTools({
    ...
    const denied = createDenebCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createDenebCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.DENEB_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".deneb", `workspace-${profile}`);
  }
  return path.join(home, ".deneb", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".deneb";

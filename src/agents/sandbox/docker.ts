...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "deneb.sandbox=1");
  args.push("--label", `deneb.sessionKey=${params.scopeKey}`);
  args.push("--label", `deneb.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `deneb.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markDenebExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "deneb.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`deneb sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`deneb sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("deneb sandbox recreate --all");
}

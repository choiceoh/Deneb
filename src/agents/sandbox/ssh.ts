...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "deneb-sandbox-ssh-"));
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredDenebTmpDir() ?? os.tmpdir());
}

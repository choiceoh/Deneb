    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "deneb.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "deneb.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "deneb.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("deneb-");
  const acpxRoot = await tempDirs.make("deneb-acpx-plugin-");
  const helperRoot = await tempDirs.make("deneb-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("deneb-");
  const pluginRoot = await tempDirs.make("deneb-plugin-");
  const outsideDir = await tempDirs.make("deneb-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("deneb-");
    const pluginRoot = await tempDirs.make("deneb-claude-bundle-");

...
async function resolveContext(config: DenebConfig, sessionKey: string, workspaceDir: string) {
...
function createDefaultsSandboxConfig(scope: "agent" | "shared" | "session" = "agent"): DenebConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): DenebConfig {
  return {
    agents: {
      defaults: {
        sandbox: {

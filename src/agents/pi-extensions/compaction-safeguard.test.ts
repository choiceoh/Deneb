...
vi.mock("../compaction.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    summarizeWithFallback: vi.fn(actual.summarizeWithFallback),
  };
});

const mockSummarizeWithFallback = vi.mocked(compactionModule.summarizeWithFallback);
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("keeps required headings when all turns are preserved and history is carried forward", async () => {
    ...
    expect(result.cancel).not.toBe(true);
    expect(mockSummarizeWithFallback).not.toHaveBeenCalled();
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-compaction-summary-"));

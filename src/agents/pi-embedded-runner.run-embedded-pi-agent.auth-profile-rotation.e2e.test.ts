...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): DenebConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): DenebConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): DenebConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverride({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `deneb-auth-rotation-${Date.now()}.log`),
    });

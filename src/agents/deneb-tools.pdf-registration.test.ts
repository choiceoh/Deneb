...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-tools-pdf-"));
  try {
...
describe("createDenebTools PDF registration", () => {
  it("includes pdf tool when pdfModel is configured", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const tools = createDenebTools({ config: cfg, agentDir });

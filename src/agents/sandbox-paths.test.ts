...
import { resolvePreferredDenebTmpDir } from "../infra/tmp-deneb-dir.js";
...
async function withOutsideHardlinkInDenebTmp<T>(
  params: {
    denebTmpDir: string;
  ...
  const hardlinkPath = path.join(params.denebTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.denebTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.denebTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.denebTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const denebTmpDir = resolvePreferredDenebTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Deneb tmp root",
      media: path.join(denebTmpDir, "image.png"),
      expected: path.join(denebTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Deneb tmp root",
      media: pathToFileURL(path.join(denebTmpDir, "photo.png")).href,
      expected: path.join(denebTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Deneb tmp root",
      media: path.join(denebTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(denebTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(denebTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside deneb tmp root",
      media: path.join(os.tmpdir(), "outside-deneb", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Deneb tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(denebTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(denebTmpDir, { recursive: true });
      const symlinkPath = path.join(denebTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Deneb tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDenebTmp(
      {
        denebTmpDir,
  ...
  it("rejects symlinked Deneb tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDenebTmp(

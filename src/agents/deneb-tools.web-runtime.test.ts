...
function findTool(name: string, config: DenebConfig) {
  const allTools = createDenebTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: DenebConfig; env?: NodeJS.ProcessEnv }) {
...
describe("deneb tools runtime web metadata wiring", () => {

...
function createExecTool(workspaceDir: string) {
  const tools = createDenebCodingTools({
    workspaceDir,
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("deneb-ws-", async (workspaceDir) => {
      await withTempDir("deneb-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createDenebCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello deneb");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("deneb-ws-", async (workspaceDir) => {
      await withTempDir("deneb-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("deneb-ws-", async (workspaceDir) => {
      const cfg: DenebConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createDenebCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-deneb.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("deneb-sandbox-", async (sandboxDir) => {
      await withTempDir("deneb-workspace-", async (workspaceDir) => {
        ...
        const tools = createDenebCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);

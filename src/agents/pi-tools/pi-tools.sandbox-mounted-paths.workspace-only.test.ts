...
type CodingToolsInput = NonNullable<Parameters<typeof createDenebCodingTools>[0]>;
...
function resolveApplyPatchTool(
  params: Pick<CodingToolsInput, "sandbox" | "workspaceDir"> & { config: DenebConfig },
): ToolWithExecute {
...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const tools = createDenebCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as DenebConfig;
      const tools = createDenebCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });

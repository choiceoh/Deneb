...
import { loadConfig, type DenebConfig, writeConfigFile } from "../config/config.js";
...
function createOpenAiFileModelsConfig(): NonNullable<DenebConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("resolves env refs for config and auth profiles", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/deneb-agent-main"],
      loadAuthStore: () =>
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: DenebConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    ...
    const store = ensureAuthProfileStore("/tmp/deneb-agent-main");
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("deneb-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("deneb-secrets-runtime-refresh-fail-", async (home) => {
      const configDir = path.join(home, ".deneb");
      const secretFile = path.join(configDir, "secrets.json");
      const agentDir = path.join(configDir, "agents", "main", "agent");
      const authStorePath = path.join(agentDir, "auth-profiles.json");
  ...
  it("keeps last-known-good web runtime snapshot when reload introduces unresolved active web refs", async () => {
    await withTempHome("deneb-secrets-runtime-web-reload-lkg-", async (home) => {
      const prepared = await prepareSecretsRuntimeSnapshot({
        config: asConfig({
          tools: {
            web: {
      ...
      const persistedConfig = JSON.parse(
        await fs.readFile(path.join(home, ".deneb", "deneb.json"), "utf8"),
      ) as DenebConfig;
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("deneb-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".deneb", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".deneb", "agents", "ops", "agent");
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-secrets-runtime-"));
    ...
    const prevStateDir = process.env.DENEB_STATE_DIR;
    ...
    try {
      ...
      process.env.DENEB_STATE_DIR = stateDir;
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.DENEB_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });

...
const DENEB_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of DENEB_COMPILED_SECRET_TARGETS) {
...
const DENEB_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargets(config: DenebConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: DENEB_COMPILED_SECRET_TARGETS,
    entriesById: DENEB_TARGETS_BY_ID,
  });

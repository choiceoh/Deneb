...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredDenebTmpDir: vi.fn(() => "/tmp/deneb"),
}));
vi.mock("../infra/tmp-deneb-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredDenebTmpDir.mockReturnValue("/tmp/deneb");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".deneb-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredDenebTmpDir.mockReturnValue("/tmp/deneb-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "deneb-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "deneb-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredDenebTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "deneb-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "deneb-preferred", "downloads")}${path.sep}`),
    );
  });

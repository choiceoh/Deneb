import type { BrowserConfig, BrowserProfileConfig, DenebConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_DENEB_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "deneb" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_DENEB_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_DENEB_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: DenebConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_DENEB_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_DENEB_BROWSER_PROFILE_NAME]
        ? DEFAULT_DENEB_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "deneb";

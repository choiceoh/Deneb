...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.DENEB_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "deneb",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/deneb" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    deneb: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchDenebChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.DENEB_GATEWAY_PORT;
  process.env.DENEB_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.DENEB_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.DENEB_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.DENEB_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.DENEB_GATEWAY_PASSWORD;
  } else {
    process.env.DENEB_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".deneb-profile-decorated");
}
 ...
 */
export function decorateDenebProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_DENEB_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_DENEB_BROWSER_COLOR).toUpperCase();

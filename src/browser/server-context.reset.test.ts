...
function localDenebProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "deneb",
...
function createLocalDenebResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localDenebProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveDenebUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-reset-"));
    const profileDir = path.join(tempRoot, "deneb");
    ...
    const ops = createLocalDenebResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveDenebUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "deneb-reset-no-own-"));
    const profileDir = path.join(tempRoot, "deneb");
    fs.mkdirSync(profileDir, { recursive: true });

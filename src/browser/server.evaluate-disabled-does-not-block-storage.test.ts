  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          deneb: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.DENEB_GATEWAY_PORT;
    process.env.DENEB_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.DENEB_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.DENEB_GATEWAY_PASSWORD;

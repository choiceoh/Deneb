...
vi.mock("./chrome.js", () => ({
  resolveDenebUserDataDir: vi.fn(() => "/tmp/deneb-test/deneb/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "deneb-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveDenebUserDataDir).mockReturnValue(userDataDir);

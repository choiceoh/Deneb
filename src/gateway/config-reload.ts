...
import type { DenebConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function diffConfigPaths(prev: unknown, next: unknown, prefix = ""): string[] {
  ...
  if (Array.isArray(prev) && Array.isArray(next)) {
    // Arrays can contain object entries (for example memory.vega.paths/scope.rules);
...
export function resolveGatewayReloadSettings(cfg: DenebConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: DenebConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: DenebConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: DenebConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: DenebConfig) => {

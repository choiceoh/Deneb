...
describe("resolveGatewayRuntimeConfig", () => {
  describe("trusted-proxy auth mode", () => {
    // Validates that runtime config allows lan binding when authMode === "trusted-proxy"
    it.each([
      {
        name: "lan binding",
        cfg: {
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.DENEB_GATEWAY_TOKEN;
      delete process.env.DENEB_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.DENEB_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.DENEB_GATEWAY_TOKEN;
      }
    });

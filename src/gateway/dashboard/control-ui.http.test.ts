...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-ui-"));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-ui-root-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/deneb${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/deneb",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/deneb");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/deneb/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-avatar-http-outside-"));
    try {
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-ui-index-outside-"));
        try {
  ...
  it("rejects hardlinked index.html for non-package control-ui roots", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-ui-index-hardlink-"));
        try {
          const outsideIndex = path.join(outsideDir, "index.html");
  ...
  it("does not handle POST to root-mounted paths (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const webhookPath of ["/telegram-webhook", "/custom-webhook", "/callback"]) {
  ...
  it("does not handle POST to paths outside basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: "/telegram-webhook", method: "POST" } as IncomingMessage,
          res,
          { basePath: "/deneb", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/deneb", "/deneb/", "/deneb/some-page"]) {
          const { handled, end } = runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,

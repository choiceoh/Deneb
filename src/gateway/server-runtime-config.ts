...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  const canvasHostEnabled =
    process.env.DENEB_SKIP_CANVAS_HOST !== "1" && params.cfg.canvasHost?.enabled !== false;
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set DENEB_GATEWAY_TOKEN/DENEB_GATEWAY_PASSWORD)`,
    );
  }
  if (
    controlUiEnabled &&

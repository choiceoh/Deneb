...
import type { ModelsConfig, DenebConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.DENEB_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.DENEB_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.DENEB_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.DENEB_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.DENEB_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<DenebConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveDenebAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-live-state-"));
  ...
  process.env.DENEB_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.deneb-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDenebAgentDir();
  const sanitizedCfg: DenebConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-live-"));
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              message: strictReply
                ? "Deneb live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureDenebModelsJson(cfg);
      ...
      const rawModels = process.env.DENEB_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via DENEB_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.deneb-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

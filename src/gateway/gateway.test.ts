...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "deneb");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = nextGatewayId("nonce-a");
      const nonceB = nextGatewayId("nonce-b");
      const toolProbePath = path.join(workspaceDir, `.deneb-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".deneb");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-wizard-home-"));
      process.env.HOME = tempHome;

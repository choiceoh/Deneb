...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gw-"));
    try {
  ...
  test("sessions.send forwards dashboard messages into existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.steer forwards dashboard messages into existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gw-"));

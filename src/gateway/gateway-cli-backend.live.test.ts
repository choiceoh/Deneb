...
const CLI_LIVE = isTruthyEnvValue(process.env.DENEB_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.DENEB_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.DENEB_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("DENEB_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const previous = {
      configPath: process.env.DENEB_CONFIG_PATH,
      token: process.env.DENEB_GATEWAY_TOKEN,
    ...
    const rawModel = process.env.DENEB_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `DENEB_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.DENEB_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(`DENEB_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`);
    }
    const baseCliArgs =
      parseJsonStringArray(
        "DENEB_LIVE_CLI_BACKEND_ARGS",
        process.env.DENEB_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`DENEB_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "DENEB_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.DENEB_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.DENEB_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.DENEB_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "DENEB_LIVE_CLI_BACKEND_IMAGE_MODE requires DENEB_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-live-cli-"));
    const disableMcpConfig = process.env.DENEB_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "deneb.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.DENEB_CONFIG_PATH = tempConfigPath;
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        ...
        process.env.DENEB_CONFIG_PATH = previous.configPath;
      }
      if (previous.token === undefined) {
        delete process.env.DENEB_GATEWAY_TOKEN;
      } else {
        process.env.DENEB_GATEWAY_TOKEN = previous.token;
      }
      if (previous.skipChannels === undefined) {
        delete process.env.DENEB_SKIP_CHANNELS;
      } else {
        process.env.DENEB_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.DENEB_SKIP_GMAIL_WATCHER;
      } else {

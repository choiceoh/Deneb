...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.DENEB_SKIP_CHANNELS;
    prevSkipGmail = process.env.DENEB_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.DENEB_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.DENEB_SKIP_CHANNELS = "0";
    delete process.env.DENEB_SKIP_GMAIL_WATCHER;
    delete process.env.DENEB_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.DENEB_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.DENEB_SKIP_GMAIL_WATCHER;
    } else {
      process.env.DENEB_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.DENEB_SKIP_PROVIDERS;
    } else {
      process.env.DENEB_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("DENEB_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.DENEB_STATE_DIR;
    if (!stateDir) {
      throw new Error("DENEB_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_DENEB_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_DENEB_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_DENEB_AUTH_REF" is missing or empty.',
      );
    } finally {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.DENEB_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.DENEB_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
      envSnapshot.restore();
      ws.close();
      await server.close();

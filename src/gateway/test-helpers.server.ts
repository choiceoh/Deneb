...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.DENEB_TEST_MINIMAL_GATEWAY = "1";
  process.env.DENEB_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "deneb-test-no-bundled-extensions")
    : "deneb-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".deneb-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".deneb-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__denebTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__denebTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["DENEB_GATEWAY_TOKEN"]);
  const prev = process.env.DENEB_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.DENEB_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.DENEB_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

...
(() => {
  ...
  window.addEventListener("deneb:a2ui-action-status", onStatus);
  ...
  function send(name, sourceComponentId) {
    ...
    const sendUserAction =
      typeof window.denebSendUserAction === "function"
        ? window.denebSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DENEB_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.DENEB_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Deneb Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
      ...
      const mime =
        lower.endsWith(".html") || lower.endsWith(".htm")
          ? "text/html"
          : (detectMime({ filePath: realPath }) ?? "application/octet-stream");

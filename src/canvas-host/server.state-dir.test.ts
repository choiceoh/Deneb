...
describe("canvas host state dir defaults", () => {
  it("uses DENEB_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("deneb-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Deneb Canvas");
      } finally {
        await handler.close();
      }
    });

...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __denebDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__denebDiagnosticEventsState) {
    globalStore.__denebDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__denebDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: DenebConfig): boolean {

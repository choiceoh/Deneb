...
vi.mock("./deneb-root.js", () => ({
  resolveDenebPackageRoot: vi.fn(async () => null),
  resolveDenebPackageRootSync: vi.fn(() => null),
}));
...
let denebRoot: typeof import("./deneb-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    denebRoot = await import("./deneb-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/deneb");
    const wrapperArgv1 = abs("fixtures/bin/deneb");
  ...
  it("uses resolveDenebPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/deneb");
    (
      denebRoot.resolveDenebPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/deneb"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "deneb" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "deneb.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-deneb");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/deneb-bundle");
    (
      denebRoot.resolveDenebPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/deneb-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (

...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "deneb.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Deneb)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "deneb.internal", expected: "deneb.internal." },
    { value: "deneb.internal.", expected: "deneb.internal." },
    { value: "  deneb.internal  ", expected: "deneb.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { DENEB_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { DENEB_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("deneb.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "deneb.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/deneb",
    });

    expect(txt).toContain(`$ORIGIN deneb.internal.`);
    ...
    expect(txt).toContain(`_deneb-gw._tcp IN PTR studio-london._deneb-gw._tcp`);
    expect(txt).toContain(`studio-london._deneb-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Deneb)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/deneb`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN deneb.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._deneb-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Deneb)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("deneb.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("deneb.internal."),

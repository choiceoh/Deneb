...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "deneb-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["DENEB_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.DENEB_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveDenebPackageRoot } = await import("./deneb-root.js"));
    ...
    vi.mocked(resolveDenebPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveDenebPackageRoot).mockResolvedValue("/opt/deneb");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/deneb/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });

...
describe("shell env fallback", () => {
  ...
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ DENEB_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ DENEB_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("uses the same truthy env parsing for deferred fallback", () => {
    ...
    expect(shouldDeferShellEnvFallback({ DENEB_DEFER_SHELL_ENV_FALLBACK: "false" })).toBe(false);
    expect(shouldDeferShellEnvFallback({ DENEB_DEFER_SHELL_ENV_FALLBACK: "yes" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ DENEB_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        DENEB_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
  });

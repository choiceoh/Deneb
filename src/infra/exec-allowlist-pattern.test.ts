...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevDenebHome = process.env.DENEB_HOME;
    const prevHome = process.env.HOME;
    process.env.DENEB_HOME = "/srv/deneb-home";
    process.env.HOME = "/home/other";
    const denebHome = path.join(path.resolve("/srv/deneb-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", denebHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevDenebHome === undefined) {
        delete process.env.DENEB_HOME;
      } else {
        process.env.DENEB_HOME = prevDenebHome;
      }

...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/deneb-state", "devices")).toEqual({
      dir: path.join("/tmp/deneb-state", "devices"),
      pendingPath: path.join("/tmp/deneb-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/deneb-state", "devices", "paired.json"),
    });
  });

...
function createDenebBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "deneb-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createDenebBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses deneb-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "deneb-gateway" },
          { pid: process.pid, cmd: "deneb-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'deneb'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple deneb pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "deneb-gateway" },
          { pid: pid2, cmd: "deneb-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-deneb pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks deneb processes — non-deneb listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty deneb stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty deneb stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + deneb pid in stdout — container-restricted lsof reports partial results
          return createDenebBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "deneb-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'deneb'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("deneb")
      // inside the mid-loop flush: a non-deneb cmd between two entries must not
      // be pushed, but the following deneb entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-deneb entry first, then deneb entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid deneb entry must still be found.
      const stdout = `p0\ncdeneb-gateway\np${stalePid}\ncdeneb-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero deneb pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-deneb stdout (line 145)", () => {
    it("treats status 1 + non-deneb stdout as port-free (not an deneb process)", () => {
      // status 1 + non-empty stdout where no deneb pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill deneb pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-deneb output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no deneb pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();

...
function candidateBinDirs(opts: EnsureDenebPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `deneb` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "deneb");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.DENEB_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "deneb"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `deneb` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDenebCliOnPath(opts: EnsureDenebPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DENEB_PATH_BOOTSTRAPPED)) {
    return;
  }

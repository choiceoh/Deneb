 ...
 * returning control to the caller (typically `triggerDenebRestart` →
...
/**
 * Parse deneb gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("deneb")) {
 ...
 * Returns only PIDs that belong to deneb gateway processes (not the current process).
 */

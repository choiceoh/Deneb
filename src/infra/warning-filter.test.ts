...
const warningFilterKey = Symbol.for("deneb.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "DENEB_TEST_WARNING" });
      emitWarning(
        Object.assign(new Error("The punycode module is deprecated."), {
          name: "DeprecationWarning",
          code: "DEP0040",
        }),
        { type: "Warning", code: "DENEB_VISIBLE_OVERRIDE" },
      );
      await flushWarnings();
      expect(seenWarnings.find((warning) => warning.code === "DENEB_TEST_WARNING")).toBeDefined();
      expect(

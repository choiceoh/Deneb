...
describe("resolveOutboundSessionRoute", () => {
  ...
  it("resolves provider-specific session routes", async () => {
    const perChannelPeerCfg = { session: { dmScope: "per-channel-peer" } } as DenebConfig;
    ...
    const cases: Array<{
      ...
      {
        name: "Mattermost channel target",
        cfg: baseConfig,
        channel: "mattermost",
        target: "channel:townhall123",
        expected: {
          sessionKey: "agent:main:mattermost:channel:townhall123",
          from: "mattermost:channel:townhall123",
          to: "channel:townhall123",
          chatType: "channel",
        },
      },
      {
  ...
  it("uses resolved Discord user targets to route bare numeric ids as DMs", async () => {
    const route = await resolveOutboundSessionRoute({
      cfg: { session: { dmScope: "per-channel-peer" } } as DenebConfig,
      channel: "discord",
      agentId: "main",
      target: "123",
      resolvedTarget: {

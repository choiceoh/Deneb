...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Deneb tmp root", async () => {
      const tmpRoot = resolvePreferredDenebTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideDeneb = path.join(os.tmpdir(), "outside-deneb", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",

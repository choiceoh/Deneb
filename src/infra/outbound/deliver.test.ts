...
import { resolvePreferredDenebTmpDir } from "../tmp-deneb-dir.js";
...
const telegramChunkConfig: DenebConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: DenebConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes Deneb tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredDenebTmpDir()]),
      }),
    );
  });

  it("includes Deneb tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes Deneb tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes Deneb tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: DenebConfig = { channels: { signal: { mediaMaxMb: 2 } } };

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DenebConfig | null = null;
  ...
  get(key: string, cfg: DenebConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: DenebConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: DenebConfig): void {

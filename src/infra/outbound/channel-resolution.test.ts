...
const loadDenebPluginsMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../plugins/loader.js", () => ({
  loadDenebPlugins: (...args: unknown[]) => loadDenebPluginsMock(...args),
}));
...
describe("outbound channel resolution", () => {
  beforeEach(() => {
    ...
    loadDenebPluginsMock.mockReset();
  ...
  it("returns the already-registered plugin without bootstrapping", async () => {
    ...
    expect(loadDenebPluginsMock).not.toHaveBeenCalled();
  });
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadDenebPluginsMock).toHaveBeenCalledWith({
    ...
    expect(loadDenebPluginsMock).toHaveBeenCalledTimes(1);
    expect(loadDenebPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadDenebPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadDenebPluginsMock).toHaveBeenCalledTimes(2);
  });
});

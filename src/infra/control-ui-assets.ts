...
import { resolveDenebPackageRoot, resolveDenebPackageRootSync } from "./deneb-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveDenebPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "deneb" + dist/control-ui/index.html
...
function resolveControlUiRootSyncUncached(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveDenebPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/deneb -> .../dist/index.js)
...
export async function ensureControlUiAssetsBuilt(
  ...
  return {
    ok: false,
    built: false,
    message: `${hint}. Run \`pnpm build\` to generate them.`,
  };
}

...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.DENEB_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*deneb-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "deneb";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "deneb");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "deneb-gw");
  ...
  records.push(`_deneb-gw._tcp IN PTR ${instanceLabel}._deneb-gw._tcp`);
  records.push(`${instanceLabel}._deneb-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._deneb-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; deneb-content-hash: ${contentHash}\n${contentBody}`;
}

...
export const POSIX_DENEB_TMP_DIR = "/tmp/deneb";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredDenebTmpDirOptions = {
...
export function resolvePreferredDenebTmpDir(
  options: ResolvePreferredDenebTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "deneb" : `deneb-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[deneb] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Deneb temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Deneb temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_DENEB_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_DENEB_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_DENEB_TMP_DIR)) {
      return POSIX_DENEB_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_DENEB_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_DENEB_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_DENEB_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_DENEB_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_DENEB_TMP_DIR;

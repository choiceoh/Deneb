 ...
 * `deneb update`. Use it to verify the completion installation flow
...
\`deneb update\`, \`deneb doctor\`, and \`deneb onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Deneb Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`deneb completion --install\` later to enable.`));
    return;
  }

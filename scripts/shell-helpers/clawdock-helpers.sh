#!/usr/bin/env bash
# ClawDock - Docker helpers for Deneb
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/deneb"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^DENEB_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d deneb-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart deneb-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f deneb-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec deneb-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm deneb-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build deneb-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "DENEB_GATEWAY_TOKEN=$token" deneb-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" deneb-gateway \
    bash -c './deneb.mjs config set gateway.remote.token "$TOKEN" && ./deneb.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec deneb-gateway \
    bash -c "./deneb.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart deneb-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm deneb-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec deneb-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Deneb${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (deneb alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to deneb project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.deneb)${_CLR_RESET}"

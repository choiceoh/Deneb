...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/deneb/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Deneb Re-Auth" -c "After getting key, run: ssh $SERVER '~/deneb/scripts/mobile-reauth.sh'" --id deneb-auth
        ;;
    *)

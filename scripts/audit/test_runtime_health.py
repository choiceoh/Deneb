...
class ParsingAndDataSufficiencyTests(unittest.TestCase):
    def test_journal_no_entries_is_insufficient_data_exit_2(self) -> None:
        stdout = io.StringIO()
        stderr = io.StringIO()
    ...
    def test_zero_runs_cannot_be_scored_through_importable_api(self) -> None:
        with self.assertRaisesRegex(health.InsufficientDataError, "0 completed runs"):
            health.compute(health.Signals())

    def test_zero_agent_ms_samples_cannot_be_scored_through_importable_api(self) -> None:
        with self.assertRaisesRegex(health.InsufficientDataError, "0 agentMs samples"):
            health.compute(health.Signals(runs=1))

    def test_zero_agent_ms_samples_are_insufficient_data_in_cli(self) -> None:
        stdout = io.StringIO()
        stderr = io.StringIO()
    ...
    def test_parser_keeps_valid_run_fields(self) -> None:
...
class JournalSpanTests(unittest.TestCase):
    def test_iso_journal_span_is_inclusive(self) -> None:
        lines = [
    ...
    def test_english_journal_span_is_inclusive(self) -> None:
        lines = [
    ...
    def test_english_journal_span_handles_year_rollover(self) -> None:
        lines = [
    ...
    def test_english_journal_span_uses_non_leap_reference_year(self) -> None:
        lines = [
    ...
    def test_korean_journal_span_is_inclusive(self) -> None:
        lines = [
...
class ScoringBoundaryTests(unittest.TestCase):
    def test_graded_soft_midpoint_and_hard_boundaries(self) -> None:
    ...
    def test_latency_score_honors_soft_midpoint_and_hard_boundaries(self) -> None:
    ...
    def test_partial_latency_coverage_reduces_latency_score(self) -> None:
...
class OutputContractTests(unittest.TestCase):
    ...
    def test_hyphenated_cli_path_remains_executable(self) -> None:

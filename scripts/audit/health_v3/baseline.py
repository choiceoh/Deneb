"""Baseline persistence and regression policy for Health Bench 3.0."""
...
from .model import DOMAIN_WEIGHTS, RUBRIC_VERSION, SCHEMA_VERSION, SEVERITY_ORDER, Report
...
KEEP_POLICY = (
    "Health Bench 3.0 baseline. One-way ratchet across overall, ratcheted domains, "
    "and metrics. Never lower floors or accept new high/critical findings merely to "
    "make --check pass. Fitness starts advisory (ratcheted=false)."
)
...
class CheckResult:
    ...
    def format_lines(self) -> list[str]:
        if self.ok:
            return ["Health Bench 3.0 baseline check passed."]
...
def _validate(data: object) -> dict[str, Any]:
    ...
    ratcheted = data.get("ratcheted_domains", ["structure", "runtime"])
...
def _atomic_write(path: Path, payload: dict[str, Any]) -> None:
    ...
    fd, tmp_name = tempfile.mkstemp(prefix="health-v3-baseline-", dir=str(path.parent))
    try:
...
def update(path: Path, report: Report, *, provenance: dict[str, Any] | None = None) -> None:
    ...
    if path.is_file():
        ...
        for domain_id, floor in current["domains"].items():
            ...
            if domain_id in current.get("ratcheted_domains", DOMAIN_WEIGHTS) and (
...
def check(report: Report, baseline: dict[str, Any]) -> CheckResult:
    ...
    ratcheted = set(baseline.get("ratcheted_domains") or ["structure", "runtime"])
    regressions: list[str] = []

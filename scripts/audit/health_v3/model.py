"""Stable report model and composite helpers for Health Bench 3.0."""
...
# Domain weights for the geometric composite (must sum to 1.0).
DOMAIN_WEIGHTS = {
    "structure": 0.55,
    "runtime": 0.25,
    "fitness": 0.20,
...
# Product impact for AI maintainability lane credit (shell/rust N/A).
AI_LANE_IMPACT = {
    "go": 0.45,
    "kotlin": 0.25,
    "typescript": 0.15,
...
def geometric_composite(domain_scores: dict[str, float], weights: dict[str, float] = DOMAIN_WEIGHTS) -> float:
    """Weighted geometric mean; any non-positive domain score yields 0."""
...
class Domain:
    id: str
    title: str
    weight: float  # share of overall geometric composite (0–1)
    metrics: list[Metric]
...
class Report:
    ...
    def confidence(self) -> float:
        applicable = [
            ...
            if item.status not in {"not_applicable"}
        ]
        if not applicable:
            return 0.0
    ...
    def pillar_scores(self) -> dict[str, float]:
        """Flat metric id → score map for baseline ratchet."""
        out: dict[str, float] = {}

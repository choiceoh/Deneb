...
export function collectBundledProviderAuthEnvVars(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(extensionsRoot, dirent.name, "deneb.plugin.json");
...
export function renderBundledProviderAuthEnvVarModule(entries) {
  const renderedEntries = Object.entries(entries)
    ...
    .join("\n");
  const body = renderedEntries ? `{\n${renderedEntries}\n}` : `{}`;
  return `// Auto-generated by ${GENERATED_BY}. Do not edit directly.

export const BUNDLED_PROVIDER_AUTH_ENV_VAR_CANDIDATES = ${body} as const satisfies Record<
  string,
  readonly string[]
>;
`;

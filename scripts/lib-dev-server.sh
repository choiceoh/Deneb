#!/usr/bin/env bash
# lib-dev-server.sh — Shared dev server management library.
#
# Source this file from dev-*.sh scripts:
#   source "$(dirname "$0")/lib-dev-server.sh"
#
# Provides:
...
# Generate dev config via dev-config-gen.sh.
...
devlib_gen_config() {
  ...
  if [[ -n "${2:-}" ]]; then
    DENEB_DEV_TELEGRAM_TOKEN="$2" \
      "$DEVLIB_SCRIPT_DIR/dev-config-gen.sh" --out "$out" >/dev/null 2>&1
  else
    "$DEVLIB_SCRIPT_DIR/dev-config-gen.sh" --out "$out" >/dev/null 2>&1
  fi
}

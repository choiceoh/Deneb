...
#
# Usage:
#   scripts/metric-gen.sh list                         # show presets
#   scripts/metric-gen.sh smoke                        # generate smoke metric
#   scripts/metric-gen.sh quality                      # generate quality metric
#   scripts/metric-gen.sh combined                     # generate combined metric
#   scripts/metric-gen.sh custom "메시지"               # generate custom metric
...
source "$REPO_DIR/scripts/lib-server.sh"
...
# Quality metric via Telegram (delegates to quality-metric.sh).
QUALITY_OUT=$("$REPO_DIR/scripts/quality-metric.sh" 2>&1) || true
...
QUALITY_OUT=\$("\$REPO_DIR/scripts/quality-metric.sh" "\$PORT" $escaped_msg 2>&1) || true
METRIC_VAL=\$(echo "\$QUALITY_OUT" | grep -oP 'metric_value=\K[\d.]+' | tail -1)
DETAIL=\$(echo "\$QUALITY_OUT" | grep '^DENEB_METRIC_DETAIL ' | tail -1)
...
QUALITY_OUT=$("$REPO_DIR/scripts/quality-metric.sh" "$MESSAGE" 2>&1) || true
...
if [[ -n "${JUDGE_API_KEY:-${ANTHROPIC_API_KEY:-}}" ]] && [[ -n "$RESPONSE_TEXT" ]]; then
  JUDGE_OUT=$(python3 "$REPO_DIR/scripts/bench-judge.py" absolute \
...
CAND_OUT=$("$REPO_DIR/scripts/quality-metric.sh" "$MESSAGE" 2>&1) || true
...
BASE_OUT=$(METRIC_PORT=$BASELINE_PORT "$REPO_DIR/scripts/quality-metric.sh" "$MESSAGE" 2>&1) || true
BASE_TEXT=$(echo "$BASE_OUT" | grep '  reply: ' | sed 's/^  reply: //' | head -1)

# 3) Pairwise judge.
if [[ -n "${JUDGE_API_KEY:-${ANTHROPIC_API_KEY:-}}" ]] && [[ -n "$CAND_TEXT" ]] && [[ -n "$BASE_TEXT" ]]; then
  PW_OUT=$(python3 "$REPO_DIR/scripts/bench-judge.py" pairwise \
...
BENCH_OUT=$(python3 "$REPO_DIR/scripts/quality-test.py" \
...
cmd_list() {
  ...
  echo "             Usage: metric-gen.sh custom \"메시지\""
  echo ""

...
#
# Usage:
#   scripts/ar-results.sh                    # human-readable summary
#   scripts/ar-results.sh --json             # structured JSON
#   scripts/ar-results.sh --table            # compact table
#   scripts/ar-results.sh --best             # top improvements only
#   scripts/ar-results.sh --failures         # recent failures analysis
#   scripts/ar-results.sh --suggest          # next action suggestion
#
# Output:
...
_check_exists() {
  if [[ ! -f "$CONFIG_FILE" ]]; then
    ...
    echo "  Start with: scripts/autoresearch.sh start --target FILE"
    exit 0
  fi
}
...
cmd_suggest() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    ...
    echo "  action: Start autoresearch with autoresearch.sh start"
    return 0
  fi
...
case "$FORMAT" in
  ...
  summary)    cmd_summary ;;
  *)
    echo "Usage: ar-results.sh [--json|--table|--best|--failures|--suggest]"
    exit 1
    ;;
esac

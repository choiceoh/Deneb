...
type DriverMode = "token" | "webhook" | "deneb";
...
function resolveStateDir(): string {
  const override = process.env.DENEB_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.DENEB_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".deneb");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver deneb] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Deneb spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|deneb> Driver transport mode (default: token)\n" +
    ...
    "  --deneb-bin <path>        Deneb CLI binary for driver=deneb (default: deneb)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  DENEB_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  DENEB_DISCORD_SMOKE_DRIVER\n" +
    "  DENEB_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  DENEB_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  DENEB_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  DENEB_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  DENEB_DISCORD_SMOKE_AGENT\n" +
    "  DENEB_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  DENEB_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  DENEB_DISCORD_SMOKE_POLL_MS\n" +
    "  DENEB_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  DENEB_DISCORD_SMOKE_DENEB_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.DENEB_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.DENEB_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.DENEB_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.DENEB_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.DENEB_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.DENEB_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.DENEB_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.DENEB_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.DENEB_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.DENEB_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.DENEB_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.DENEB_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const denebBin =
    resolveArg("--deneb-bin") || process.env.DENEB_DISCORD_SMOKE_DENEB_BIN || "deneb";
...
async function denebCliJson<T>(params: { denebBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.denebBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`deneb ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithDeneb(params: {
  denebBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await denebCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    denebBin: params.denebBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "deneb") {
    return await readMessagesWithDeneb({
      denebBin: params.args.denebBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `deneb-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("deneb message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "deneb"
            ? await readMessagesWithDeneb({
                denebBin: args.denebBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Deneb.`,
        diagnostics: {
          bindingCandidates: [
            {

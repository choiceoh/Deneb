...
# Deneb Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Deneb."
...
GUM_VERSION="${DENEB_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Deneb Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://deneb.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$DENEB_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.deneb.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_deneb_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.deneb-* "$npm_root"/deneb 2>/dev/null || true
}

extract_deneb_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_deneb_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/deneb" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/deneb"|"/usr/local/bin/deneb")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/deneb/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale deneb symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing deneb binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Deneb package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_deneb_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Deneb npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*deneb" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_deneb_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_deneb_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Deneb npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an deneb binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "DENEB_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "DENEB_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "DENEB_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "DENEB_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "DENEB_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "DENEB_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "DENEB_BETA" "CLAWDBOT_BETA"
map_legacy_env "DENEB_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "DENEB_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "DENEB_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "DENEB_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "DENEB_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "DENEB_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${DENEB_TAGLINE_INDEX:-}" ]]; then
        if [[ "${DENEB_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((DENEB_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${DENEB_NO_ONBOARD:-0}
NO_PROMPT=${DENEB_NO_PROMPT:-0}
DRY_RUN=${DENEB_DRY_RUN:-0}
INSTALL_METHOD=${DENEB_INSTALL_METHOD:-}
DENEB_VERSION=${DENEB_VERSION:-latest}
USE_BETA=${DENEB_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/deneb"
GIT_DIR=${DENEB_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${DENEB_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${DENEB_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${DENEB_VERBOSE:-0}"
VERIFY_INSTALL="${DENEB_VERIFY_INSTALL:-0}"
DENEB_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Deneb installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/deneb)
...
Environment variables:
  DENEB_INSTALL_METHOD=git|npm
  DENEB_VERSION=latest|next|main|<semver>|<spec>
  DENEB_BETA=0|1
  DENEB_GIT_DIR=...
  DENEB_GIT_UPDATE=0|1
  DENEB_NO_PROMPT=1
  DENEB_VERIFY_INSTALL=1
  DENEB_DRY_RUN=1
  DENEB_NO_ONBOARD=1
  DENEB_VERBOSE=1
  DENEB_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Deneb checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Deneb source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_deneb_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"deneb"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://deneb.ai/install.sh | bash"
}
...
ensure_deneb_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/deneb" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/deneb" ]]; then
        ln -sf "$npm_root/deneb/dist/entry.js" "${npm_bin}/deneb"
        ui_info "Created deneb bin link at ${npm_bin}/deneb"
    fi
    return 0
}

# Check for existing Deneb installation
check_existing_deneb() {
    if [[ -n "$(type -P deneb 2>/dev/null || true)" ]]; then
        ui_info "Existing Deneb installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make deneb show as \"command not found\" in new terminals."
...
warn_deneb_not_found() {
    ui_warn "Installed, but deneb is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t deneb 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named deneb; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_deneb_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P deneb 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/deneb" ]]; then
        echo "${npm_bin}/deneb"
        return 0
    fi
...
install_deneb_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/deneb/deneb.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Deneb from git checkout: ${repo_dir}"
    else
        ui_info "Installing Deneb from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Deneb" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Deneb" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/deneb" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/deneb"
    ui_success "Deneb wrapper installed to \$HOME/.local/bin/deneb"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view deneb dist-tags.beta 2>/dev/null || true)"
...
install_deneb() {
    local package_name="deneb"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            DENEB_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="deneb"
        else
            DENEB_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${DENEB_VERSION}" ]]; then
        DENEB_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${DENEB_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${DENEB_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Deneb v${resolved_version}"
    else
        ui_info "Installing Deneb (${DENEB_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${DENEB_VERSION}")"

    if ! install_deneb_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_deneb_paths
        install_deneb_npm "${install_spec}"
    fi

    if [[ "${DENEB_VERSION}" == "latest" && "${package_name}" == "deneb" ]]; then
        if ! resolve_deneb_bin &> /dev/null; then
            ui_warn "npm install deneb@latest failed; retrying deneb@next"
            cleanup_npm_deneb_paths
            install_deneb_npm "deneb@next"
        fi
    fi

    ensure_deneb_bin_link || true

    ui_success "Deneb installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${DENEB_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_deneb_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (deneb not on PATH yet)"
        warn_deneb_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${DENEB_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.deneb/workspace-${profile}"
    else
        echo "${HOME}/.deneb/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${DENEB_CONFIG_PATH:-$HOME/.deneb/deneb.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run deneb onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but deneb not on PATH; skipping onboarding"
        warn_deneb_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run deneb onboard to retry"
        return
    }
}
...
resolve_deneb_version() {
    local version=""
    local raw_version_output=""
    local claw="${DENEB_BIN:-}"
    if [[ -z "$claw" ]] && command -v deneb &> /dev/null; then
        claw="$(command -v deneb)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_deneb_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/deneb/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/deneb/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: deneb not on PATH yet"
        warn_deneb_not_found
        return 1
    fi

    run_quiet_step "Checking Deneb version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: deneb gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_deneb_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Deneb checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set DENEB_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_deneb_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/deneb" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/deneb"
    ...
    DENEB_BIN="$(resolve_deneb_bin || true)"
    ...
    installed_version=$(resolve_deneb_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Deneb installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Deneb installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/deneb"
        ui_kv "Update command" "deneb update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://deneb.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running deneb doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                DENEB_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                DENEB_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                DENEB_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run deneb doctor and deneb plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run deneb onboard later"
        else
            local config_path="${DENEB_CONFIG_PATH:-$HOME/.deneb/deneb.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (deneb not on PATH yet)"
                    warn_deneb_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run deneb onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v deneb &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (deneb daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if DENEB_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: deneb daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${DENEB_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi

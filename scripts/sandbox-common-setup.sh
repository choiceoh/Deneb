...
BASE_IMAGE="${BASE_IMAGE:-deneb-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-deneb-sandbox-common:bookworm-slim}"
...
DENEB_DOCKER_BUILD_USE_BUILDX="${DENEB_DOCKER_BUILD_USE_BUILDX:-0}"
DENEB_DOCKER_BUILD_CACHE_FROM="${DENEB_DOCKER_BUILD_CACHE_FROM:-}"
DENEB_DOCKER_BUILD_CACHE_TO="${DENEB_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${DENEB_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${DENEB_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${DENEB_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${DENEB_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${DENEB_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=deneb.sandbox=1)
NOTE

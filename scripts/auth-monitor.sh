...
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., deneb-alerts)
...
STATE_FILE="$HOME/.deneb/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Deneb if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use deneb
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Deneb to $NOTIFY_PHONE..."
            deneb send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Deneb is down. Run: ssh l36 '~/deneb/scripts/mobile-reauth.sh'" "urgent"
    exit 1

...
const extensionRestartMetadataFiles = new Set(["deneb.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.DENEB_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.DENEB_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[deneb] ${message}\n`);
};

const runDeneb = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["deneb.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });

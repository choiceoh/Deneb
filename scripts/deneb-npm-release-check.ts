...
const EXPECTED_REPOSITORY_URL = "https://github.com/deneb/deneb";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "deneb") {
    errors.push(`package.json name must be "deneb"; found "${pkg.name ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`deneb-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `deneb-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

...
IMAGE_NAME="deneb-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="deneb-net-e2e-$$"
GW_NAME="deneb-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "DENEB_GATEWAY_TOKEN=$TOKEN" \
  -e "DENEB_SKIP_CHANNELS=1" \
  -e "DENEB_SKIP_GMAIL_WATCHER=1" \
  -e "DENEB_SKIP_CRON=1" \
  -e "DENEB_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \

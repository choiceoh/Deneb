...
IMAGE_NAME="deneb-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/deneb-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/deneb-bin/systemctl

  cat > /tmp/deneb-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/deneb-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/deneb"
	  npm_root="/tmp/npm-prefix/lib/node_modules/deneb"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/deneb-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/deneb-gateway.service"
    if [ ! -f "$unit_path" ]; then

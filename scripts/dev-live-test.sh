...
#   scripts/dev-live-test.sh concurrency [S]    Run concurrency tests (all|queue|fifo|interrupt|abort|multi-conn|rapid)
...
# Concurrency tests: multi-turn queuing, interrupt, abort, multi-connection.
cmd_concurrency() {
  local scenario="${1:-all}"
  python3 "$SCRIPT_DIR/dev-concurrency-test.py" --port "$DEV_PORT" --scenario "$scenario"
}
...
case "${1:-help}" in
  ...
  concurrency)    shift; cmd_concurrency "$@" ;;
  ...
  help|*)
    ...
    echo "  concurrency [S]     Concurrency test (all|queue|fifo|interrupt|abort|multi-conn|rapid)"
    echo ""
    echo "Autoresearch:"

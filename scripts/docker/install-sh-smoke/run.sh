...
INSTALL_URL="${DENEB_INSTALL_URL:-https://deneb.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${DENEB_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${DENEB_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="deneb"
PACKAGE_NAME="${DENEB_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (DENEB_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${DENEB_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${DENEB_INSTALL_LATEST_OUT:-}"
fi

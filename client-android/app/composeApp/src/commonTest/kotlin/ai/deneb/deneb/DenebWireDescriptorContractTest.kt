...
class DenebWireDescriptorContractTest {
    ...
    private fun descriptorContractCases(): List<() -> Unit> = listOf(
        ...
        {
            val serializer = DedupPairRow.serializer()
            val descriptor = serializer.descriptor

            assertEquals("DedupPairRow", descriptor.serialName.substringAfterLast('.'))
            assertTrue(descriptor.elementsCount > 0)
            assertEquals(descriptor.elementsCount, descriptor.names().distinct().size)
            assertTrue((0 until descriptor.elementsCount).all(descriptor::isElementOptional))

            val empty = json.decodeFromString(serializer, "{}")
            assertEquals(DedupPairRow(), empty)
        ...
        {
            val serializer = DedupPartyRow.serializer()
            val descriptor = serializer.descriptor

            assertEquals("DedupPartyRow", descriptor.serialName.substringAfterLast('.'))
            assertTrue(descriptor.elementsCount > 0)
            assertEquals(descriptor.elementsCount, descriptor.names().distinct().size)
            assertTrue((0 until descriptor.elementsCount).all(descriptor::isElementOptional))

            val empty = json.decodeFromString(serializer, "{}")
            assertEquals(DedupPartyRow(), empty)
        ...
        {
            val serializer = ContactsAdjudicatePayload.serializer()
            val descriptor = serializer.descriptor

            assertEquals("ContactsAdjudicatePayload", descriptor.serialName.substringAfterLast('.'))
            assertTrue(descriptor.elementsCount > 0)
            assertEquals(descriptor.elementsCount, descriptor.names().distinct().size)
            assertTrue((0 until descriptor.elementsCount).all(descriptor::isElementOptional))

            val empty = json.decodeFromString(serializer, "{}")
            assertEquals(ContactsAdjudicatePayload(), empty)

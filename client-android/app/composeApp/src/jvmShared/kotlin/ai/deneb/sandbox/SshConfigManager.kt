 ...
 * [ai.deneb.tools.SshConfigureHostTool].
 *
 ...
 *
 * All blocks we own are bracketed with `# deneb:<marker>:start` /
 * `# deneb:<marker>:end` so upserts are idempotent without disturbing
...
class SshConfigManager(private val homeDir: File) {
    ...
    fun listAliases(): List<String> {
        val regex = Regex("""# deneb:host:([^:\s]+):start""")
    ...
    private fun upsertBlock(text: String, marker: String, body: String): String {
        val startTag = "# deneb:$marker:start"
        val endTag = "# deneb:$marker:end"

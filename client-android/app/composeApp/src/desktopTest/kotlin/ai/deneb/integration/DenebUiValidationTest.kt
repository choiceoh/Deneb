 ...
 * 4. When green, copy the now-passing payloads into `DenebUiParserTest` as regressions
 */
class DenebUiValidationTest {
    ...
    fun `validate deneb-ui output across providers`() {
        ...
        val reportDir = File(System.getenv("DENEB_REPORT_DIR") ?: "build/reports/denebui-integration")
        reportDir.mkdirs()
        val requests = Requests()
        val allResults = mutableListOf<PromptResult>()

        for (provider in providers) {
            println("\n[DenebUiValidationTest] === ${provider.label} (${provider.modelId}) ===")
    ...
    fun `replay saved responses`() {
        ...
        if (!reportDir.isDirectory) {
            println("[DenebUiValidationTest] Replay skipped — no report directory at ${reportDir.absolutePath}")
            return
        }
        ...
        println("\n[DenebUiValidationTest] Replaying ${rawFiles.size} saved responses from ${reportDir.absolutePath}")
        var success = 0
        var partial = 0
        var noUi = 0
        var parseError = 0
        ...
        for (file in rawFiles) {
            ...
            val ui = blocks.filterIsInstance<DenebUiBlock>().size
            val err = blocks.filterIsInstance<DenebUiError>().size
    ...
    private fun runPrompt(
        ...
        val errorBlocks = blocks.filterIsInstance<DenebUiError>()
    ...
    private fun writeReport(reportDir: File, results: List<PromptResult>, providers: List<ProviderSpec>) {
        val report = buildString {
            appendLine("# deneb-ui Integration Report")
            appendLine()
        ...
        println("\n[DenebUiValidationTest] Report written to: ${File(reportDir, "report.md").absolutePath}")
    }

    private fun printSummary(results: List<PromptResult>, providers: List<ProviderSpec>) {
        println("\n[DenebUiValidationTest] === SUMMARY ===")
        for (p in providers) {
            val r = results.filter { it.provider.slug == p.slug }
    ...
    /**
     * Duplicates the deneb-ui portion of the app's system prompt. Mirrors
    ...
    companion object {
        private val TEST_PROMPTS = listOf(
            TestPrompt(
                "simple-question",
                "Simple question expecting a text response with deneb-ui elements",
            ...
            TestPrompt(
                "mixed-markdown",
                "Response that mixes markdown explanation with deneb-ui interactive block",

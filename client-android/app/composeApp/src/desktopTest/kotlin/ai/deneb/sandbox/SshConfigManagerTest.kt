...
class SshConfigManagerTest {
    ...
    fun ensureDefaultsCreatesBlockOnce() {
        ...
        assertContains(first, "# deneb:defaults:start")
        ...
        assertContains(first, "# deneb:defaults:end")
    ...
    fun upsertHostAddsBlockAndSeedsDefaults() {
        ...
        assertTrue(changed)
        val text = configText()
        assertContains(text, "# deneb:defaults:start")
        assertContains(text, "# deneb:host:prod:start")
        assertContains(text, "Host prod")
        assertContains(text, "HostName 1.2.3.4")
        assertContains(text, "# deneb:host:prod:end")
    }
    ...
    fun upsertHostReplacesPreviousBlock() {
        ...
        val startCount = "# deneb:host:prod:start".toRegex().findAll(text).count()
        val endCount = "# deneb:host:prod:end".toRegex().findAll(text).count()
    ...
    @Test
    fun upsertHostKeepsNonManagedContent() {

...
private const val PID_PROBE_PREFIX = "${RS}DENEBBASHPID$US"
...
class PersistentSandboxShell(
    ...
    suspend fun run(
        ...
        val cmdFile = File(tmpPath, ".deneb_cmd_$nonce")
        try {
        ...
        val line = ". /tmp/.deneb_cmd_$nonce; __deneb_st=\$?; rm -f /tmp/.deneb_cmd_$nonce; " +
            "printf '\\n\\036%s\\037%d\\037%d\\037%s\\036\\n' '$nonce' \"\$__deneb_st\" \"\$\$\" \"\$PWD\" >&2"

...
class DenebGatewayClient private constructor(
    ...
    // Guard against double-tap on "이전 대화 더 보기" racing two page fetches with
    // the same offset — duplicate Conversation.id values crash LazyColumn.
    @Volatile
    private var loadMoreConversationsInFlight = false
    ...
    override fun loadMoreConversations() {
        if (loadMoreConversationsInFlight) return
        loadMoreConversationsInFlight = true
        scope.launch {
            try {
                val epoch = credEpoch
                ...
                loadMoreConversationsInFlight = false
            }
        }
    }

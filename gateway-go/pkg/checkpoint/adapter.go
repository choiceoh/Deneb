...
// interface defined in internal/pipeline/chat/toolport/ (which doesn't want
// to pull the richer *Snapshot return type into its public surface).
//
// Construct with NewToolAdapter and attach via toolport.WithCheckpointer.
type ToolAdapter struct{ M *Manager }

// NewToolAdapter returns a Checkpointer adapter for m. Returns nil if m is
// nil so callers can funnel through toolport.WithCheckpointer without extra
// guards.

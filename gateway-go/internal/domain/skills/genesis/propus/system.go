...
// UsageStats is the usage projection consumed by Propus summaries.
...
// SkillCuratorRecord is the curator projection consumed by Propus summaries.
...
// UsageQualitySummary is the usage-quality projection used by the overview.
...
type SkillValidationCaseSummary struct {
	...
	UniqueRecords            int
	SkillsWithCases          int
	UniqueEasyAnchorCases    int
	UniqueMixedFrontierCases int
	UniqueHardFrontierCases  int
}

// SkillOpportunityRecord marks one open exploration opportunity.
type SkillOpportunityRecord struct {
	SkillName string
	Route     string
}

// SelfCorrectionCandidateRecord marks one pending self-correction candidate.
type SelfCorrectionCandidateRecord struct {
	SkillName string
	Status    string
}

// SelfHarnessSignalSummary is the self-harness projection used by Propus.
type SelfHarnessSignalSummary struct {
	Rejections7d                  int
	ValidationDrafts7d            int
	TargetRecurrences7d           int
	SignatureMismatchRejections7d int
	SurfaceMismatchRejections7d   int
}

// SkillLivenessState is the Propus liveness projection.
...
// EvolutionHealthSummary is the evolution-health projection.
type EvolutionHealthSummary struct {
	Thrash             bool
	EvolveRolledBack7d int
	EvolveRejected7d   int
}

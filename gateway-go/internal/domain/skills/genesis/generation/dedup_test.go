...
func TestJaccardSimilarityReturnsRatioAcrossIdenticalDisjointAndPartialOverlapSets(t *testing.T) {
	...
	if got := genesiscommon.JaccardSimilarity(a, b); got != 0 {
		t.Fatalf("disjoint sets: want 0, got %v", got)
	}
	if got := genesiscommon.JaccardSimilarity(a, nil); got != 0 {
		t.Fatalf("empty set: want 0, got %v", got)
	}
	// 3 shared of 4 union = 0.75
	c := map[string]struct{}{"git": {}, "rebase": {}, "flow": {}, "extra": {}}
	if got := genesiscommon.JaccardSimilarity(a, c); got < 0.74 || got > 0.76 {

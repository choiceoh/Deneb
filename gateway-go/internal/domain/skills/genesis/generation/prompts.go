...
3. **비자명한 결정**: 시행착오를 거쳐 확인한 판단 기준·경계·검증법
...
세션에서 사용한 외부 도구가 스킬 실행에 필수라면 description의 Use when: 옆에 (requires: tool-name)을 쓰고, Procedure에도 실제 도구 이름을 명시하세요. 그래야 replay 검증이 필요한 도구를 로드할 수 있습니다.
...
- "Procedure" 섹션: 기대 결과와 대상, 결과를 실제로 바꾸는 근거/입력, 2~4개의 중요한 경계, 필요한 도구·명령·판단 기준을 명시
- 방법 선택은 에이전트에 맡기고, 안전·컴플라이언스·감사·결정적 재현 때문에 순서 자체가 계약일 때만 번호 순서를 강제
- "Pitfalls" 섹션: 주의사항과 흔한 실수
- "Verification" 섹션: 2~4개의 관찰 가능한 pass/fail 완료 기준. "충분히 좋게" 같은 모호한 기준 금지
- 세션의 배경정보는 결과를 바꾸는 것만 포함하고, 날짜·수치·사용자 교정처럼 결과에 영향을 주는 사실은 정확히 보존
- 파일 경로, 환경변수 등 하드코딩 금지 (플레이스홀더 사용)
- 5000자 이하로 유지

## 거부 기준 (다음이면 skip=true)
구체적이고 실행 가능한 스킬만 가치가 있습니다. 다음 중 하나라도 해당하면 skip 하세요:
- 실행 계약이 모호함 ("맥락을 잘 살펴라" 같은 일반론뿐이고 결과·판단 기준·도구·명령 중 실행 가능한 지시가 없음)
- When to Use / Procedure / Verification 섹션이 비거나 트리거·완료 기준이 불명확함
...
14. **Hermes-style patch first**: 출력 형식은 전체 body지만, 실제 변경은 작은 targeted patch여야 한다. 스킬 제목/목적을 바꾸거나 여러 섹션을 동시에 크게 갈아엎지 마라. 원칙: 한 번에 최대 1~2개 섹션만 수정하고, 추가·삭제·변경 라인 합계가 총 20행을 넘지 않아야 한다. 3개 이상의 섹션을 건드리거나 20행을 초과하는 변경이 필요하면 skip하고 source-level self-correction 또는 별도 review 후보로 남겨야 한다
...
// skill body against the original BEFORE it is committed. It enforces the
// qualitative safety rules (no invented tools, structure preserved, patch-sized,
// no held-out regression) plus a score-improvement margin. The margin was
// lowered (proactive-refinement mode) so genuine incremental improvements land
// instead of freezing every skill at its v1; bad rewrites are caught by the
// qualitative rules here and auto-rolled-back after 3 post-evolve failures.
...
2. 존재하지 않는 도구·명령어·파일 경로를 지어내지 않았다
   - 판정 기준은 입력의 "실제 등록된 게이트웨이 도구" 목록이다. 원본 SKILL.md에 없더라도
     그 목록에 있으면 실재하는 도구다 — 도구가 통합·개명되면 낡은 스킬을 고치는 후보가
     새 도구를 "지어낸" 것처럼 보이므로, 원본에 없다는 이유만으로 거부하지 마라
...
9. original_score와 candidate_score를 0~100 정수/실수로 매기고, candidate_score가 original_score보다 최소 2점 높을 때만 pass=true다

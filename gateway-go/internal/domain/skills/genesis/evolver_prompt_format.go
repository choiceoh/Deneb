...
func formatOptimizerMemory(memory SkillOptimizerMemoryEntry) string {
	...
	if len(memory.StableDirections) > 0 {
		...
		for _, direction := range memory.StableDirections {
			fmt.Fprintf(&b, "  - %s\n", genbind.TruncateRunes(direction, 240))
		}
	}
...
func formatValidationCasesForPrompt(cases []SkillValidationCaseRecord) string {
	...
	for i, tc := range cases {
		fmt.Fprintf(&b, "\n### %d. %s\n", i+1, genbind.TruncateRunes(validationCaseLabel(tc), 100))
		if desc := strings.TrimSpace(tc.Description); desc != "" {
			fmt.Fprintf(&b, "- description: %s\n", genbind.TruncateRunes(desc, 240))
		}
		if source := strings.TrimSpace(tc.Source); source != "" {
			fmt.Fprintf(&b, "- source: %s\n", genbind.TruncateRunes(source, 80))
		}
...
func writePromptReplayCase(b *strings.Builder, replay SkillReplayCaseRecord) {
	if strings.TrimSpace(replay.Input) != "" {
		fmt.Fprintf(b, "- replay input: %s\n", genbind.TruncateRunes(replay.Input, 220))
	}
...
func writePromptList(b *strings.Builder, label string, values []string) {
	...
	for _, value := range values {
		...
		fmt.Fprintf(b, "  - %s\n", genbind.TruncateRunes(value, 180))
	}
}
...
func writePromptToolCalls(b *strings.Builder, label string, calls []SkillReplayToolCallRecord) {
	...
	for _, call := range calls {
		...
		if name := strings.TrimSpace(call.Name); name != "" {
			parts = append(parts, "tool="+genbind.TruncateRunes(name, 80))
		}
		if len(call.InputIncludes) > 0 {
			parts = append(parts, "input includes ["+genbind.TruncateRunes(strings.Join(promptFragments(call.InputIncludes), "; "), 180)+"]")
		}
		if len(call.InputExcludes) > 0 {
			parts = append(parts, "input excludes ["+genbind.TruncateRunes(strings.Join(promptFragments(call.InputExcludes), "; "), 180)+"]")
		}
		if call.FixtureError {
			parts = append(parts, "fixture errored")
		}
		if fixture := strings.TrimSpace(call.FixtureOutput); fixture != "" {
			parts = append(parts, "fixture output example ["+genbind.TruncateRunes(fixture, 180)+"]")
		}
		if len(parts) == 0 {
			continue
		}
...
func formatConfirmedEvolveExemplars(exemplars []confirmedEvolveExemplar) string {
	...
	for _, ex := range exemplars {
		line := "- [" + ex.SkillName + "] 대상: " + genbind.TruncateRunes(strings.TrimSpace(ex.Audit.TargetSignature), 100)
		if s := strings.TrimSpace(ex.Audit.EditedSurface); s != "" {
			line += " · 편집면: " + genbind.TruncateRunes(s, 40)
		}
		if c := strings.TrimSpace(ex.Audit.ExpectedBehaviorChange); c != "" {
			line += " · 효과: " + genbind.TruncateRunes(c, 120)
		}
		sb.WriteString(line + "\n")
	}

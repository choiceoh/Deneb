...
const (
	...
	activityReviewAttempt = "review_attempt"
	activityReviewSkipped = "review_skipped"
)

// ActivityTracker records review liveness and generated-skill lifecycle events.
type ActivityTracker interface {
	RecordEvolutionActivity(kind string, ok bool, errMsg string)
	LogGenesis(skillName, source, sessionKey, category, description string) error
}
...
func (n *Nudger) OnToolCalls(ctx context.Context, sessionKey string, delta int, snapshot generation.SessionContext) {
...
func (n *Nudger) fire(parent context.Context, sessionKey string, snapshot generation.SessionContext) {
...
func (n *Nudger) WouldReview(snapshot generation.SessionContext) bool {
...
func (n *Nudger) RunStaleReview(sessionKey string, snapshot generation.SessionContext) (bool, error) {
...
func (n *Nudger) runReviewOnce(sessionKey string, snapshot generation.SessionContext) (bool, error) {
	...
	defer cancel()
	if n.tracker != nil {
		n.tracker.RecordEvolutionActivity(activityReviewAttempt, true, "")
	}
	if !n.svc.EvaluateReview(snapshot) {
		n.logger.Debug("skill nudger: review evaluate rejected session",
			...
			"tools", len(snapshot.ToolActivities))
		if n.tracker != nil {
			n.tracker.RecordEvolutionActivity(activityReviewSkipped, true, "")
		}
		return false, nil
	}
	...
	if n.tracker != nil {
		n.tracker.RecordEvolutionActivity(activityReview, err == nil, genesiscommon.ErrorString(err))
	}
	if err != nil {
...
func (n *Nudger) runOnce(sessionKey string, snapshot generation.SessionContext) (bool, error) {
	if !n.Enabled() {
		return false, nil
	}
	ctx, cancel := context.WithTimeout(n.baseContext(), nudgeGenerationTimeout)
	...
	if n.tracker != nil {
		_ = n.tracker.LogGenesis(genesiscommon.SanitizeSkillName(skill.Name), "nudge", sessionKey, skill.Category, skill.Description)
	}

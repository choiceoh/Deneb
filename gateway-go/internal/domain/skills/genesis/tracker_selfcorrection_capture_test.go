...
func TestSelfCorrectionReopenRejectedUnlessAppliedCooledAndFreshlyRecurring(t *testing.T) {
	...
	cases := []struct {
		...
		{"superseded + cooled + fresh recurrence → REOPEN (review ruling, not a veto)", []SelfCorrectionCandidateRecord{{Source: src, Status: selfCorrectionStatusSuperseded, CreatedAt: old}}, recent, false},
		{"superseded but not cooled → block", []SelfCorrectionCandidateRecord{{Source: src, Status: selfCorrectionStatusSuperseded, CreatedAt: recent}}, now.UnixMilli(), true},
		{"superseded + cooled but no fresh recurrence → block", []SelfCorrectionCandidateRecord{{Source: src, Status: selfCorrectionStatusSuperseded, CreatedAt: old}}, old - 1000, true},

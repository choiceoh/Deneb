...
- stage: **프로젝트 대표페이지 전용** — 사업 단계를 고정 어휘 하나로: 제안 → 견적 → 입찰 → 계약협의 → 시공 → 운영 (진행 순), 종결/유실 (말단). 일지·메일 신호로 단계 진행이 확인되면 갱신 (예: "계약서 검토 요청" 수신 → 계약협의). 어휘 밖 값은 무시됨. 불확실하면 생략(추측 금지)
- 현장 상세 문서 게이트: 대표페이지의 현장 상세 섹션(주소 목록·현장 스펙 문단)과 write-site 현장 페이지는 stage가 **계약협의 이상**(계약협의/시공/운영, 또는 기존 O&M 자산)일 때만 작성하라. 그 전 단계(제안·견적·입찰)는 sites 메타데이터 한 줄까지만 — 현장 원본 정보는 메일분석 페이지가 이미 보존한다 (운영자 확정 2026-07-19)
- 업데이트가 불필요하면 빈 배열 [] 반환

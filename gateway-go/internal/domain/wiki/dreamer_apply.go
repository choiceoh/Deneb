...
- 프로젝트 카테고리의 문서 구조 (프로젝트당 폴더 하나, 정해진 슬롯만 사용):
  ...
  - 대표페이지 title은 **프로젝트의 이름**이다 — 이벤트·상태·단계어·날짜 금지 ("세창스틸 태양광 — 6/29 견적 재송부" ✗ → "세창스틸 1,2공장 태양광" ✓). 상태 변화는 title이 아니라 summary와 로그.md에 쓴다. 현장/지역 토큰은 이름의 일부이니 보존
...
- program: **프로젝트 대표페이지 전용** — 한 벤처의 워크스트림인 형제 프로젝트들을 묶는 사업군 슬러그 (예: "비금-130mw" — 케이블 조달·커넥터·EPC 폴더가 공유). 형제 딜이 분명할 때만 기입하고, 기존 프로그램에 합류할 땐 기존 표기를 재사용하라. 단독 딜은 생략(대부분)
- 현장 상세 문서 게이트: 대표페이지의 현장 상세 섹션(주소 목록·"## 현장" 표·현장 스펙 문단)은 stage가 **개발 또는 계약협의 이상**(개발/계약협의/시공/운영, 또는 기존 O&M 자산)일 때만 작성하라 — 자체개발은 부지가 사업의 본체라 개발 단계부터 현장 문서가 정당하다. 영업 단계(제안·견적·입찰)는 sites 메타데이터 한 줄까지만 — 현장 원본 정보는 메일분석 페이지가 이미 보존한다 (운영자 확정 2026-07-19)
- 업데이트가 불필요하면 빈 배열 [] 반환

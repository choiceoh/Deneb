...
- 사용자 working-style 추론(사용자 카테고리 한정): 명시적으로 말한 선호뿐 아니라, 일지에서 **반복적으로 드러난** 상호작용 패턴(예: 답변을 산문으로 고침, 특정 맥락서 늘 숫자 요구, 특정 형식 반복 거부)도 working-style 규칙으로 도출하라. 단 **2회 이상 반복**이 분명할 때만이고 단발 행동·추측은 금지. 말한 선호는 confidence=high, 행동에서 추론한 규칙은 confidence=medium 이하로 구분해 운영자가 검토·정정하게 하라

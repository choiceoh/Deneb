...
func TestFormatACPSubagentSummary(t *testing.T) {
	...
	reg.Register(ACPAgent{ID: "a", Status: "running", SpawnedAt: now})
	reg.Register(ACPAgent{ID: "b", Status: "done", SpawnedAt: now, EndedAt: now})
	reg.Register(ACPAgent{ID: "c", Status: "failed", SpawnedAt: now, EndedAt: now})
...
func TestPruneStaleACPAgents(t *testing.T) {
	...
	reg.Register(ACPAgent{ID: "recent", Status: "done", SpawnedAt: now - 10000, EndedAt: now - 5000})
	reg.Register(ACPAgent{ID: "running", Status: "running", SpawnedAt: now})

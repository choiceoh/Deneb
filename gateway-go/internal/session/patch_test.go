...
func TestPatchStr(t *testing.T) {
	t.Run("nil src no change", func(t *testing.T) {
		dst := "original"
		if patchStr(&dst, nil) {
			t.Error("expected false for nil src")
		}
		if dst != "original" {
			t.Errorf("dst = %q, want %q", dst, "original")
		}
	})
	...
	t.Run("same value no change", func(t *testing.T) {
		...
		src := "same"
		if patchStr(&dst, &src) {
			t.Error("expected false for same value")
		}
	})

	t.Run("different value changed", func(t *testing.T) {
		dst := "old"
		src := "new"
		if !patchStr(&dst, &src) {
			t.Error("expected true for different value")
		}
		if dst != "new" {
			t.Errorf("dst = %q, want %q", dst, "new")
		}
	})
}

func TestPatchBool(t *testing.T) {
	t.Run("nil src no change", func(t *testing.T) {
		v := true
		dst := &v
		if patchBool(&dst, nil) {
			t.Error("expected false for nil src")
		}
		if *dst != true {
			t.Error("dst should remain true")
		}
	})

	t.Run("nil dst set from src", func(t *testing.T) {
		var dst *bool
		src := true
		if !patchBool(&dst, &src) {
			t.Error("expected true when dst is nil")
		}
		if dst == nil || *dst != true {
			t.Error("dst should be true")
		}
	})

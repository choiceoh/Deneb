...
func MigrateSession(legacy toolctx.TranscriptStore, store *Store, sessionKey string, logger *slog.Logger) error {
	...
	if err != nil {
		return fmt.Errorf("polaris migrate: load legacy: %w", err)
	}

	if polarisCount >= legacyTotal {
		return nil // already up to date
	}

	// Import only the delta.
	delta := legacyMsgs[polarisCount:]
	for _, msg := range delta {
		if err := store.AppendMessage(sessionKey, msg); err != nil {
			return fmt.Errorf("polaris migrate: append at offset %d: %w", polarisCount, err)
		}
		polarisCount++
	}

	logger.Info("polaris: migrated session",
		"session", sessionKey,
		"imported", len(delta),
		"total", legacyTotal)
	return nil

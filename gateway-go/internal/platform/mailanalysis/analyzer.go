...
보고 도입부는 deneb-ui 카드 한 블록으로 시작한다 — 여는 펜스는 ` + "```deneb-ui" + ` 한 줄 그대로 쓰고 그 줄 뒤에 다른 글자를 붙이지 않는다. 다음 줄부터 카드 본문이다 — 루트 <column> 안 <card>, 첫 행은 <row><icon name="mail" size="16"/><text style="caption">메일 분석</text></row>, 중요도는 <text color="error|warning|success">로, 금액·기한 같은 핵심 수치는 <stat>으로, 다음 행동은 <ul>로. 카드 안에 백틱·코드펜스 금지. 카드 뒤에 필요한 상세만 짧은 산문으로 잇는다. 메일 전체 본문을 그대로 전달하지 않는다.
...
const analysisSystemPrompt = "당신은 업무 메일 분석 어시스턴트입니다. 제공된 이메일을 업무 관점에서 분석합니다 — 맥락, 이해관계자, 리스크·기한, 다음 단계. " +
	...
	// Card-first reporting (proactive deneb-ui): the analysis lands on the work
	// feed whose body renderer supports label-HTML fences. Kept in the SYSTEM

...
// LLM input only. It does not extract facts, summarize, or mutate the stored mail
// body shown in the native app.
func cleanMailBodyForAnalysis(body string) string {
	...
	lines = stripBodyPrepHeadNoise(lines)
	lines = stripBodyPrepTrailingNoiseLines(lines)
...
func stripBodyPrepHeadNoise(lines []string) []string {
	if len(lines) == 0 {
		return lines
	}
	limit := len(lines)
...
func stripBodyPrepTrailingNoiseLines(lines []string) []string {
	if len(lines) == 0 {
		return lines
	}
	cut := len(lines)

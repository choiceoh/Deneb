...
누구에게 어떤 입장에서 보냈고, 담당자 교체나 에스컬레이션처럼 평소와 다른 신호가
있는지. 결제 기한·마감·금액·미해결 이슈처럼 시간과 돈에 민감한 것은 묻히지 않게
하고, 마지막은 추측이 아닌 구체적인 다음 행동입니다.

무엇을 앞세우고 어떻게 묶을지는 그 메일이 정합니다. 고정된 섹션 틀에 끼워 맞추지
말고 사안에서 중요한 것이 먼저 오게 하세요. 짧으면 짧게, 복잡하면 깊게. 근거 있는
것만 말하고 모르는 건 모른다고 두세요. 한국어로 간결하게.
...
const batchAnalysisPrompt = `다음 %d건의 이메일을 한꺼번에 읽고 하나의 통합
...
읽기 쉽게 돕는 장치는 자유롭게 쓰되 의무는 아닙니다: 긴급도 표시(🔴 긴급, 🟠 중요,
🟡 일반, 🔵 참고), 그룹 사이 구분선, 발신자 실명, 핵심 불릿, 다음 행동. 사안이
단순하면 간략히, 복잡하면 깊게.

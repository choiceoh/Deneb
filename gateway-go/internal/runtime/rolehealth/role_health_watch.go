...
// Watch probes the cloud providers behind model roles and persists verdicts.
...
// Start launches the watch loop on ctx.
func (w *roleHealthWatch) Start(ctx context.Context) {
	if w == nil {
		return
	}
	w.logger.Info("role health watch started",
		"interval", roleHealthInterval.String(), "state", w.statePath)
	safego.GoWithSlog(w.logger, "role-health-watch", func() {
		w.run(ctx)
	})
}

// Verdicts returns a copy of the latest per-provider verdicts.
func (w *roleHealthWatch) Verdicts() map[string]string {
	if w == nil {
		return nil
	}
	return w.verdictsCopy()
}

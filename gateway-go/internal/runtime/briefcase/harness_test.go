...
func TestExportRunArtifactsCreatesFreshDurableBundle(t *testing.T) {
	pack := writeHarnessCase(t)
	pack.Manifest.Artifacts = []casepack.Artifact{{
		ID: "report", Path: "output/report.txt", MIME: "text/plain", Required: true, MaxBytes: 64,
	}}
	sourceRoot := t.TempDir()
	source := filepath.Join(sourceRoot, "output", "report.txt")
	if err := os.MkdirAll(filepath.Dir(source), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(source, []byte("durable report"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(sourceRoot, "undeclared.txt"), []byte("private"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if exported.ArtifactRoot != destination {
		t.Fatalf("artifactRoot = %q, want %q", exported.ArtifactRoot, destination)
	}
	data, err := os.ReadFile(filepath.Join(destination, "output", "report.txt"))
	if err != nil || string(data) != "durable report" {
		t.Fatalf("exported artifact = %q, %v", data, err)
	}
	if _, err := os.Stat(filepath.Join(destination, "undeclared.txt")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("undeclared file was exported: %v", err)
	}
	if _, err := ExportRunArtifacts(context.Background(), pack, &RunResult{ArtifactRoot: sourceRoot}, destination); err == nil {
...
func TestExportRunArtifactsRejectsDurableDestinationInsidePlaintextRunRoot(t *testing.T) {
	...
	source := filepath.Join(paths.Workspace, "output", "report.txt")
	if err := os.MkdirAll(filepath.Dir(source), 0o700); err != nil {
		t.Fatal(err)
	}

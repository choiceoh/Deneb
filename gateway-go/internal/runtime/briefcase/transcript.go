...
// or Polaris store.
func NewRunTranscript(paths RunPaths, logger *slog.Logger) (*RunTranscript, error) {
	...
	legacy := chat.NewCachedTranscriptStore(chat.NewFileTranscriptStore(transcriptDir), 0)
...
func validateTranscriptPaths(paths RunPaths) error {

...
func (t *heartbeatTask) detectSelfImproveSweepNudge(now time.Time) string {
	...
	t.logger.Info("heartbeat: self-improvement sweep nudge fired",
		"rejections7d", funnel.Rejections7d, "recurrences7d", recurrences)
	return buildSelfImproveSweepNudge(funnel, recurrences)
}
...
// skill_lifecycle), never edit repository code from a heartbeat turn.
func buildSelfImproveSweepNudge(funnel genesis.SelfCorrectionFunnelSummary, recurrences int) string {
	return fmt.Sprintf(`[자가개선 스윕] 자가개선 후보 큐는 비어 있지만 최근 7일 신호가 쌓여 있습니다: evolve 거절 %d건(승격자격 %d) · 수정 재발 %d건. 이번 점검에서 후보를 직접 발굴하세요:
1) skill_lifecycle(action=status)로 self-harness 신호·최근 거절 사유·실패 패턴을 확인하세요.
2) 반복되는 실패/마찰 중 고칠 가치가 있는 것을 skill_lifecycle(action=self_correction_propose)로 최대 2건 제안하세요 — evidence·targetFiles·risk를 채우고, 저장소 코드 수정이 필요한 건은 제안 기록만 남기세요.
3) 후보화할 가치가 없으면 제안하지 마세요 — 억지 후보는 필터 정밀도를 망칩니다. 보고는 임원 판단이 필요한 발견일 때만(기본 NO_REPLY).`,

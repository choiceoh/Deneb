...
//
// Receives a real-time event from the user's phone (native app RPC, or the legacy
// loopback endpoint) and runs a proactive
...
// loopback /api/event/ingest legacy path and the authenticated miniapp.event.ingest
// native path. The phone forwards broadly; the gateway does the per-type judgment +
// relay. Fire-and-forget — the caller only needs to know the event was accepted; the
// report arrives later via the proactive push, exactly like a cron run.

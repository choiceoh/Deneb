...
3. 진짜로 새롭거나 바뀐 사실이 있으면 wiki(action=write)로 본문에 반영합니다:
   ...
   - kinds(특성): 비어 있으면 2단 체계 "1차/2차"로 기입하세요 — 시공(토지/루프탑/수상/ESS — ESS 사업은 시공), 기자재(모듈/인버터/케이블/기타), 풍력(육상/해상), 개발, 기타(용역/협력). 2차가 불명확하면 1차만, 1차만 있는 기존 값에 2차가 확인되면 세분화(예: 시공 → 시공/루프탑). 명백한 것만(추측 금지)

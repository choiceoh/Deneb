...
// New constructs a multi-model consultation panel.
func New(registry *modelrole.Registry, logger *slog.Logger) *Panel {
	return &Panel{modelRegistry: registry, logger: logger}
}
...
func (s *Panel) panelHealthyModels(ctx context.Context) []string {
	reg := s.modelRegistry
	if reg == nil {
		return nil
	}

...
//     is harmless.
//
// Called from InitToolsAndDeps after the spillover store is wired on
// m.ToolDeps (see chat_pipeline.go); the returned unsubscribe handle is
// stored on the Manager and invoked on shutdown by the composition root.
func (m *Manager) initSpilloverLifecycle(store *agent.SpilloverStore) {
	if store == nil || m.Sessions == nil {
		return
	}
	logger := m.Host.Logger()
	m.SpilloverLifecycleUnsub = m.Sessions.EventBusRef().Subscribe(func(e session.Event) {
...
// trigger cleanup. See serverauto's checkpoint lifecycle for the full rationale.

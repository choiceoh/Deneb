...
// InitModelRegistry initializes the model role registry and session-thinking
// defaults, wires the mail-owned wiki store into chatCfg, and (when a wiki
// store is available) builds the wiki dreamer — handed to serverauto via
// Host.SetWikiDreamer since AutonomousSubsystem owns its lifecycle. The
// mail/wiki stores themselves are created by servermail.Manager.InitMemory,
// which the composition root runs before this.
func (m *Manager) InitModelRegistry(chatCfg *chat.HandlerConfig, regPtr **modelrole.Registry) {
	logger := m.Host.Logger()

	// Model role registry.
	chatCfg.DefaultModel = configresolve.DefaultModel(logger)
	chatCfg.SubagentDefaultModel = configresolve.SubagentDefaultModel(logger)
	...
	*regPtr = reg
	...
	if defaults := configresolve.SessionThinkingDefaults(logger); defaults.ThinkingLevel != "" || defaults.InterleavedThinking != nil {
		m.Sessions.SetSessionDefaults(defaults)
		interleaved := false
		if defaults.InterleavedThinking != nil {
			interleaved = *defaults.InterleavedThinking
		}
		logger.Info("session thinking defaults",
			"level", defaults.ThinkingLevel,
			"interleaved", interleaved)
	}

	// Wiki knowledge base (store itself opened by servermail.Manager.InitMemory,
	// which the composition root runs before this).
	wikiStore := m.Mail.WikiStore
	if wikiStore != nil {
		...
		if lwClient != nil && lwModel != "" {
			dreamer := wiki.NewWikiDreamer(wikiStore, lwClient, lwModel, wiki.ConfigFromEnv(), logger)
			...
			if cs := m.Mail.ContactsStore; cs != nil {
				dreamer.SetPersonDirectory(func() []wiki.PersonSeed {
					all := cs.All()
			...
			m.Host.SetWikiDreamer(dreamer)
			logger.Info("wiki-dream: enabled")
		}
	}
}

// InitToolsAndDeps builds CoreToolDeps, registers core/plugin tools,
// and stores toolDeps on the manager.
func (m *Manager) InitToolsAndDeps(chatCfg *chat.HandlerConfig, reg *modelrole.Registry, transcriptStore chat.TranscriptStore, agentLogWriter *agentlog.Writer) {
	logger := m.Host.Logger()
	...
	// notebook tool (nil store), it does not block chat init. Owned by
	// servermail (the mail pipeline's fileDealFromMail auto-pins deal evidence).
	notebookDir := filepath.Join(config.ResolveStateDir(), "notebooks")
	if ns, err := notebook.NewStore(notebookDir); err != nil {
		logger.Warn("notebook store unavailable", "error", err)
	} else {
		m.Mail.NotebookStore = ns
		logger.Info("notebook store enabled", "dir", notebookDir)
	}
	notebookStore := m.Mail.NotebookStore
	...
	m.ToolDeps = &chat.CoreToolDeps{
		...
		Wiki: chat.WikiDeps{
			...
			Contacts: m.Mail.ContactsStore,
		},
		MailStore: m.Mail.MailStore,
		...
		Calendar: chat.CalendarDeps{
			...
			Local: serverwire.ResolveLocalCalendar(logger),
		},
		...
		ConsultPanel: modelpanel.New(m.ModelRegistry, logger).Consult,
		AgentLog:     agentLogWriter,
		LogCapture:   m.Host.LogCapture(),
		WorkFeed:     m.Mail.WorkFeedStore,
		...
		PhoneActionSender: m.Mail.DispatchPhoneAction,
		// Fleet management: the agent's twin of the /api/v1/fleet passthrough —
		// reaches the same SparkFleet control plane via Host.Fleet()'s accessors,
		// so "is the fleet ok / restart qwen36" works from chat. "" base = off.
		Fleet: chat.FleetDeps{BaseURL: m.Host.Fleet().BaseURL, Token: m.Host.Fleet().Token},
		...
		AsrHotwords: func() string {
			...
			if c := m.Mail.ContactsStore; c != nil {
	...
	if mc := m.Host.MarketCache(); mc != nil {
		m.ToolDeps.MarketSummary = mc.Summary
	}
	...
	if nw := m.Mail.NativeWorkFeedStore(); nw != nil {
		m.ToolDeps.WorkFeedRW = nw
	}
	...
	chatCfg.Ambient.CalendarGlance = chat.NewCalendarGlanceFunc(&m.ToolDeps.Calendar)
	...
	chatCfg.Ambient.PersonaOverride = m.Host.PersonaOverride
	...
	// spill files out of the production store.
	{
		...
		m.ToolDeps.SpilloverStore = spillStore
		m.initSpilloverLifecycle(spillStore)
	}

	// Semantic (vector) file search: opens the shared file store + BGE-M3 index
	// and wires m.ToolDeps.FilesSemanticSearch. Must run before RegisterCoreTools
	...
	m.FileSemindex = filesemindex.New(serverwire.LocalFileStoreOrNil(logger), m.EmbeddingClient, logger)
	if m.FileSemindex != nil {
		m.ToolDeps.FilesSemanticSearch = m.FileSemindex.Search
	}

	// Core tools (file I/O, exec, process, sessions, gateway, cron, image).
	chat.RegisterCoreTools(chatCfg.Tools, m.ToolDeps)
	...
	m.ExternalMCP = externalmcp.Start(m.Host.ShutdownCtx(), chatCfg.Tools, logger)
	...
	m.ChatToolRegistry = chatCfg.Tools
	...
	if m.FileSemindex != nil {
		filesAdapter = m.FileSemindex.KnowledgeAdapter()
	}
	knowledgeRouter := knowledge.New(
		knowledge.NewWikiAdapter(m.Mail.WikiStore),
		filesAdapter,
	)
	...
	if filesAdapter != nil {
		chatCfg.Memory.FileRecall = m.FileSemindex.Recall
	}
	...
	if bridge, ok := transcriptStore.(*polaris.Bridge); ok {
		...
		if dreamer := m.Host.WikiDreamer(); dreamer != nil {
			...
			dreamer.SetPolarisContextFn(func() string {

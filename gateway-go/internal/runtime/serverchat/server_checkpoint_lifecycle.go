...
//
// Called from configureSessionChatHandler after SetCheckpointRoot; the
// returned unsubscribe handle is stored on the Manager and invoked on
// shutdown by the composition root.
func (m *Manager) initCheckpointLifecycle(root string) {
	if root == "" || m.Sessions == nil {
		return
	}
	logger := m.Host.Logger()
	m.CheckpointLifecycleUnsub = m.Sessions.EventBusRef().Subscribe(func(e session.Event) {

...
func LoadProviderConfigs(logger *slog.Logger) map[string]modelport.ProviderConfig {
	...
	var root struct {
		Models struct {
			Providers map[string]modelport.ProviderConfig `json:"providers"`
		} `json:"models"`
	}
...
// convertRoutingConfig maps the deneb.json routing block (modelport JSON shape)
...
func convertRoutingConfig(r *modelport.RoutingConfig) *modelrole.RoutingOverride {
	if r == nil {
		return nil
	}
...
// TinyFallbackModel reads the optional agents.tinyFallbackModel override from
// deneb.json — the tiny role's own first fallback, tried before lightweight.
// Empty leaves RoleTinyFallback absent and tiny's chain unchanged.
func TinyFallbackModel(logger *slog.Logger) string {

...
func (s *Controller) miniappModelSnapshot(ctx context.Context) miniappModelSnapshot {
	...
	if s.roleHealthVerdicts != nil {
		roleVerdicts = s.roleHealthVerdicts()
	}
	return miniappModelSnapshot{
		sections: sections,
		health:   buildMiniappModelHealth(sections, probes, roleVerdicts),
	}
}

func (s *Controller) miniappModelSections(ctx context.Context) []modelSection {
	roles := registryRoleEntries(s.modelRegistry, s.currentMiniappModel())
	providers := appendBuiltinProviders(loadConfiguredProviders())
	discovered := s.discoverMiniappLocalModels(ctx, providers)
	for i := range providers {
...
func (s *Controller) miniappModelHealthProbes(
	ctx context.Context,
...
func miniappModelHealthForEntry(entry modelEntry, probes map[string]providerModelProbe, roleVerdicts map[string]string) string {
	...
	if roleVerdicts[entry.provider] == miniappModelHealthAuth {
...
func (s *Controller) discoverMiniappLocalModels(ctx context.Context, providers []providerSpec) map[string][]string {

...
// Implements serverport.Host's PostLowConfidenceEvolveCard surface.
func (m *Manager) PostLowConfidenceEvolveCard(result genesis.EvolveResult) {
...
의도한 개선이면 확정하고, 실제로 부적절하면 즉시 이전 버전으로 되돌리세요. 결정은 판정자 공진화(P3)의 실제 라벨로 축적됩니다.`,
	...
	if _, err := nf.Append(item); err != nil {
		m.Host.Logger().Warn("low-confidence evolve 카드 생성 실패", "skill", result.SkillName, "error", err)
	}
}
...
func (m *Manager) handleEvolveVerdictAction(item workfeed.Item, actionID string) error {
	tracker := m.Host.GenesisTracker()
	evolver := m.Host.GenesisEvolver()
	if tracker == nil || evolver == nil {
	...
	if skill == "" || version == "" || decisionID == "" || err != nil {
		m.Host.Logger().Warn("invalid low-confidence evolve verdict card", "ref", item.RefID)
		return fmt.Errorf("invalid low-confidence evolve verdict card")
	}
	if _, settled := tracker.OperatorJudgeVerdictByDecisionID(decisionID); settled {
		return nil
	}
	verdict := genesis.OperatorJudgeVerdictConfirm
	if actionID == evolveVerdictRollback {
		catalog := m.Host.SkillCatalog()
		if catalog == nil {
			return fmt.Errorf("skill catalog unavailable")
		}
		entry, ok := catalog.Get(skill)
		if !ok || entry.Skill.Version != version {
			m.Host.Logger().Warn("stale evolve rollback verdict ignored", "skill", skill, "cardVersion", version)
			return fmt.Errorf("stale evolve rollback verdict for %s@%s", skill, version)
		}
		if !evolver.RollbackSkillWithResult(skill) {
			m.Host.Logger().Warn("operator evolve rollback failed", "skill", skill, "version", version)
	...
	if err := tracker.LogOperatorJudgeVerdict(genesis.OperatorJudgeVerdict{
		...
		m.Host.Logger().Warn("operator judge verdict ledger write failed", "skill", skill, "error", err)
...
// Implements serverport.Host's PostMetaProposalCard surface.
func (m *Manager) PostMetaProposalCard(artifact, epoch, reason, path string, adopted bool) {
	nf := m.NativeWorkFeedStore()
	if nf == nil {
		return
	}
...
아래 버튼으로 채택하거나 기각하세요 — 결정은 메타 경험 원장에 기록되어 다음 사이클이 읽습니다.`,
	...
	if _, err := nf.Append(item); err != nil {
		m.Host.Logger().Warn("meta proposal 카드 생성 실패", "artifact", artifact, "error", err)
	}
}

// PostDriftFreezeCard notifies the operator that the evolution self-brake
// engaged or released (auto-adopt freeze transition). Implements
// serverport.Host's PostDriftFreezeCard surface.
func (m *Manager) PostDriftFreezeCard(frozen bool, reasons []string) {
	nf := m.NativeWorkFeedStore()
	if nf == nil {
		return
	}
...
궤적이 회복되면(다음 사이클 재감사) 자동으로 해제됩니다. 즉시 해제하려면 ~/.deneb/data/auto_adopt_freeze.json 을 지우세요.`, signalList)
	...
	if _, err := nf.Append(workfeed.Item{
		...
		m.Host.Logger().Warn("drift freeze 카드 생성 실패", "frozen", frozen, "error", err)
	}
}

// PostMetaRevertedCard notifies the operator that the meta rollback watch
// reverted an adoption. Informational — no decision required. Implements
// serverport.Host's PostMetaRevertedCard surface.
func (m *Manager) PostMetaRevertedCard(artifact, reason string) {
	nf := m.NativeWorkFeedStore()
	if nf == nil {
		return
	}
...
복원 결정은 메타 경험 원장에 기록되어 다음 사이클이 같은 방향을 재제안하지 않습니다.`, artifact, reason),
		...
		m.Host.Logger().Warn("meta reverted 카드 생성 실패", "artifact", artifact, "error", err)
	}
}
...
func (m *Manager) HandleMetaProposalAction(item workfeed.Item, actionID string) {
	meta := m.Host.GenesisMeta()
	tracker := m.Host.GenesisTracker()
	if meta == nil || tracker == nil {
		return
	}
	...
	fromVersion := meta.Version(artifact, fallback)
	switch actionID {
	case metaProposalActionAdopt:
		toVersion, err := meta.AdoptProposal(artifact)
		if err != nil {
			m.Host.Logger().Warn("meta proposal 채택 실패", "artifact", artifact, "error", err)
			return
		}
		...
		// feedback, #3459).
		eh := tracker.EvolutionHealth()
		if err := tracker.LogMetaRevision(genesis.MetaRevisionRecord{
			Artifact:    artifact,
			...
			m.Host.Logger().Warn("meta adoption ledger write failed", "artifact", artifact, "error", err)
		}
		m.Host.Logger().Info("meta proposal adopted from feed card", "artifact", artifact, "from", fromVersion, "to", toVersion)
	case metaProposalActionRevert:
		toVersion, err := meta.RevertAdoption(artifact)
		if err != nil {
			m.Host.Logger().Warn("meta proposal 되돌리기 실패", "artifact", artifact, "error", err)
			return
		}
		if err := tracker.LogMetaRevision(genesis.MetaRevisionRecord{
			Artifact:    artifact,
			...
			m.Host.Logger().Warn("meta revert ledger write failed", "artifact", artifact, "error", err)
		}
		m.Host.Logger().Info("meta adoption reverted from feed card", "artifact", artifact, "restored", toVersion)
	case metaProposalActionReject:
		if err := meta.RejectProposal(artifact); err != nil {
			m.Host.Logger().Warn("meta proposal 기각 실패", "artifact", artifact, "error", err)
			return
		}
		if err := tracker.LogMetaRevision(genesis.MetaRevisionRecord{
			Artifact:    artifact,
			...
			m.Host.Logger().Warn("meta rejection ledger write failed", "artifact", artifact, "error", err)
		}
		m.Host.Logger().Info("meta proposal rejected from feed card", "artifact", artifact)
	}
}

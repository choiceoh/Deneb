...
// Host.ModelRegistry, Host.ProactiveRelay) is fine — by the time a user triggers
// a merge, all are wired, even though MemoryDeps is assembled in the early phase.
func (m *Manager) MakeWikiMergeStarter(hub *rpcutil.GatewayHub) func(targetPath, sourcePath string, target, source *wiki.Page) {
	return func(targetPath, sourcePath string, target, source *wiki.Page) {
		safego.GoWithSlog(m.Host.Logger(), "miniapp-wiki-merge", func() {
			ctx, cancel := context.WithTimeout(m.Host.ShutdownCtx(), wikiMergeJobTimeout)
			defer cancel()

			store := hub.WikiStore()
			if store == nil {
				m.notifyMergeResult(ctx, "⚠️ 프로젝트 병합 실패\n위키 저장소를 사용할 수 없습니다.")
				return
			}
			...
			body, usedLLM := m.synthesizeMergeBody(ctx, target, source)

			res, err := store.MergePage(targetPath, sourcePath, body, wiki.MergeOptions{})
			if err != nil {
				m.Host.Logger().Error("miniapp wiki merge failed",
					"target", targetPath, "source", sourcePath, "error", err)
				m.notifyMergeResult(ctx, fmt.Sprintf("⚠️ 프로젝트 병합 실패\n「%s」 → 「%s」\n%v",
...
func (m *Manager) synthesizeMergeBody(ctx context.Context, target, source *wiki.Page) (string, bool) {
	concat := concatMergeBody(target, source)
	reg := m.Host.ModelRegistry()
	if reg == nil {
		return concat, false
	}
	...
	if err != nil || strings.TrimSpace(out) == "" {
		if err != nil {
			m.Host.Logger().Warn("wiki merge: model synthesis failed, using concatenation", "error", err)
		}
		return concat, false
	}
...
func (m *Manager) notifyMergeResult(ctx context.Context, message string) {
	if _, err := m.Host.ProactiveRelay().Relay(ctx, proactive.NativeWorkSessionKey, message); err != nil {
		m.Host.Logger().Error("wiki merge: completion notify failed", "error", err)
	}
}

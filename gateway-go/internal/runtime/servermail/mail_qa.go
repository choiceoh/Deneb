...
// nil when the chat handler isn't ready, in which case the handler skips
// registering miniapp.gmail.ask entirely.
func (m *Manager) MakeMailQAAsk() func(context.Context, string, []handlermail.QATurn, string) (string, error) {
	...
	return func(ctx context.Context, mailContext string, history []handlermail.QATurn, question string) (string, error) {
		...
		res, err := chatHandler.SendSync(ctx, "mail-qa:"+shortid.New("m"), "", "", &chat.SyncOptions{
			Messages:           msgs,

...
func (m *Manager) MiniappMailClientFactory(denebDir string) func() (handlermail.GmailClient, error) {
	return func() (handlermail.GmailClient, error) {
		client, err := m.newMiniappMailClient(denebDir)
		if err != nil {
			return nil, err
		}
		return client, nil
...
func (m *Manager) newMiniappMailClient(denebDir string) (handlermail.GmailClient, error) {
	// Native-archive-only — no Gmail fallback (see errNativeMailUnconfigured).
	if repo := m.newArchiveMailRepository(denebDir, nil); repo != nil {
		return repo, nil
	}
	return nil, errNativeMailUnconfigured
}

func (m *Manager) NewMiniappMailAttachmentClient() (gatewayhttp.MailAttachmentClient, error) {
	if repo := m.newArchiveMailRepository(m.Host.DenebDir(), nil); repo != nil {
		return repo, nil
	}
	return nil, errNativeMailUnconfigured
}

func (m *Manager) newArchiveMailRepository(denebDir string, fallback mailarchive.FallbackClient) *mailarchive.Repository {

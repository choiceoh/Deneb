...
func (m *Manager) NativeWorkFeedStore() *nativeWorkFeedStore {
	if m == nil || m.WorkFeedStore == nil {
		return nil
	}
	return &nativeWorkFeedStore{
		store:           m.WorkFeedStore,
		sync:            m.NativeSyncStore,
		log:             m.Host.Logger(),
		onEvolveVerdict: m.handleEvolveVerdictAction,
		onLadderAction:  m.handleLadderCardAction,
	}
}

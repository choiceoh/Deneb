...
// gmail.analyze handler invokes after a fresh LLM run. Page assembly lives in
// buildMailAnalysisPage. The returned sink no-ops when no wiki store exists.
func (m *Manager) MakeMailAnalysisWikiSink() func(handlermail.WikiAnalysisInput) error {
	return func(in handlermail.WikiAnalysisInput) error {
		if m.WikiStore == nil {
			return nil
		}
		return m.WikiStore.WritePage(mailAnalysisWikiPath(in.MsgID, in.RelatedProjects), buildMailAnalysisPage(in))
	}
}
...
func (m *Manager) WikiSenderFacts(ctx context.Context, from string) string {
	if m.WikiStore == nil {
		return ""
	}
	if email := senderEmailFromHeader(from); email != "" {
		if path := m.WikiStore.ResolvePersonByEmail(email); path != "" {
			if facts, err := m.WikiStore.PageConnections(ctx, path, 0); err == nil && facts != "" {
				return facts
			}
		}
	}
	// GraphContext strips any "<address>" itself, so the raw From is a safe query.
	facts, err := m.WikiStore.GraphContext(ctx, from, 0)
	if err != nil {
		return ""
	}
	return facts
...
func (m *Manager) ProjectCandidatesFn() func() []mailanalysis.ProjectCandidate {
...
// manual tap.
func (m *Manager) MakeMailAnalysisSink() func(*gmail.MessageDetail, mailanalysis.AnalysisResult) error {
	cacheStore := handlermail.NewAnalysisStore(filepath.Join(m.Host.DenebDir(), "cache", "mail_analysis"))
	workStore := mailwork.New(filepath.Join(m.Host.DenebDir(), "mail_work_state.json"))
	return func(msg *gmail.MessageDetail, res mailanalysis.AnalysisResult) error {
		...
		if err := cacheStore.SaveAnalysis(handlermail.CachedAnalysis{
			...
			m.Host.Logger().Warn("mail analysis cache 저장 실패", "id", msg.ID, "error", err)
		...
		if m.WikiStore != nil {
			...
			if err := m.WikiStore.WritePage(mailAnalysisWikiPath(msg.ID, res.RelatedProjects), page); err != nil {
				m.Host.Logger().Warn("mail analysis 위키 저장 실패", "id", msg.ID, "error", err)
				errs = append(errs, err)
			}
			...
			// the analysis.
			m.appendMailStatusToProjects(msg, res)
		}
		...
		m.fileDealFromMail(msg, res.Deal, res.RelatedProjects)
		...
		calendarCount := m.autoProposeCalendarFromMail(msg, res.ActionItems, res.Deal, res.Importance)
		if _, err := workStore.MarkAnalysisDone(mailwork.AnalysisInput{
			...
			m.Host.Logger().Warn("mail workflow state 저장 실패", "id", msg.ID, "error", err)
			errs = append(errs, err)
		}
...
func (m *Manager) MakeMailFeedDeliverySink() func([]string) {
	workStore := mailwork.New(filepath.Join(m.Host.DenebDir(), "mail_work_state.json"))
	return func(ids []string) {
		for _, id := range ids {
			...
			if _, err := workStore.MarkFeedCreated(id); err != nil {
				m.Host.Logger().Warn("mail workflow feed 상태 저장 실패", "id", id, "error", err)
			}
		}
	}
}

func (m *Manager) MakeMailAnalysisFailureSink() func(*gmail.MessageDetail, error) {
	workStore := mailwork.New(filepath.Join(m.Host.DenebDir(), "mail_work_state.json"))
	return func(msg *gmail.MessageDetail, err error) {
		...
		if _, werr := workStore.MarkAnalysisFailed(mailwork.MessageInput{
			...
			m.Host.Logger().Warn("mail workflow failure 상태 저장 실패", "id", msg.ID, "error", werr)
		}
	}
}
...
func (m *Manager) fileDealFromMail(msg *gmail.MessageDetail, deal *mailanalysis.DealInfo, relatedProjects []string) {
	if deal == nil || msg == nil || m.WikiStore == nil {
		return
	}
	now := time.Now()
	...
	requote := m.WikiStore.DetectRequote(input, now)
	relPath, created, err := m.WikiStore.UpsertDealPage(input, now)
	if err != nil {
		m.Host.Logger().Warn("mail→deal: 거래 페이지 저장 실패", "id", msg.ID, "counterparty", deal.Counterparty, "error", err)
		return
	}
	m.Host.Logger().Info("mail→deal: 거래 페이지 갱신", "id", msg.ID, "path", relPath, "created", created)
	...
	if requote != nil {
		...
		m.Host.Logger().Info("mail→deal: 재견적 변동 감지", "id", msg.ID, "counterparty", deal.Counterparty, "detail", line)
		for _, p := range input.RelatedProjects {
			if aerr := m.WikiStore.AppendProjectStatusLine(p, line, "requote:mail:"+msg.ID, now); aerr != nil {
				m.Host.Logger().Warn("mail→deal: 재견적 상태줄 기록 실패", "id", msg.ID, "project", p, "error", aerr)
			}
		}
	}
	...
	m.pinDealEvidenceToNotebook(msg, deal, relPath, relatedProjects)
	...
	if created {
		m.appendDealQuestionCard(deal, relPath)
	}
}
...
func (m *Manager) pinDealEvidenceToNotebook(msg *gmail.MessageDetail, deal *mailanalysis.DealInfo, dealRef string, relatedProjects []string) {
	if m.NotebookStore == nil || dealRef == "" {
		return
	}
	added, err := m.NotebookStore.PinUnique(dealRef, deal.Counterparty, notebook.Source{
	...
	if err != nil {
		m.Host.Logger().Warn("mail→notebook: 딜 증거 핀 실패", "id", msg.ID, "deal", dealRef, "error", err)
		return
	}
	if added {
		m.Host.Logger().Info("mail→notebook: 딜 증거 핀", "id", msg.ID, "deal", dealRef)
	}
	...
	if refs := directProjectPages(relatedProjects); len(refs) > 0 {
		if _, serr := m.NotebookStore.StampProjectRefs(dealRef, refs); serr != nil {
			m.Host.Logger().Warn("mail→notebook: 프로젝트 각인 실패", "id", msg.ID, "deal", dealRef, "error", serr)
		}
	}
}
...
func (m *Manager) appendMailStatusToProjects(msg *gmail.MessageDetail, res mailanalysis.AnalysisResult) {
	if m.WikiStore == nil || msg == nil {
		return
	}
	...
	for _, r := range directProjectPages(res.RelatedProjects) {
		if err := m.WikiStore.AppendProjectStatusLine(r, line, ref, now); err != nil {
			m.Host.Logger().Warn("mail→project 현재 상태 갱신 실패", "id", msg.ID, "path", r, "error", err)
		}
	}
}

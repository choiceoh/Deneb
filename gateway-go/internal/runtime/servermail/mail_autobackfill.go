...
func (m *Manager) maybeAutoBackfillMailStore(mailStoreDir string) {
	if m.MailStore == nil || mailStoreDir == "" {
		return
	}
	...
	safego.GoWithSlog(logger, "mail-autobackfill", func() {
		ctx, cancel := context.WithTimeout(m.Host.ShutdownCtx(), mailAutoBackfillTimeout)
		defer cancel()
		...
		total, err := mailarchive.FetchAllContextMessages(ctx, cfg, opts, mailAutoBackfillBatch,
			func(msg mailarchive.ContextMessage) error {
				created, perr := store.Put(msg)
				if perr != nil {
					return perr
				}
				if created {

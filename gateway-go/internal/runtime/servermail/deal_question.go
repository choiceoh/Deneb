...
func (m *Manager) appendDealQuestionCard(deal *mailanalysis.DealInfo, dealPagePath string) {
	...
	if _, err := nf.Append(workfeed.Item{
		...
		m.Host.Logger().Warn("deal question 카드 생성 실패", "counterparty", deal.Counterparty, "error", err)
	}
}
...
// settled by then).
func (m *Manager) RecordDealQuestionAnswer(item workfeed.Item, actionID string) {
	if m.WikiStore == nil {
		return
	}
	...
	if code == "none" {
		m.Host.Logger().Info("deal question: 딜 아님 응답", "path", pagePath)
		return
	}
	page, err := m.WikiStore.ReadPage(pagePath)
	if err != nil || page == nil {
		m.Host.Logger().Warn("deal question 기록: 페이지 읽기 실패", "path", pagePath, "error", err)
		return
	}
	...
	page.Body = strings.TrimRight(page.Body, "\n") +
		fmt.Sprintf("\n\n## 담당 팀\n%s (%s, 사용자 지정)\n", label, stamp)
	if err := m.WikiStore.WritePage(pagePath, page); err != nil {
		m.Host.Logger().Warn("deal question 기록 실패", "path", pagePath, "error", err)
		return
	}
	m.Host.Logger().Info("deal question: 담당 팀 기록", "path", pagePath, "team", label)
}

...
func ingestPhoneActionResult(m *Manager, eventType, source, text string) {
	logger := slog.Default()
	if m.Host != nil {
	...
	phoneevents.New(phoneevents.Config{
		...
		ResolvePhoneAction: func(res phoneevents.ActionResult) bool {
			return m.phoneActions.resolve(PhoneActionResult{ID: res.ID, OK: res.OK, Error: res.Error})
		},
...
func phoneActionTestManager() *Manager {
	return &Manager{

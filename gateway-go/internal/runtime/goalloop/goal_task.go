...
// Task advances one active standing goal per scheduler tick.
type Task = goalTask
...
func (t *goalTask) driveOne(ctx context.Context, g *goals.State) error {
	...
	if err != nil {
		...
		t.store.RecordRun(sessionKey, "continue", "run error: "+textutil.TruncateRunes(err.Error(), 200, "\n...(truncated)"), false)
		return nil
	}
...
func (t *goalTask) judge(ctx context.Context, goal string, subgoals []string, answer string) (verdict, reason string, parseFailed bool) {
	...
	goalBlock := textutil.TruncateRunes(goal, 2000, "\n...(truncated)")
	if len(subgoals) > 0 {

...
// Handler proxies an allowlisted subset of SparkFleet to the native client.
type Handler struct {
	fleet  *sparkfleet.Client
	logger *slog.Logger
}

// New creates a SparkFleet proxy handler.
func New(fleet *sparkfleet.Client, logger *slog.Logger) *Handler {
	return &Handler{fleet: fleet, logger: logger}
}
...
func PathAllowed(method, path string) bool {
	...
	case http.MethodGet:
		...
		if rest, ok := strings.CutPrefix(path, "/api/recipes/"); ok {
			...
			return found && validFleetSegment(name)
		}
	case http.MethodPost:
		...
		if rest, ok := strings.CutPrefix(path, "/api/jobs/"); ok {
			...
			return found && validFleetSegment(id)
		}
	}
	return false
}

// validFleetSegment rejects path syntax that could be reinterpreted by an
// upstream proxy after this allowlist check. In particular, dot segments and
// percent-encoded separators must never turn an allowed job/recipe lookup into
// a different SparkFleet endpoint.
func validFleetSegment(segment string) bool {
	...
	return strings.IndexFunc(segment, func(r rune) bool {
...
// Proxy forwards an already-authenticated allowlisted request to SparkFleet.
func (s *Handler) Proxy(w http.ResponseWriter, r *http.Request) {
	if s.fleet == nil {

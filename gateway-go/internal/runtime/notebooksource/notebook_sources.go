...
// FetchURL retrieves a safe HTTP source for notebook ingestion.
func FetchURL(ctx context.Context, rawURL string) (string, error) {
	return notebookFetchURL(ctx, rawURL)
}
...
// ReadMail loads a mail thread as notebook source text.
func ReadMail(ctx context.Context, threadID string) (string, error) {
	return notebookReadMail(ctx, threadID)
}
...
// ReadDiary loads one validated diary reference from the wiki store.
func ReadDiary(store *wiki.Store, ref string) (string, error) {
	return notebookReadDiary(store, ref)
}

...
// CalendarClient is the minimal calendar source needed by briefing and
// post-meeting automation.
type CalendarClient = briefingCalendarClient
...
// ResolveCalendarClient merges the configured Google and local calendar
// sources, falling back cleanly when either side is unavailable.
func ResolveCalendarClient() (CalendarClient, error) {
	return resolveBriefingCalendarClient()
}
...
// CalendarBriefingService polls for upcoming events and delivers concise,
// optionally enriched meeting briefings.
type CalendarBriefingService = calendarBriefingService
...
// NewCalendarBriefingService constructs the runtime calendar briefing worker.
func NewCalendarBriefingService(
	deliver func(text string) (bool, error),
	resolve func() (CalendarClient, error),
	logger *slog.Logger,
) *CalendarBriefingService {
	return newCalendarBriefingService(deliver, resolve, logger)
}

// EnableEnrichment attaches best-effort wiki and mail context to briefings.
func (s *calendarBriefingService) EnableEnrichment(
	wikiGetter func() *wiki.Store,
	logger *slog.Logger,
) {
	if s != nil {
		s.enricher = newBriefingEnricher(wikiGetter, logger)
	}
}

...
func (s *RunMarkerStore) Write(m session.RunMarker) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *RunMarkerStore) Read(sessionKey string) (*session.RunMarker, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.readLocked(sessionKey)
}

func (s *RunMarkerStore) readLocked(sessionKey string) (*session.RunMarker, error) {
	path := s.pathFor(sessionKey)
...
func (s *RunMarkerStore) List() ([]session.RunMarker, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *RunMarkerStore) writeLocked(m session.RunMarker) error {
	if m.SessionKey == "" {

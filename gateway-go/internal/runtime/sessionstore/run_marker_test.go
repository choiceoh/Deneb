...
func TestRunMarkerStore_ListReturnsAllWrittenMarkers(t *testing.T) {
	...
	if err := store.Write(session.RunMarker{SessionKey: "telegram:1", StartedAt: 100}); err != nil {
		t.Fatal(err)
	}
	if err := store.Write(session.RunMarker{SessionKey: "telegram:2", StartedAt: 200}); err != nil {
		t.Fatal(err)
	}
...
func TestRunMarkerStore_ListIgnoresCorruptFiles(t *testing.T) {
	...
	// Good marker.
	if err := store.Write(session.RunMarker{SessionKey: "telegram:good", StartedAt: 1}); err != nil {
		t.Fatal(err)
	}
...
func TestRunMarkerStore_IncrementResumeAttemptsUpdatesPersistedCount(t *testing.T) {
	...
	if err := store.Write(session.RunMarker{SessionKey: "telegram:42", StartedAt: 1, ResumeAttempts: 0}); err != nil {
		t.Fatal(err)
	}
...
func TestRunMarkerStore_TouchUpdatesLastActivityAt(t *testing.T) {
	...
	original := int64(100)
	if err := store.Write(session.RunMarker{SessionKey: "telegram:1", StartedAt: 1, LastActivityAt: original}); err != nil {
		t.Fatal(err)
	}
...
func TestRunMarkerStore_SanitizeKeyNormalizesPathTraversal(t *testing.T) {
	...
	nasty := "../../etc/passwd"
	if err := store.Write(session.RunMarker{SessionKey: nasty, StartedAt: 1}); err != nil {

...
// CalendarProposalsFromMail derives conservative calendar proposals from one
// analyzed mail without mutating the calendar.
func CalendarProposalsFromMail(msgID, subject, from string, docs []string, items []mailanalysis.ActionItem, deal *mailanalysis.DealInfo, importance string, now time.Time) []calprop.CreateInput {
	return calendarProposalsFromMail(msgID, subject, from, docs, items, deal, importance, now)
}
...
// DocumentAttachmentNames returns schedule-relevant attachment names.
func DocumentAttachmentNames(atts []gmail.AttachmentInfo) []string {
	return documentAttachmentNames(atts)
}

...
- `miniapp_bridge.go`의 `MiniappMethods`, `DefaultSessionKey`가 native-client
...
- 빈 native session key는 blocking/streaming 모두 `client:main`으로 정규화해야
  한다. RPC 오류는 `rpcerr` code와 원인 chain을 보존한다.

...
// fakeGmailClient implements GmailClient with function fields so
// gmail_context_test.go can wire up exactly the behavior it needs.
// Deliberately duplicated from the gmailops/analyzebind leaf packages'
// test helper of the same name — Go test files aren't importable across
// packages, and sender_context only needs Search, so the fake stays small.
...
func (f *fakeGmailClient) GetMessage(context.Context, string) (*gmail.MessageDetail, error) {

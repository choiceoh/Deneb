...
func cronsGet(deps CronsDeps) rpcutil.HandlerFunc {
	...
	return minibind.BindOptional[params](func(ctx context.Context, req *protocol.RequestFrame, p params) *protocol.ResponseFrame {
		_, job, errResp := resolveCronJob(deps, p.ID, req.ID)
		if errResp != nil {
			return errResp
		}
...
func cronsUpdate(deps CronsDeps) rpcutil.HandlerFunc {
	...
	return minibind.BindOptional[params](func(ctx context.Context, req *protocol.RequestFrame, p params) *protocol.ResponseFrame {
		svc, existing, errResp := resolveCronJob(deps, p.ID, req.ID)
		if errResp != nil {
			return errResp
		}
...
func cronsRun(deps CronsDeps) rpcutil.HandlerFunc {
	...
	return minibind.BindOptional[params](func(ctx context.Context, req *protocol.RequestFrame, p params) *protocol.ResponseFrame {
		svc, _, errResp := resolveCronJob(deps, p.ID, req.ID)
		if errResp != nil {
			return errResp
		}
...
func resolveCronJob(deps CronsDeps, id, reqID string) (CronService, *cron.StoreJob, *protocol.ResponseFrame) {
	if id == "" {
		return nil, nil, rpcerr.MissingParam("id").Response(reqID)
	}
	service, err := deps.Service()
	if err != nil {
		return nil, nil, rpcerr.WrapUnavailable("cron service unavailable", err).Response(reqID)
	}
	job := service.Job(id)
	if job == nil {
		return nil, nil, rpcerr.NotFound("cron job " + rpcutil.TruncateForError(id)).Response(reqID)
	}
...
func miniappRowFromJob(j cron.StoreJob) MiniappCronRow {
	row := MiniappCronRow{
		...
		Schedule:          formatCronSchedule(j.Schedule),
		PayloadKind:       j.Payload.Kind,
		PayloadPreview:    textutil.TruncateRunes(payloadPreviewText(j.Payload), maxCronPayloadPreview, "…"),
		NextRunAtMs:       j.State.NextRunAtMs,
		ConsecutiveErrors: j.State.ConsecutiveErrors,
		AutoDisabledAtMs:  j.State.AutoDisabledAtMs,
		LastError:         j.State.LastDeliveryError,

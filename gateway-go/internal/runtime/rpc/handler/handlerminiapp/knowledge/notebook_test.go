...
// New notebooks default to soft grounding: mode is omitted (omitempty) from
// get. (The set_mode RPC was removed — strictness is toggled through the chat
// `notebook` tool's "mode" action.)
func TestNotebookDefaultsToSoftGrounding(t *testing.T) {
	m := notebookTestMethods(t)
	created := decodePayload(t, callNotebook(t, m, "miniapp.notebook.create", map[string]any{"name": "딜"}))
	id, _ := created["id"].(string)
	if id == "" {

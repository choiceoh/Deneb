...
const PushKindWorkfeed = pushKindWorkfeed

// PushKindFleet opens the fleet pane from a native push.
const PushKindFleet = pushKindFleet
...
const (
	...
	KindMobile = kindMobile
	// KindDesktop identifies the desktop client.
	KindDesktop = kindDesktop
)
...
func ClientKindFromHeader(v string) ClientKind { return clientKindFromHeader(v) }
...
func NewHub() *Hub { return newClientPushHub() }
...
func (h *clientPushHub) Subscribe(kind ClientKind) (<-chan Event, func()) {
	return h.subscribe(kind)
}
...
func (h *clientPushHub) Publish(ev Event) { h.publish(ev) }
...
func (h *clientPushHub) SubscriberCount() int { return h.subscriberCount() }
...
// PublishWithFallback delivers through SSE and falls back to FCM when no
// mobile subscriber is connected.
func PublishWithFallback(hub *Hub, fcm *push.Notifier, ev Event) {
	publishProactive(hub, fcm, ev)
}

...
func Start(shutdownCtx context.Context, registry *chat.ToolRegistry, logger *slog.Logger) map[string]*mcpclient.Client {
	...
	if err != nil {
		logger.Error("external mcp: invalid "+mcpServersEnv, "error", err)
		return nil
	}
	clients := make(map[string]*mcpclient.Client, len(specs))
	for _, spec := range specs {
		// ShutdownCtx as process lifetime: children die with the gateway.
		client, err := mcpclient.New(shutdownCtx, strings.Fields(spec.Cmdline), logger)
		if err != nil {
			logger.Error("external mcp: invalid command", "server", spec.Name, "error", err)
			continue
		}
		clients[spec.Name] = client
		safego.GoWithSlog(logger, "mcp-init-"+spec.Name, func() {
			ctx, cancel := context.WithTimeout(shutdownCtx, mcpInitTimeout)
			defer cancel()
			...
			if err != nil {
				...
				// backoff path.
				logger.Error("external mcp: tool discovery failed (check 'mcp server stderr' lines for auth instructions)",
					"server", spec.Name, "error", err)
				client.CloseProcess()
				return
			}
			if len(tools) == 0 {
				logger.Warn("external mcp: server reported no tools", "server", spec.Name)
				client.CloseProcess()
				return
			}
			names := registerMCPServerTools(registry, spec, tools, client.CallTool)
			logger.Info("external mcp tools registered (deferred, activate via fetch_tools)",

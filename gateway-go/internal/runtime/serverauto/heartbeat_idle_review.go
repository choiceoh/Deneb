...
// process owns the production state dir (serverport.ProductionStateDir — the
// same invariant memory-backup gates on). A dev/live-test instance
...
func (m *Manager) idleSkillReviewLaneIfProduction(homeDir string) func(ctx context.Context) (bool, string) {
	logger := m.Host.Logger()
	if _, prod := serverport.ProductionStateDir(homeDir); !prod {
		logger.Info("idle skill review lane disabled: non-production state dir")
		return nil
	}
	return m.newIdleSkillReviewLane()
}
...
func (m *Manager) newIdleSkillReviewLane() func(ctx context.Context) (bool, string) {
	logger := m.Host.Logger()
	staleAfter := idleReviewStaleAfterFromEnv(logger)
	var lastAttempt time.Time
	return func(_ context.Context) (bool, string) {
		tracker, nudger, store := m.GenesisTracker, m.GenesisNudger, m.GenesisTranscripts
		...
		if err != nil {
			...
			if errors.Is(err, fs.ErrNotExist) {
				logger.Debug("idle skill review: transcript dir absent", "dir", transcriptBaseDir())
			} else {
				logger.Warn("idle skill review: transcript dir unreadable", "dir", transcriptBaseDir(), "error", err)
			}
			return false, ""
		}
		for _, key := range keys {
			...
			if err != nil {
				...
				logger.Warn("idle skill review: fenced review failed", "session", key, "error", err)
				return false, ""
			}
			if fired {
		...
		logger.Debug("idle skill review: no recent session passed the review gate", "staleFor", staleFor)
		return false, ""
	}
}

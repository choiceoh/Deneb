...
const (
	...
	// defaultTurnDeadline mirrors server.DefaultTurnDeadline (the end-to-end
	// budget for one user turn) — duplicated here as a plain constant to
	// avoid a composition-root import cycle for a single time.Duration value.
	defaultTurnDeadline = 5 * time.Minute
)

// runMarkerStore returns the manager's marker store, lazily constructing
// it on first use. The store lives under <denebDir>/run_markers/.
func (m *Manager) runMarkerStore() *sessionstore.RunMarkerStore {
	m.resumeMu.Lock()
	defer m.resumeMu.Unlock()
	if m.markerStore == nil {
		base := filepath.Join(m.Host.DenebDir(), "run_markers")
		m.markerStore = sessionstore.NewRunMarkerStore(base)
	}
...
func (m *Manager) InitRunMarkerLifecycle() func() {
	sessions := m.Host.Sessions()
	if sessions == nil {
		return func() {}
	}
	store := m.runMarkerStore()
	logger := m.Host.Logger()
	return sessions.EventBusRef().Subscribe(func(e session.Event) {
		switch e.Kind {
		case session.EventStatusChanged:
			switch {
			case e.NewStatus == session.StatusRunning:
				// Guard: only persistent direct sessions get markers.
				sess := sessions.Get(e.Key)
				if sess == nil || sess.Kind != session.KindDirect {
					return
				}
				now := time.Now().UnixMilli()
				marker := session.RunMarker{
					SessionKey:     e.Key,
					StartedAt:      now,
					LastActivityAt: now,
...
func (m *Manager) AutoResumeInterruptedRuns(ctx context.Context) {
	opts := autoResumeOptions{
		Enabled:     autoResumeEnabled(),
		MaxAge:      resumeMaxAge,
		MaxAttempts: resumeMaxAttempts,
		Now:         time.Now,
		DispatchFn:  m.dispatchResumeMessage,
	}
	m.autoResumeInterruptedRunsWithOpts(ctx, opts)
}

// autoResumeInterruptedRunsWithOpts is the testable entry point.
func (m *Manager) autoResumeInterruptedRunsWithOpts(ctx context.Context, opts autoResumeOptions) {
	logger := m.Host.Logger()
	store := m.runMarkerStore()
	...
	if !opts.Enabled {
		...
		if markers, _ := store.List(); len(markers) > 0 {
			for _, marker := range markers {
				_ = store.Delete(marker.SessionKey)
			}
		}
		return
	}
	...
	for _, marker := range markers {
		...
		channel, ok := m.Host.ResumableSessionForMarker(marker.SessionKey)
		if !ok {
		...
		if _, incErr := store.IncrementResumeAttempts(marker.SessionKey); incErr != nil {
			logger.Warn("auto-resume: failed to increment attempts, skipping",
				"session", marker.SessionKey, "error", incErr)
			continue
		}
		...
		safego.GoWithSlog(logger, "auto-resume-dispatch", func() {
			...
			if err := opts.DispatchFn(ctx, markerCopy.SessionKey, channelCopy, ""); err != nil {
...
func (m *Manager) dispatchResumeMessage(ctx context.Context, sessionKey, channel, to string) error {
	...
	defer cancel()
	resp := chatHandler.Send(sendCtx, req)
	if resp != nil && !resp.OK {
		if resp.Error != nil {

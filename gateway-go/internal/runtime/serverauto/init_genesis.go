...
// Called after the chat handler is created but BEFORE registerLateMethods, so
// the RPC methods can be registered in method_registry.go (Rule 1 compliance).
//
...
func (m *Manager) InitGenesisServices() {
	logger := m.Host.Logger()
	chatHandler := m.Host.ChatHandler()
	modelRegistry := m.Host.ModelRegistry()
	if chatHandler == nil || modelRegistry == nil {
		logger.Debug("genesis: skipped (chat handler or model registry unavailable)")
		return
	}

	lwClient := modelRegistry.Client(modelrole.RoleLightweight)
	lwModel := modelRegistry.Model(modelrole.RoleLightweight)
	if lwClient == nil || lwModel == "" {
		logger.Debug("genesis: skipped (lightweight model not configured)")
		return
	}

	workspaceDir := ""
	if toolDeps := m.Host.ToolDeps(); toolDeps != nil {
		workspaceDir = toolDeps.WorkspaceDir
	}
	thinkingKwargs := m.genesisThinkingKwargs()
	mainClient := modelRegistry.Client(modelrole.RoleMain)
	mainModel := modelRegistry.Model(modelrole.RoleMain)
	...
	bundle := skilllifecycle.BuildCore(skilllifecycle.CoreBuildInput{
		...
		LowConfidenceObserver: m.Host.PostLowConfidenceEvolveCard,
		ConfigureEvolver: func(evolver *skilllifecycle.Evolver) (string, string) {
			evolverRole, evolverModel = m.configureGenesisEvolverModels(evolver)
	...
	m.SkillCatalog = bundle.Catalog
	m.GenesisSvc = bundle.Service
	m.GenesisTracker = bundle.Tracker
	m.GenesisEvolver = bundle.Evolver
	m.GenesisMeta = bundle.Meta
	cfg := bundle.Config
	...
	reviewModel := modelRegistry.FullModelID(modelrole.RoleCoding)
	if reviewModel == "" {
		reviewModel = modelRegistry.FullModelID(modelrole.RoleLightweight)
	}
	reviewFork := skilllifecycle.NewReviewFork(chatHandler, m.GenesisTranscripts, m.GenesisTracker, reviewModel, logger)
	m.GenesisNudger = skilllifecycle.NewNudgerFromEnvWithTrackerAndReviewer(
		m.GenesisSvc,
		m.GenesisTracker,
		reviewFork,
		logger,
	...
	m.GenesisNudger.SetShutdownContext(m.Host.ShutdownCtx())
	...
	_, nudgerProdState := serverport.ProductionStateDir(nudgerHome)
	if !nudgerProdState && os.Getenv("DENEB_SKILL_NUDGE_INTERVAL") == "" {
		logger.Info("genesis: skill nudger disabled (non-production state dir; set DENEB_SKILL_NUDGE_INTERVAL to force)")
	} else if chatHandler != nil && m.GenesisNudger.Enabled() {
		chatHandler.SetSkillNudger(skilllifecycle.NewChatNudgerAdapter(m.GenesisNudger))
	}
	...
	if chatHandler != nil && m.GenesisTracker != nil {
		chatHandler.SetSkillUsageRecorder(skilllifecycle.NewChatUsageRecorder(
			m.GenesisTracker,
			m.GenesisTranscripts,
			logger,
			replayExecutorEnabled(),
		))
	}
	m.registerSkillLifecycleTool()

	logger.Info("genesis: services initialized",
		"model", lwModel, "evolverRole", evolverRole, "evolverModel", evolverModel, "outputDir", cfg.OutputDir,
		"nudgeInterval", m.GenesisNudger.Interval(),
		"minToolCalls", cfg.MinToolCalls,
		"minTurns", cfg.MinTurns,
		"maxSkillsPerDay", cfg.MaxSkillsPerDay)
}

func (m *Manager) RefreshCodingModelConsumers() {
	modelRegistry := m.Host.ModelRegistry()
	if modelRegistry == nil {
		return
	}
	logger := m.Host.Logger()
	codingModel := modelRegistry.FullModelID(modelrole.RoleCoding)
	if toolDeps := m.Host.ToolDeps(); toolDeps != nil {
		toolDeps.Sessions.CodingDefaultModel = codingModel
	}
	if m.GenesisEvolver != nil {
		role, model := m.configureGenesisEvolverModels(m.GenesisEvolver)
		logger.Info("genesis: evolver model refreshed",
			"codingModel", codingModel, "evolverRole", role, "evolverModel", model)
	}
}

func (m *Manager) configureGenesisEvolverModels(evolver *skilllifecycle.Evolver) (modelrole.Role, string) {
	modelRegistry := m.Host.ModelRegistry()
	if evolver == nil || modelRegistry == nil {
		return "", ""
	}
	evolverRole := modelrole.RoleLightweight
	evolverClient := modelRegistry.Client(modelrole.RoleLightweight)
	evolverModel := modelRegistry.Model(modelrole.RoleLightweight)
	if codingModel := modelRegistry.Model(modelrole.RoleCoding); codingModel != "" {
		if codingClient := modelRegistry.Client(modelrole.RoleCoding); codingClient != nil {
	...
	evolver.SetThinkingKwargs(m.genesisThinkingKwargs())
	...
	if replayExecutorEnabled() {
		evolver.SetReplayExecutor(
			modelRegistry.Client(modelrole.RoleLightweight),
			modelRegistry.Model(modelrole.RoleLightweight),
		)
	} else {
...
func (m *Manager) genesisThinkingKwargs() map[string]string {
	modelRegistry := m.Host.ModelRegistry()
	if modelRegistry == nil {
		return nil
	}
	...
	for _, role := range []modelrole.Role{modelrole.RoleLightweight, modelrole.RoleCoding, modelrole.RoleMain} {
		mc := modelRegistry.Config(role)
		if mc.Model == "" {
			continue
		}
		if k := modelRegistry.CapabilityForModel(mc.ProviderID, mc.Model).ThinkingToggleKwarg; k != "" {
...
func (m *Manager) registerSkillLifecycleTool() {
	chatHandler := m.Host.ChatHandler()
	if chatHandler == nil || m.GenesisSvc == nil {
		return
	}
	logger := m.Host.Logger()
	modelRegistry := m.Host.ModelRegistry()
	...
	if lwClient := modelRegistry.Client(modelrole.RoleLightweight); lwClient != nil {
		lwModel := modelRegistry.Model(modelrole.RoleLightweight)
		thinkingKwargs := m.genesisThinkingKwargs()
	...
	backend := skilllifecycle.NewBackend(skilllifecycle.BackendConfig{
		Genesis:      m.GenesisSvc,
		Evolver:      m.GenesisEvolver,
		Tracker:      m.GenesisTracker,
		Transcripts:  m.GenesisTranscripts,
		Logger:       logger,
		ShadowReplay: shadowReplay,
	})
	chatHandler.RegisterTool(toolport.ToolDef{
...
func (m *Manager) RegisterGenesisAutonomousTasks(_ *rpcutil.GatewayHub) {
	logger := m.Host.Logger()
	modelRegistry := m.Host.ModelRegistry()
	if m.GenesisSvc == nil || m.AutonomousSvc == nil {
		return
	}

	if m.GenesisTracker != nil {
		evolveTask := &genesis.EvolutionTask{
			...
			Bootstrap: func() {
				...
				if _, prod := serverport.ProductionStateDir(home); prod {
					m.GenesisMeta.MaterializeDefaults(skilllifecycle.DefaultMetaArtifacts())
				}
			},
		}
		m.AutonomousSvc.RegisterTask(evolveTask)
		...
		m.AutonomousSvc.RegisterTask(&genesis.MetaEvolutionTask{
			Evolver: m.GenesisEvolver,
			Meta:    m.GenesisMeta,
			Tracker: m.GenesisTracker,
			Logger:  logger,
			...
			OnProposal:    m.Host.PostMetaProposalCard,
			OnReverted:    m.Host.PostMetaRevertedCard,
			OnDriftFreeze: m.Host.PostDriftFreezeCard,
			...
			RuntimeHealth: m.metaRuntimeHealthEvidence,
			...
			QualityBench: m.metaQualityBenchEvidence,
			...
			GenesisGen: m.GenesisSvc.ShadowGenerate,
		})
		m.AutonomousSvc.RegisterTask(&genesis.SkillCuratorTask{
			Tracker: m.GenesisTracker,
			Logger:  logger,
		...
		m.AutonomousSvc.RegisterTask(skilllifecycle.NewValidationBackfillTask(
		...
		_, isProdState := serverport.ProductionStateDir(workoutHome)
		...
		if isProdState {
			m.AutonomousSvc.RegisterTask(&genesis.JudgeAccuracyTask{
				Evolver: m.GenesisEvolver,
				Meta:    m.GenesisMeta,
				Tracker: m.GenesisTracker,
				Logger:  logger,
			...
			m.AutonomousSvc.RegisterTask(&genesis.LadderWatchTask{
				Tracker: m.GenesisTracker,
				Logger:  logger,
				OnReady: m.Host.PostLadderReadyCard,
				// Operator directive 2026-07-14: evidence-met unlocks execute
				// automatically; this card is the notification + 재잠금 veto.
				OnGraduated: m.Host.PostGraduationCard,
			})
			...
			// validation state.
			m.AutonomousSvc.RegisterTask(&genesis.AdversarialCoverageTask{
				Evolver: m.GenesisEvolver,
				Tracker: m.GenesisTracker,
				Logger:  logger,
			})
			...
			if logCapture := m.Host.LogCapture(); logCapture != nil {
				m.AutonomousSvc.RegisterTask(&genesis.RuntimeErrorMiningTask{
			...
			m.AutonomousSvc.RegisterTask(&genesis.CurriculumTask{
				Evolver: m.GenesisEvolver,
				Tracker: m.GenesisTracker,
				Logger:  logger,
				...
				EnvDigest: m.curriculumEnvDigest,
			})
		}
		if replayExecutorEnabled() && isProdState {
			workoutEngine := genesis.NewSkillValidationEngine(m.GenesisTracker, logger)
			workoutEngine.SetExecutor(
				modelRegistry.Client(modelrole.RoleLightweight),
				modelRegistry.Model(modelrole.RoleLightweight),
			)
			m.AutonomousSvc.RegisterTask(&genesis.SkillWorkoutTask{
			...
			logger.Info("genesis: skill-workout lane disabled (non-production state dir)")
		}
		...
		m.GenesisTracker.SetEvolveTrigger(func() {
		...
		m.GenesisTracker.SetRollback(m.GenesisEvolver.RollbackSkillWithResult, genesis.DefaultRollbackThreshold)
	}
}

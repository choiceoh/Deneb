...
// NewSlogHandler wraps delegate so ERROR records are mirrored to n.
// When n is nil the delegate is returned unchanged — zero overhead path
// for deployments without a monitoring chat.
func NewSlogHandler(delegate slog.Handler, n *Service) slog.Handler {
...
func (n *Service) enqueueLog(body, message string) {
	if n == nil || body == "" {
		return
	}

...
func (n *Service) runHeartbeat(ctx context.Context) {
	defer func() {
		if r := recover(); r != nil {
...
func (n *Service) selfPoll(ctx context.Context) (ok bool, latency time.Duration, err error) {
	if n.boundAddr == nil {
		return true, 0, nil
	}
	addr := n.boundAddr()
...
func (n *Service) composeHangAlert(pollErr error) string {
	msg := "(unknown error)"
	if pollErr != nil {
...
func (n *Service) buildHeartbeatLine(startTime, now time.Time) string {

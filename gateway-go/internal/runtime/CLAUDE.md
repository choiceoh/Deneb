...
- **dev 게이트웨이 격리는 경로 해석기가 정한다**(`config.ResolveStateDir()` vs `os.UserHomeDir()`) — dev·live-test·puppet은 `DENEB_STATE_DIR=/tmp/…`에 **실제 `$HOME`**으로 돈다. 쓰기 가능한 Deneb 상태는 전부 상태 디렉터리 기준이다: cron·transcripts·polaris·agent-logs·workspace·세션 사이드카(모델·라벨·핀·목록 핀·포커스·저장소 바인딩, `server/state_file_path.go`)·하트비트(HEARTBEAT.md·마커·fixture)·goals·wiki-graph·files·OCR 캐시·스킬 묘비. 격리 증명은 `server/session_state_dir_isolation_test.go`(HOME≠state dir 자식 게이트웨이가 `$HOME/.deneb`를 비워 둔다). `$HOME` 기준으로 남은 것은 프로덕션 게이트(`productionStateDir`) 뒤에서만 쓰이는 genesis 원장(graduation·backlog drain·runtime error·prospect cursor — graduation은 `scripts/dev/coding-dispatch.sh`와 경로 계약)과 Deneb 상태가 아닌 호스트 자원(OAuth 크리덴셜, `~/.wormhole` 라우터 설정 — `miniapp.wormhole.set_*`은 dev에서도 실 라우터를 바꾼다, 소스 체크아웃)뿐이다. 새 스토어는 `$HOME`이 아니라 상태 디렉터리에서 경로를 만든다.

// methods_acp.go — ACP (Agent Communication Protocol) RPC method handlers.
// Exposes the ACP subsystem via Gateway RPC: status, start/stop, spawn, kill,
// list, send, bind/unbind, and binding queries.
package rpc
...
// RegisterACPMethods registers all ACP RPC methods on the dispatcher.
func RegisterACPMethods(d *Dispatcher, deps *ACPDeps) {
	if deps == nil {
		return
	}
	// Read-only methods: always available.
	d.Register("acp.status", acpStatus(deps))
	d.Register("acp.start", acpStart(deps))
	d.Register("acp.stop", acpStop(deps))
	d.Register("acp.list", acpList(deps))
	d.Register("acp.bindings", acpBindings(deps))

	// Write methods: gated on enabled state.
	d.Register("acp.spawn", acpSpawn(deps))
	d.Register("acp.kill", acpKill(deps))
	d.Register("acp.send", acpSend(deps))
	d.Register("acp.bind", acpBind(deps))
	d.Register("acp.unbind", acpUnbind(deps))
}
...
func acpStatus(deps *ACPDeps) HandlerFunc {
...
func acpStart(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
...
func acpStop(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
...
func acpList(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		var p struct {
...
func acpSpawn(deps *ACPDeps) HandlerFunc {
...
func acpKill(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		if errResp := requireEnabled(deps, req.ID); errResp != nil {
			return errResp
		}
		...
		if err := unmarshalParams(req.Params, &p); err != nil || p.AgentID == "" {
			return protocol.NewResponseError(req.ID, protocol.NewError(
...
func acpSend(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		if errResp := requireEnabled(deps, req.ID); errResp != nil {
			return errResp
		}
...
func acpBind(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		if errResp := requireEnabled(deps, req.ID); errResp != nil {
			return errResp
		}
...
func acpUnbind(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		if errResp := requireEnabled(deps, req.ID); errResp != nil {
			return errResp
		}
...
func acpBindings(deps *ACPDeps) HandlerFunc {
	return func(_ context.Context, req *protocol.RequestFrame) *protocol.ResponseFrame {
		if deps.Bindings == nil {
			return protocol.MustResponseOK(req.ID, map[string]any{
				"bindings": []any{},

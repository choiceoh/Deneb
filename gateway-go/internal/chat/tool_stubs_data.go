...
func subagentsList(children []*session.Session) string {
	...
	for i, c := range children {
		...
		// Runtime.
		if c.RuntimeMs != nil {
			parts = append(parts, commands.FormatDurationCompact(*c.RuntimeMs))
		} else if c.Status == session.StatusRunning && c.StartedAt != nil {
			elapsed := time.Now().UnixMilli() - *c.StartedAt
			parts = append(parts, commands.FormatDurationCompact(elapsed))
		}
		// Tokens.
		...
		fmt.Fprintf(&sb, "  %d. [%s] %s", i+1, status, commands.TruncateLine(label, 60))
		if len(parts) > 0 {

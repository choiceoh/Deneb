...
func Prefetch(ctx context.Context, message string, deps Deps) string {
	...
	// Early return when no knowledge sources are configured.
	if deps.MemoryStore == nil && deps.WorkspaceDir == "" && deps.UnifiedStore == nil {
		return ""
	}
	...
	// Knowledge section (memory facts). Skipped when recall already produced results.
	if len(memMatches) > 0 || len(structFacts) > 0 || len(unifiedResults) > 0 {
		parts = append(parts, formatKnowledgeWithFacts(nil, memMatches, structFacts, unifiedResults))
	}
...
// The vegaResults parameter is kept for interface compatibility but always nil.
func formatKnowledgeWithFacts(
	_ []struct{}, // legacy vega results slot (unused)
	memMatches []chattools.MemoryMatch,
	structFacts []memory.SearchResult,
	unifiedResults []unified.SearchResult,
) string {

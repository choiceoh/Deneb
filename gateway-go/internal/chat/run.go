...
func executeAgentRun(
	...
	if deps.toolProgressFn != nil && params.Delivery != nil {
		...
		hooks.OnToolResult = func(name, toolUseID, result string, isErr bool) {
			...
			evt := ToolProgressEvent{Type: "complete", Name: name, IsError: isErr}
			if isErr && result != "" {
				// Pass truncated error output for display in progress embed.
				r := result
				if len(r) > 150 {
					r = r[:150] + "…"
				}
			...
			deps.toolProgressFn(ctx, delivery, evt)
		}
	}

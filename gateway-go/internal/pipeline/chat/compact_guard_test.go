...
// TestHashMessagesReturnsSameDigestForIdenticalInput verifies that hashing the same messages slice
// twice produces the same digest — this is the invariant the idempotent-
// compaction skip relies on.
func TestHashMessagesReturnsSameDigestForIdenticalInput(t *testing.T) {
	msgs := []llm.Message{
...
// TestHashMessagesDiffersWhenRoleChanges verifies that two messages whose content
...
func TestHashMessagesDiffersWhenRoleChanges(t *testing.T) {
	a := []llm.Message{
...
// TestHashMessagesChangesWhenContentDiffers verifies that changing one byte of content
...
func TestHashMessagesChangesWhenContentDiffers(t *testing.T) {
...
// TestProtectedZoneExceedsBudgetReturnsFalseUnderBudget verifies the
// common case: a session whose head+tail fits comfortably within budget
// returns false (compaction is viable).
func TestProtectedZoneExceedsBudgetReturnsFalseUnderBudget(t *testing.T) {
	msgs := []llm.Message{
...
// TestProtectedZoneExceedsBudgetReturnsTrueWhenHeadTailOverflows verifies the thrashing
...
// of retrying.
func TestProtectedZoneExceedsBudgetReturnsTrueWhenHeadTailOverflows(t *testing.T) {
...
// TestProtectedZoneExceedsBudgetWhenAllMessagesProtected verifies the
...
func TestProtectedZoneExceedsBudgetWhenAllMessagesProtected(t *testing.T) {
...
// TestProtectedZoneExceedsBudgetDisabledWhenBudgetNonPositive verifies that a
// non-positive budget disables the guard (returns false) — used by boot
// sessions and subagents that don't configure a budget.
func TestProtectedZoneExceedsBudgetDisabledWhenBudgetNonPositive(t *testing.T) {

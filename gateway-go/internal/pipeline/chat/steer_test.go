...
func cloneMessagesForTest(in []llm.Message) []llm.Message {
	...
	for i, m := range in {
		cp := make(json.RawMessage, m.Content.Len())
		copy(cp, m.Content.Bytes())
		out[i] = llm.Message{Role: m.Role, Content: llm.FlexibleFromRaw(cp)}
	}
	return out
}
...
func messagesEqual(a, b []llm.Message) bool {
	...
	for i := range a {
		...
		if a[i].Content.String() != b[i].Content.String() {
			return false
		}
	}
	return true

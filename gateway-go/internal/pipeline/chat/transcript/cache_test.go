...
func TestCachedTranscriptStore_AppendAfterExpiryReloadsFullHistory(t *testing.T) {
	inner := newCountingStore()
	cache := NewCachedTranscriptStore(inner, time.Millisecond)
	inner.Append("resumed", NewTextChatMessage("user", "older", 0))

	if _, _, err := cache.Load("resumed", 0); err != nil {
		t.Fatalf("prime cache: %v", err)
	}
	time.Sleep(5 * time.Millisecond)
	if err := cache.Append("resumed", NewTextChatMessage("assistant", "newer", 0)); err != nil {
		t.Fatalf("Append: %v", err)
	}

	msgs, total, err := cache.Load("resumed", 0)
	testutil.NoError(t, err)
	if total != 2 || len(msgs) != 2 {
		t.Fatalf("Load after expired Append = %d messages (total %d), want full history of 2", len(msgs), total)
	}
	if msgs[0].TextContent() != "older" || msgs[1].TextContent() != "newer" {
		t.Fatalf("history order/content = %q, %q", msgs[0].TextContent(), msgs[1].TextContent())
	}
}

func TestCachedTranscriptStore_AppendBeforeFirstLoadRemainsComplete(t *testing.T) {
	inner := newCountingStore()
	inner.Append("resumed", NewTextChatMessage("user", "existing", 0))
	cache := NewCachedTranscriptStore(inner, time.Minute)

	if err := cache.Append("resumed", NewTextChatMessage("assistant", "appended", 0)); err != nil {
		t.Fatalf("Append: %v", err)
	}
	msgs, total, err := cache.Load("resumed", 0)
	testutil.NoError(t, err)

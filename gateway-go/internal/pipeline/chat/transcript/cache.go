...
func (c *CachedTranscriptStore) Append(sessionKey string, msg ChatMessage) error {
	...
	if entry, ok := c.cache[sessionKey]; ok && now.Before(entry.expiresAt) {
		...
		// Without a live entry we cannot know whether the inner store already
		// contained older messages. Leave this key cold so the next Load reads
		// the complete transcript instead of caching only the newly-appended tail.

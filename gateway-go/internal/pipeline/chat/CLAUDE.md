...
- **모델에 보여주는 과거 대화 발췌는 `ExcerptText()`로 렌더하고 `<transcript-excerpt trust="untrusted">` 봉투로 감싼다**(`tools/sessionops`, `tools/recallops`). `SearchableText()`는 검색 *매칭* 전용 — 어시스턴트 추론·툴 인자 JSON을 본문으로 되먹이면 모델이 문서 이어쓰기 모드로 빠진다(2026-09-17 사건).

...
- **mid-run 앵커**(`run_midrun_anchor.go`): 도구 결과가 마지막 메시지인 단계에 `responseLanguageAnchor`를 per-request text 블록으로 덧붙이는 `BeforeAPICall` 훅(`after:"trailing-cache"` — 캐시 마커는 깨끗한 블록에 남고 앵커 블록은 마커 없음). Ephemeral·content-prefix(kimi) 런 제외, `DENEB_MIDRUN_ANCHOR=off` 킬스위치.

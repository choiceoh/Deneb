...
- **회상·자동배달 지시문은 system이 아니라 마지막 user 메시지 꼬리로 주입**(`run_tail_inject.go`). per-turn 가변 바이트를 system에 넣으면 vLLM APC가 히스토리 전체를 무효화한다 — 근거/규칙: `docs/agent-rules/prompt-cache.md` §1.5. 꼬리의 **마지막 블록은 항상 `responseLanguageAnchor`**(기록 이어쓰기 금지 + 한국어 답변) — 새 꼬리 추가분은 그 앞에 넣는다.
- **모델에 보여주는 과거 대화 발췌는 `ExcerptText()`로 렌더하고 `<transcript-excerpt trust="untrusted">` 봉투로 감싼다**(`tools/sessionops`, `tools/recallops`). `SearchableText()`는 검색 *매칭* 전용 — 어시스턴트 추론·툴 인자 JSON을 본문으로 되먹이면 모델이 문서 이어쓰기 모드로 빠진다(2026-09-17 사건). polaris 인덱스(`pipeline/polaris/store.go`)도 같은 원칙: thinking은 **Hidden 필드**(찾되 스니펫·NextText·Wide에 절대 안 나옴), 봉투는 `toolport.WrapTranscriptExcerpt`로 공유.
- **응답 오염 가드**(`response_contamination.go`): 루프 반환 직후 한 번, 최종 답의 하드 마커(특수 토큰·우리 봉투/꼬리 헤더·트랜스크립트 행 형상, 코드펜스 안은 제외)를 탐지해 한국어 접두사만 남기고 잘라내며 `run.contamination` agentlog 이벤트를 남긴다 — Text·AllText·DeliverableText가 같은 답을 말하게 `run_exec.go`의 `runAgentWithFallbackDetailed` 직후에 배선. 한글 비율 붕괴는 소프트 신호(로그만).
- **mid-run 앵커**(`run_midrun_anchor.go`): 도구 결과가 마지막 메시지인 단계에 `responseLanguageAnchor`를 per-request text 블록으로 덧붙이는 `BeforeAPICall` 훅(`after:"trailing-cache"` — 캐시 마커는 깨끗한 블록에 남고 앵커 블록은 마커 없음). 적용 여부는 **모델의 능력**이다 — builtin `modelcaps.MidRunAnchorByDefault(provider, model)`(현재 전부 켬; kimi 포함 — per-request 블록은 히스토리 변이가 아니다) ← deneb.json `models.providers.<id>.midRunAnchor: false` 오버라이드(`Capability.NoMidRunAnchor`). Ephemeral 런은 항상 제외. 전역 env 스위치는 없다. 런당 1회 `midrun anchor active` 로그.
- **게이트웨이가 user 역할로 쓰는 노트**(`[SYSTEM: …]`·`**System:** …`, `run_delivery_failure.go`)는 표시 체인에서 `StripSyntheticSystemNotesForDisplay`로 숨기고 회상 근거에서 제외한다 — 새 노트를 추가하면 `toolport.IsSyntheticSystemNote`의 접두사 목록에 맞출 것.

...
func wikiToolSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"kinds": map[string]any{
				"type":        "array",
				"description": "프로젝트 특성 — 2단 고정 체계 '1차' 또는 '1차/2차' (프로젝트 대표페이지 write 전용, 복수 허용): 시공(EPC·턴키; 2차=토지/루프탑/수상/ESS — ESS 사업은 시공), 기자재(공급; 2차=모듈/인버터/케이블/기타), 풍력(2차=육상/해상), 개발(인허가·부지·RPS), 기타(2차=용역/협력). 2차를 모르면 1차만 적고, 확인되면 세분화. 어휘 밖 값은 드롭됨",
				"items": map[string]any{
					"type": "string",
					"enum": []string{"시공", "시공/토지", "시공/루프탑", "시공/수상", "시공/ESS", "기자재", "기자재/모듈", "기자재/인버터", "기자재/케이블", "기자재/기타", "풍력", "풍력/육상", "풍력/해상", "개발", "기타", "기타/용역", "기타/협력"},
				},
			},
			"limit": map[string]any{
				"type":        "integer",

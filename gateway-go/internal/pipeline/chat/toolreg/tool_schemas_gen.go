...
func phoneWriteToolSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"target": map[string]any{
				"type":        "string",
				"description": "액션 대상: open_url=URL, open_app=패키지/앱명, message=수신자, dial=전화번호, alarm=\"HH:MM\"(24h), timer=길이(\"10m\"/\"90s\"/\"1h30m\"/분 숫자)",
			},
			"text": map[string]any{
				"type":        "string",
			...
			"to": map[string]any{
				"type":        "string",
				"description": "작업(전부 인앱 실행): notify(알림, text 필수·title 선택) | speak(음성, text) | clipboard(클립보드에 넣기, text) | open_url(target=URL) | open_app(target=패키지/앱명) | share(text) | message(target=수신자,text) | dial(target=전화번호) | photo(카메라) | alarm(target=\"HH:MM\" 24h, text=라벨) | timer(target=길이 \"10m\"/\"90s\"/분 숫자, text=라벨)",

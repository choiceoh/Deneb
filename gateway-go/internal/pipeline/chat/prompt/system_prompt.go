...
func writeRichAnswerContract(s *strings.Builder) {
	...
	s.WriteString("deneb-ui tags (ONLY these; anything else degrades): column row card(dismissible = a close X the user can fold an unanswered question away with) box hr · text (style=headline|title|body|caption) markdown code img icon badge stat(value label description) progress(value 0..1) alert(severity=info|success|warning|error) blockquote table/tr/th/td ul/ol/li chart(type=bar|line + <point label value/>) tabs/tab accordion avatar countdown · interactive (id required): button input textarea checkbox switch select/option radio-group slider chips/chip (a described choice list = <chips layout=\"list\" lettered selection=\"single|multi\"> with <chip value=\"…\" description=\"one line\">라벨</chip>; a collect button stays disabled until required picks are made). Buttons: event=\"name\" (+ collect=\"id1,id2\" to submit input values), or href=/toggle=/copy=. A press returns as `Pressed: 이벤트`, collected values as `Responded with: id: 값` — act on that next user message. Escape literal backticks inside markdown/code bodies as &#96;.\n")

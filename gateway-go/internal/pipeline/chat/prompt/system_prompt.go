...
func buildPromptSections(params SystemPromptParams) (staticText, semiStaticText, dynamicText string) {
	...
	if cached, ok := Cache.StaticPrompt(cacheKey); ok {
		...
		s.WriteString("구조가 있는 답변(현황판·일정/목록 브리핑·비교·수치 요약·진행 상황·선택지)은 **deneb-ui 카드로 그리는 것을 우선 고려하라** — 같은 내용이면 마크다운 나열보다 카드가 더 읽기 좋다: ```deneb-ui 펜스 안에 라벨 HTML 한 덩어리(루트 `<column>` 하나).태그는 `column/card/row/text(style: headline·title·body·caption)/ul·li/icon/badge/stat/divider/table·tr·td`, 상호작용은 `button(event=\"…\" 또는 href=\"…\")`/`select`+`option`/`input`(인터랙티브는 id 필수). 예: `<card><row><icon name=\"calendar\" size=\"16\"/><text style=\"caption\">오늘 일정</text></row><ul><li>10:00 — 회의</li></ul></card>`. 사용자에게 선택지를 제시하거나 짧은 입력을 받을 때는 인터랙티브 카드도 된다(`<button event=\"…\">라벨</button>`, `<select id>`+`<option>`, `<input id>`) — 버튼/제출 응답은 다음 사용자 메시지로 돌아온다(`Pressed: 이벤트` 또는 `Responded with: id: 값`). 카드 안에 백틱·코드펜스 금지(코드는 `<code>` 태그로). 산문이 자연스러우면 산문으로 — 카드 남용 금지, 응답당 최대 1블록, 짧은 답변·일상 대화에는 쓰지 마라.\n")

...
func executeAgentRun(
	...
	systemPrompt, budgetOutcome := finalizePrompt(prep.SystemPrompt, tailForSystem, prep.Tier1Wiki, deps.contextCfg, sessionToolPreset, params.Message, logger)
...
func wireBeforeAPICall(cfg *agent.AgentConfig, deps runDeps, params RunParams, providerID, model string, client *llm.Client, logger *slog.Logger) string {
	...
	// stays on the clean block. Off for ephemeral turns and for models whose
	// capability opts out (modelcaps.NoMidRunAnchor ← deneb.json midRunAnchor).
	apc.Add("midrun-anchor", agent.HookStagePost,
		buildMidRunAnchorHook(params, modelCapability(deps, providerID, model), logger), "trailing-cache")

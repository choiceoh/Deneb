...
func TestBuildAgentConfig_PreservesCombinedPolicyAndHookContracts(t *testing.T) {
	...
	if len(usage.calls) != 0 {
		t.Fatalf("skill usage was recorded before run completion: %+v", usage.calls)
	}
	recordRunSkillUsage(usage, skillConsults, &agent.AgentResult{Text: "done", StopReason: "end_turn"}, nil, params.SessionKey, "actual-model")
	if len(usage.calls) != 1 || usage.calls[0].skill != "risk-skill" || usage.calls[0].model != "actual-model" {
		t.Fatalf("completed-run skill usage calls = %+v", usage.calls)
	}

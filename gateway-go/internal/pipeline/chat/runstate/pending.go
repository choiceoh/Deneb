...
func (pq *PendingQueue) Enqueue(sessionKey string, params Params) {
	pq.mu.Lock()
	defer pq.mu.Unlock()
...
func (pq *PendingQueue) Drain(sessionKey string) *Params {
	pq.mu.Lock()
	q, ok := pq.items[sessionKey]
	pq.mu.Unlock()
	if !ok {
...
func (q *pendingRunQueue) enqueue(params Params) {
	q.mu.Lock()
	defer q.mu.Unlock()
	q.items = []Params{params}
}

// drain removes and returns the pending message, if any.
func (q *pendingRunQueue) drain() *Params {
	q.mu.Lock()
	defer q.mu.Unlock()
	if len(q.items) == 0 {
		return nil

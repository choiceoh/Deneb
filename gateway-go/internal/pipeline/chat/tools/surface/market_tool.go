...
// MarketSummaryFunc matches the injected MarketSummary port — server adapts
// market.Cache.Summary so the agent tool and miniapp dashboard share one cache.
type MarketSummaryFunc func(ctx context.Context) (quotes []tooldeps.MarketQuote, asOf int64, stale bool, err error)

// ToolMarket returns the market tool. fetch nil is guarded at registration.
func ToolMarket(fetch MarketSummaryFunc) toolport.ToolFunc {

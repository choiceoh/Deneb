...
func ToolTranscribe(hotwords func() string) ToolFunc {
...
// → extracted text. Routes through the same extractDocument dispatcher the
...
func ToolOCR() ToolFunc {
	return func(ctx context.Context, input json.RawMessage) (string, error) {
		...
		doc := extractDocument(ctx, data, filepath.Base(p.Path), "")
		if doc.err != nil {
			return "", fmt.Errorf("ocr %q: %w", p.Path, doc.err)
		}

...
func CheckProtectedPath(path, op string) error {
	...
	// Resolve symlinks (including a symlinked nearest parent) before matching.
	// Otherwise a harmless-looking alias such as workspace/report.txt -> ~/.env
	// bypasses the suffix denylist and send_file can exfiltrate the target.
	checked := evalPathForContainment(abs)
	if protectedPathPatterns.MatchString(filepath.ToSlash(checked)) {
		return fmt.Errorf(

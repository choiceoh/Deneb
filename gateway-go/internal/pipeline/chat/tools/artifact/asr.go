...
func (f *flexStr) UnmarshalJSON(b []byte) error {
	...
	// Only a bare JSON number is valid beyond strings/null. Previously objects
	// and booleans were accepted verbatim, allowing a malformed sidecar response
	// to leak JSON syntax into a speaker label instead of failing the response.
	var n json.Number
	if err := json.Unmarshal(b, &n); err != nil {
		return fmt.Errorf("speaker must be a string, number, or null: %w", err)
	}
	// Trim a trailing ".0" so a float index like 1.0 reads as "1".
	*f = flexStr(strings.TrimSuffix(n.String(), ".0"))
	return nil
}

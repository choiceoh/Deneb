...
// VisualOutputDir returns the real-disk cache directory shared by routine,
// chart, and diagram Chromium renders.
func VisualOutputDir() string {
	return weeklyOutputDir()
}

// VisualRenderReady reports whether the host has enough commit and disk
// headroom to attempt a Chromium render without hanging or failing for ENOSPC.
func VisualRenderReady(dir string) bool {
	return weeklyCommitHeadroomMB() >= weeklyMinHeadroomMB && weeklyFreeDiskMB(dir) >= weeklyMinDiskMB
}

// ChromiumBinary returns the first supported headless Chromium executable.
func ChromiumBinary() string {
	return weeklyFindChromium()
}

// ClipRenderError bounds renderer diagnostics before they reach tool output.
func ClipRenderError(message string, maxLen int) string {
	return weeklyClip(message, maxLen)
}

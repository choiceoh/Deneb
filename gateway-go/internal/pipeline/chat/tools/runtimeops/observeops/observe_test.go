...
func TestToolObserve_RunsValidActionsAndRejectsInvalidOnes(t *testing.T) {
	...
	// behavior on an empty log still produces a formatted summary. It carries
	// no prefix-cache line: the engine's cache reuse lives under speed.
	...
	if strings.Contains(out, "prefix cache") {
		t.Errorf("behavior must not carry a prefix-cache line:\n%s", out)
	}

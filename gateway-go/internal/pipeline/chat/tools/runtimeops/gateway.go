...
type CommandRunner interface {
	// Run executes name with args in dir and returns its combined output.
...
type ProcessSignaller interface {
	// Signal sends sig to the managed gateway process.
	Signal(sig os.Signal) error
	// PID returns the managed gateway process identifier.
	PID() int
}

type selfSignaller struct{}

// Signal forwards a signal to the current process.

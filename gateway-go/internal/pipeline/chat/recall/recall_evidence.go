...
func formatRecallEvidenceAt(evidence []recallEvidence, now time.Time, filesToolReachable, sessionsToolReachable bool) (string, int) {
	...
	if sessionsToolReachable {
		...
		sb.WriteString(" source=session 행은 과거 대화의 발췌다. 발췌만으로 부족하면 sessions(action=\"history\", sessionKey=\"<그 행의 ref>\")로 해당 대화 전체를 열람하고, 관련 행이 아예 없으면 sessions(action=\"search\", query=\"<핵심 키워드>\")로 과거 대화를 직접 검색하라. 횟수·목록 질문에서는 렌더된 행이 전체의 부분집합일 수 있다 — 먼저 search로 매칭 대화를 열거한 뒤 합산하라. 합산할 때 근거 행과 search 결과에 같은 대화가 겹치면 한 번만 세라.")
	}

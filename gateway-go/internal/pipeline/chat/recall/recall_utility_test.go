...
// are recorded, Rank is the row's 1-based position in the FULL injected
// evidence list (all kinds — the position the model saw), and the query label
// and score travel with each line.
func TestRecordRecallUtility_RecordsWikiRowsWithRetrievalContext(t *testing.T) {
	...
	recordRecallUtility(store, evidence, nil)
	...
	if l := lines[0]; l.Path != "프로젝트/탑솔라.md" || l.Query != "탑솔라 계약" || l.Rank != 2 || l.Score != 0.95 {

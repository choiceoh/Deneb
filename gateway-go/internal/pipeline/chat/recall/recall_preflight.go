...
func Build(ctx context.Context, params Params, deps Deps, logger *slog.Logger) (out string, truncated bool) {
	...
	if len(evidence) == 0 {
		...
		if cue {
			...
			return appendRoutingHint(formatRecallNoEvidence(), message, params.SessionKey, logger), truncated
		}
		return "", truncated
	}
	...
	return appendRoutingHint(formatRecallEvidenceAt(evidence, deps.now()), message, params.SessionKey, logger), truncated
}

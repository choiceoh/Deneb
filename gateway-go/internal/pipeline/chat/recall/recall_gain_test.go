...
// token may appear in its own question. Build echoes the search
...
func recallOnce(store *wiki.Store, question string) string {
	out, _ := Build(context.Background(),
		Params{SessionKey: "client:main", Message: question},
		Deps{Wiki: store}, nil)
	return out
}
...
//
// applyProvenancePenalty (recall_provenance.go, wired into Build)

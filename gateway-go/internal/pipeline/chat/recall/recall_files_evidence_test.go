...
func TestBuildRecallPreflightSurfacesFileWhenTopical(t *testing.T) {
	...
	out, _ := Build(context.Background(),
		Params{SessionKey: "client:main", Message: "개발행위허가 신청서 핵심 알려줘"},
		Deps{FileRecall: search}, nil)
	...
	weather, _ := Build(context.Background(),
		Params{SessionKey: "client:main", Message: "오늘 날씨 어때"},
		Deps{FileRecall: search}, nil)

...
func WikiToolSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"action": map[string]any{
				"type":        "string",
				"description": "Action: search (ripgrep full-text search), read (wiki page(s) — one via query, several at once via paths), index (master/category index), write (create/update page), write-site (현장 페이지 생성/편집 — project=프로젝트 폴더명, title=현장명, address/status/capacity/kinds + 공정 일정 계약일·공사개시일 등을 프로젝트/<project>/현장/<title>.md 에 기록. 미지정 필드는 보존되므로 계약일만 먼저 쓰고 준공검사일은 나중에 채워도 됨), log (append diary entry), daily (recent diary), status (wiki stats), close (프로젝트 종결 — query=프로젝트명, content=결과·사유 한 줄; 폴더 전체 보관+활성 목록 제외, 삭제 아님), reopen (종결 프로젝트 재개 — query=프로젝트명), ingest (외부 자료 캡처: query=URL — 웹페이지/유튜브를 요약+발췌와 함께 자료 페이지로 영속화. 사용자가 '기억해둬/저장해둬'라며 링크를 주면 web 대신 이걸 쓴다. project=연결 프로젝트명(선택), content=메모(선택); 같은 URL은 멱등, 갱신은 force=true)",
				"enum":        []string{"search", "read", "index", "write", "write-site", "log", "daily", "status", "close", "reopen", "ingest"},
			},

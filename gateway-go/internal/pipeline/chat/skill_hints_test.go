...
func TestRecordRunSkillUsageIgnoresReviewForks(t *testing.T) {
	...
	log.Add("topsolar-db")
	recordRunSkillUsage(rec, log, &agent.AgentResult{Text: "done", StopReason: "end_turn"}, nil, "system:skill-review:cron:email:123", "m1")
	if rec.calls != 0 {
	...
	recordRunSkillUsage(rec, log2, &agent.AgentResult{Text: "done", StopReason: "end_turn"}, nil, "client:main", "m1")
	if rec.calls != 1 {

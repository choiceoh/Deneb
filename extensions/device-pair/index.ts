...
function resolveGatewayPort(cfg: DenebPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.DENEB_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: DenebPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: DenebPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Deneb devices",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target) {
            try {
              ...
              if (send) {
                await send(
                  target,
                  ["Scan this QR code with the Deneb iOS app:", "", "```", qrAscii, "```"].join(
                    "\n",
                  ),
                  {
          ...
          return {
            text: [
              "Scan this QR code with the Deneb iOS app:",
              "",
              "```",
              qrAscii,
              "```",

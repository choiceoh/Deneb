---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either Deneb ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
...
Trigger this skill when the user asks Deneb to:
...
- After reading, follow `Deneb ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. Deneb ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no Deneb ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@deneb/acpx` extension.

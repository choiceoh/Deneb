...
import { buildOutboundBaseSessionKey, normalizeOutboundThreadId } from "deneb/plugin-sdk/core";
import { resolveExecApprovalCommandDisplay } from "deneb/plugin-sdk/infra-runtime";
import { buildExecApprovalPendingReplyPayload } from "deneb/plugin-sdk/infra-runtime";
import { resolveThreadSessionKeys, type RoutePeer } from "deneb/plugin-sdk/routing";
...
function hasTelegramExecApprovalDmRoute(cfg: DenebConfig): boolean {
...
export const telegramPlugin: ChannelPlugin<ResolvedTelegramAccount, TelegramProbe> = {
  ...
  pairing: createTextPairingAdapter({
    ...
    notify: async ({ cfg, id, message }: { cfg: unknown; id: string; message: string }) => {
      const { token } = getTelegramRuntime().channel.telegram.resolveTelegramToken(
        cfg as DenebConfig,
      );
      if (!token) {
  ...
  allowlist: buildDmGroupAccountAllowlistAdapter({
    ...
    normalize: ({
      ...
      telegramConfigAdapter.formatAllowFrom!({
        cfg,
        accountId,
        allowFrom: values as (string | number)[],
      }),
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as DenebConfig;

import { resolveDefaultModelForAgent } from "deneb/plugin-sdk/agent-runtime";
import type { DenebConfig } from "deneb/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "deneb/plugin-sdk/reply-runtime";
import type { MsgContext } from "deneb/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "deneb/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "deneb/plugin-sdk/reply-runtime";
...
type LoadConfigFn = typeof import("deneb/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("deneb/plugin-sdk/config-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/deneb-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("deneb/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("deneb/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("deneb/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("deneb/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("deneb/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("deneb/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("deneb/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: DenebConfig): {
...
vi.doMock("deneb/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("deneb/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("deneb/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("deneb/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("deneb/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("deneb/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: DenebConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: DenebConfig) => {

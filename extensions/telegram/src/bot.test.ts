...
import type { PluginInteractiveTelegramHandlerContext } from "deneb/plugin-sdk/core";
...
describe("createTelegramBot", () => {
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/deneb-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/deneb-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Deneb" },
        },
      },
      me: { id: 999, username: "deneb_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });

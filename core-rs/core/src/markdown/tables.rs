...
mod tests {
    use super::*;
    use crate::markdown::render_state::{HeadingStyle, RenderState, TableMode, TableState};
    ...
    // --- render_table_as_bullets ---

    #[test]
    fn bullets_empty_table_is_noop() {
        let mut state = make_state_with_table(Vec::new(), Vec::new());
        render_table_as_bullets(&mut state);
        assert!(state.text.is_empty());
    }

    #[test]
    fn bullets_no_table_is_noop() {
        let mut state = RenderState::new(HeadingStyle::None, String::new(), TableMode::Bullets);
        render_table_as_bullets(&mut state);
        assert!(state.text.is_empty());
    }
    ...
    #[test]
    fn bullets_single_column_no_header() {
        let mut state = make_state_with_table(
            Vec::new(),
            vec![vec![make_cell("Alice")], vec![make_cell("Bob")]],
        );
        render_table_as_bullets(&mut state);
        // No header, no label column: each cell becomes a "• " bullet.
        assert!(state.text.contains("Alice"));
        assert!(state.text.contains("Bob"));
    }

    #[test]
    fn bullets_multi_column_uses_header_as_label() {
        let mut state = make_state_with_table(
            vec![make_cell("Name"), make_cell("Age")],
            vec![vec![make_cell("Alice"), make_cell("30")]],
        );
        render_table_as_bullets(&mut state);
        // First column value ("Alice") becomes row label (bolded).
        assert!(state.text.contains("Alice"));
        // Second column shown as "• Age: 30".
        assert!(state.text.contains("Age"));
        assert!(state.text.contains("30"));
    }
    ...
    fn bullets_row_label_is_bold() {
        let mut state = make_state_with_table(
            ...
            vec![vec![make_cell("Carol"), make_cell("99")]],
        );
        render_table_as_bullets(&mut state);
        let label_text = "Carol";
        let pos = state.text.find(label_text).expect("label in text");
    ...
    #[test]
    fn bullets_whitespace_trimmed_in_cells() {
        let mut state = make_state_with_table(
            vec![make_cell("  Col1  "), make_cell("  Col2  ")],
            vec![vec![make_cell("  Alice  "), make_cell("  30  ")]],
        );
        render_table_as_bullets(&mut state);
        // Trimmed values appear without extra whitespace.
        assert!(state.text.contains("Alice"));
        assert!(state.text.contains("30"));
    }

    // --- render_table_as_code ---

    #[test]
    fn code_empty_table_is_noop() {
        let mut state = make_state_with_table(Vec::new(), Vec::new());
        render_table_as_code(&mut state);
        assert!(state.text.is_empty());
    }

    #[test]
    fn code_basic_two_column_table() {
        let mut state = make_state_with_table(
            vec![make_cell("Name"), make_cell("Age")],
            vec![vec![make_cell("Alice"), make_cell("30")]],
        );
        render_table_as_code(&mut state);
        // Must contain header row and divider.
        assert!(state.text.contains("Name"));
        assert!(state.text.contains("Age"));
    ...
    #[test]
    fn code_output_has_code_block_style() {
        let mut state = make_state_with_table(
            vec![make_cell("A"), make_cell("B")],
            vec![vec![make_cell("x"), make_cell("y")]],
        );
        render_table_as_code(&mut state);
        let code_spans: Vec<_> = state
            .styles
            .iter()
            .filter(|s| s.style == MarkdownStyle::CodeBlock)
            .collect();
        assert_eq!(code_spans.len(), 1, "exactly one CodeBlock span");
        // Span must cover the entire rendered table.
        assert_eq!(code_spans[0].start, 0);
        assert!(code_spans[0].end > 0);
    }

    #[test]
    fn code_columns_padded_to_same_width() {
        let mut state = make_state_with_table(
            vec![make_cell("LongHeader"), make_cell("B")],
            vec![vec![make_cell("x"), make_cell("y")]],
        );
        render_table_as_code(&mut state);
        // Rows use pipe-delimited layout; each row must start with '|'.
        let rows: Vec<&str> = state.text.lines().collect();
        assert!(rows.len() >= 3, "header + divider + data row");
        for row in &rows {
            if !row.is_empty() {
                assert!(row.starts_with('|'), "each row starts with '|': {row}");
            }
        }
    }
}

...
mod tests {
    ...
    fn setup_health_env() -> Result<(TempDir, VegaConfig), Box<dyn std::error::Error>> {
        let tmp = tempfile::tempdir()?;
        let md_dir = tmp.path().join("projects");
        std::fs::create_dir_all(&md_dir)?;
        std::fs::write(md_dir.join("alpha.md"), "# Alpha")?;
    ...
    #[test]
    fn cmd_health_reports_healthy_when_db_exists() -> Result<(), Box<dyn std::error::Error>> {
        let (_tmp, cfg) = setup_health_env()?;

        let result = cmd_health(&json!({}), &cfg);
        assert!(result.success);
        assert_eq!(
            result.data.get("status").and_then(|v| v.as_str()),
            Some("healthy")
        );
        let checks = result
            .data
    ...
    #[test]
    fn cmd_health_reports_degraded_when_db_missing() {
        let cfg = VegaConfig {
            db_path: PathBuf::from("/tmp/deneb/nonexistent-health.db"),
            md_dir: PathBuf::from("/tmp/deneb/nonexistent-md"),
            ..VegaConfig::default()
        };
        let result = cmd_health(&json!({}), &cfg);
        assert!(result.success);
